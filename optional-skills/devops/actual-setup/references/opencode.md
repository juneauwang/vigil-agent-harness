...
is typically only in `~/.vigil/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Vigil,
...
`hermes-custom-providers` skill, pitfall 2). Vigil hits this because it forwards
