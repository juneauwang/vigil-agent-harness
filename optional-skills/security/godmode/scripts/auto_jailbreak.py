...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("VIGIL_HOME", os.path.expanduser("~/.vigil")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("VIGIL_HOME", Path.home() / ".vigil")) / "skills" / "red-teaming" / "godmode"
...
VIGIL_HOME = Path(os.getenv("VIGIL_HOME", Path.home() / ".vigil"))
CONFIG_PATH = VIGIL_HOME / "config.yaml"
PREFILL_PATH = VIGIL_HOME / "prefill.json"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Vigil config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.vigil/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Vigil for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Vigil for changes to take effect.")
