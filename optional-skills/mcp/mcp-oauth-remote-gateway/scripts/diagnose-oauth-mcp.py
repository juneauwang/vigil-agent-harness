...
Usage:
  ...
  <server_name> matches the files in $VIGIL_HOME/mcp-tokens/<server>.json etc.
...
def _hermes_home():
    # Prefer Vigil' own resolver (profile-safe); fall back to env then ~/.vigil.
    try:
        from hermes_constants import get_hermes_home
        return str(get_hermes_home())
    except Exception:
        return os.environ.get("VIGIL_HOME") or os.path.expanduser("~/.vigil")
