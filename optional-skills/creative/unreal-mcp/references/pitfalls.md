...
### 1. Start order: editor first, Vigil second

Vigil probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Vigil session.
...
restart the editor, restart the Vigil session.
...
server fails to bind (Output Log) or Vigil' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.vigil/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Vigil

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Vigil' connection
lives in `~/.vigil/config.yaml` via `vigil mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Vigil — just don't
mistake it for the Vigil setup step.
...
explicitly warns against overlapping calls. Vigil executes same-turn tool
...
### 9. Timeouts: Vigil gives up before Unreal does

Vigil' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Vigil runs elsewhere (SSH backend,
