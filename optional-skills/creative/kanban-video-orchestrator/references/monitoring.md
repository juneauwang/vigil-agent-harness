...
vigil kanban watch --tenant <project-slug>

# Snapshot of the board
vigil kanban list --tenant <project-slug>
vigil kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
vigil kanban stats --tenant <project-slug>

# Visual dashboard (browser)
vigil dashboard

# Inspect a specific task (includes comments + events)
vigil kanban show <task-id>

# Follow a single task's event stream
vigil kanban tail <task-id>
```

Verify available subcommands with `vigil kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `vigil kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `vigil kanban block <id>` then `vigil kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `vigil kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `vigil kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `vigil profile list` (profile exists?), `vigil status` (gateway/dispatcher up?), and verify tenant. |
...
vigil kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
vigil kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(vigil kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
vigil kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
vigil kanban block <task-id>
vigil kanban archive <task-id>

# Diagnose what happened
vigil kanban show <task-id>      # task body, comments, recent events
vigil kanban tail <task-id>      # follow the live event stream
vigil kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `vigil kanban create` for the director
...
while true; do
    clear
    vigil kanban list --tenant <slug>
    echo "---"
    vigil kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `vigil kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `vigil profile list` and start
