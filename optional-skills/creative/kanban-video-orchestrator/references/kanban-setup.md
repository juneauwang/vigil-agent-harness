...
the actual `setup.sh` that creates the project workspace, configures Vigil
...
Each profile has a YAML config at `~/.vigil/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.vigil/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Vigil
...
Each profile gets a `SOUL.md` at `~/.vigil/profiles/<name>/SOUL.md` that
...
the Vigil `.env` (`${VIGIL_HOME:-$HOME/.vigil}/.env`) and macOS Keychain
(if on macOS):
