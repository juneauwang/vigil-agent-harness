#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `vigil kanban list` and
...
def kanban_list(tenant: str) -> list[dict]:
    ...
    output isn't supported by the installed vigil CLI."""
    try:
        out = subprocess.run(
        ...
        pass
    # Fallback: textual parse of `vigil kanban list`
    out = subprocess.run(
...
def main():
    ...
    if not hermes_available():
        print("ERROR: 'vigil' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
