...
probe date 2026-07-02) into the Vigil tool registry with mock handlers,
...
def run_one(scenario, mode, scale, rep, out_dir: Path):
    ...
    os.environ["VIGIL_HOME"] = str(hermes_home)
    ...
    shutil.rmtree(Path(os.environ["VIGIL_HOME"]).parent, ignore_errors=True)
    return rec
