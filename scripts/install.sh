...
# Python-driven tool session (e.g. Vigil terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:juneauwang/vigil-agent-harness.git"
REPO_URL_HTTPS="https://github.com/juneauwang/vigil-agent-harness"
VIGIL_HOME="${VIGIL_HOME:-$HOME/.vigil}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${VIGIL_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$VIGIL_INSTALL_DIR"
...
#   data still at /root/.vigil (VIGIL_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.vigil/hermes-agent"
            ...
            echo "  Data, config, sessions, and logs still live in \$VIGIL_HOME"
            echo "  (default /root/.hermes).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$VIGIL_HOME/hermes-agent are preserved in-place."
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $VIGIL_HOME/hermes-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $VIGIL_HOME/hermes-agent
...
#                              $VIGIL_HOME/hermes-agent — then preserve it)
#
# Always no-op when the user set --dir or $VIGIL_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in VIGIL_HOME.
    if is_termux; then
        INSTALL_DIR="$VIGIL_HOME/hermes-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$VIGIL_HOME/hermes-agent/.git" ]; then
            INSTALL_DIR="$VIGIL_HOME/hermes-agent"
        ...
        log_info "  Data:    $VIGIL_HOME (unchanged)"
...
# so global package binaries land in $VIGIL_HOME/node/bin — which is NOT on
...
# Vigil's own global installs pass an explicit --prefix and are unaffected.
...
configure_managed_node_npm_prefix() {
    [ -x "$VIGIL_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$VIGIL_HOME/node/etc"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://github.com/juneauwang/vigil-agent-harness)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    log_info "Installing managed uv into $VIGIL_HOME/bin ..."
    mkdir -p "$VIGIL_HOME/bin"
    ...
    # directly into $VIGIL_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$VIGIL_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
check_node() {
    ...
    if [ -x "$VIGIL_HOME/node/bin/node" ] && node_satisfies_build "$("$VIGIL_HOME/node/bin/node" --version)"; then
        export PATH="$VIGIL_HOME/node/bin:$PATH"
        log_success "Node.js $("$VIGIL_HOME/node/bin/node" --version) found (Hermes-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.vigil/node/..."
    ...
    mkdir -p "$VIGIL_HOME"
    mv "$extracted_dir" "$VIGIL_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$VIGIL_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$VIGIL_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$VIGIL_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$VIGIL_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.vigil/node/"
    HAS_NODE=true
}
...
clone_repo() {
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            # cannot succeed — mirror ``vigil update`` and reset to the
...
setup_path() {
    ...
    if [ "$USE_VENV" = true ]; then
        VIGIL_BIN="$INSTALL_DIR/venv/bin/python"
        VIGIL_ENTRYPOINT="$INSTALL_DIR/hermes"
    else
    ...
    if [ ! -x "$VIGIL_BIN" ] || { [ "$USE_VENV" = true ] && [ ! -f "$VIGIL_ENTRYPOINT" ]; }; then
...
unset PYTHONPATH
unset PYTHONHOME
exec "$VIGIL_BIN" "$VIGIL_ENTRYPOINT" "\$@"
EOF
    else
        cat > "$command_link_dir/hermes" <<EOF
#!/usr/bin/env bash
...
unset PYTHONPATH
unset PYTHONHOME
exec "$VIGIL_BIN" "$INSTALL_DIR/run_agent.py" "\$@"
EOF
    else
        cat > "$command_link_dir/hermes-agent" <<EOF
#!/usr/bin/env bash
...
EOF
    ...
    # this launcher those hosts report Vigil as not installed. (#21454 applies
...
unset PYTHONPATH
unset PYTHONHOME
exec "$VIGIL_BIN" "$VIGIL_ENTRYPOINT" acp "\$@"
EOF
    else
        cat > "$command_link_dir/hermes-acp" <<EOF
#!/usr/bin/env bash
...
copy_config_templates() {
    ...
    # Create ~/.vigil directory structure (config at top level, code in subdir)
    mkdir -p "$VIGIL_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.vigil/.env (top level, easy to find)
    if [ ! -f "$VIGIL_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$VIGIL_HOME/.env"
            log_success "Created ~/.vigil/.env from template"
        else
            touch "$VIGIL_HOME/.env"
            log_success "Created ~/.vigil/.env"
        fi
    else
        log_info "~/.vigil/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$VIGIL_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.vigil/config.yaml (top level, easy to find)
    if [ ! -f "$VIGIL_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$VIGIL_HOME/config.yaml"
            log_success "Created ~/.vigil/config.yaml from template"
        fi
    else
        log_info "~/.vigil/config.yaml already exists, keeping it"
    fi
    ...
    if [ ! -f "$VIGIL_HOME/SOUL.md" ]; then
        cat > "$VIGIL_HOME/SOUL.md" << 'SOUL_EOF'
You are Hermes Agent, an intelligent AI assistant created by Nous Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.vigil/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.vigil/"
    ...
    if [ "$NO_SKILLS" = true ]; then
        ...
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.vigil/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$VIGIL_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$VIGIL_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.vigil/skills/"
            fi
        fi
    fi
}
...
maybe_start_gateway() {
    ...
    WHATSAPP_SESSION="$VIGIL_HOME/whatsapp/session/creds.json"
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $VIGIL_CMD gateway install 2>/dev/null; then
            ...
            nohup $VIGIL_CMD gateway > "$VIGIL_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
...
write_bootstrap_marker() {
    # Writes $INSTALL_DIR/.hermes-bootstrap-complete, which tells the Vigil
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $VIGIL_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $VIGIL_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $VIGIL_HOME/cron/, sessions/, logs/"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$VIGIL_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$VIGIL_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$VIGIL_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$VIGIL_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$VIGIL_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
install_desktop() {
    ...
    # `vigil desktop`, so install/repair and self-update agree about the app's
    ...
    # importing Python cannot reverse it by loading VIGIL_HOME/.env. If the
    ...
    if [ "$OS" = "macos" ] && [ -z "${CSC_LINK:-}" ] && [ -z "${APPLE_SIGNING_IDENTITY:-}" ] && command -v codesign >/dev/null 2>&1; then
        ...
        if [ -x "$config_python" ]; then
            if VIGIL_HOME="$VIGIL_HOME" "$config_python" - "$desktop_dir" <<'PYEOF'
import sys
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            # provisioned during prerequisites/node-deps (at $VIGIL_HOME/node/bin)
