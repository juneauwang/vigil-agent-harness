...
batch (tracker: [#77056](https://github.com/juneauwang/vigil-agent-harness)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Vigil home, same model, same tasks, same reps.
...
1. Create a dedicated Vigil home with credentials for the models under test:
   ...
   VIGIL_HOME=$ABEVAL_HOME vigil plugins enable observability/nemo_relay
   ```
