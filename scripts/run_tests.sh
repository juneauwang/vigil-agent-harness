...
#   * Proper venv activation (probes .venv, venv, then ~/.vigil/...)
#
# Usage:
...
# (VIGIL_PYTHON is exported by the devShell hook and ships [dev] extras:
# pytest, pytest-asyncio, pytest-timeout, ruff, ty).
#
# A candidate must have pytest INSTALLED, not merely exist. The release venv
# at ~/.vigil/hermes-agent/venv has bin/activate but no pytest, so an
...
VENV_PYTHON=""
SKIPPED_VENVS=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.vigil/hermes-agent/venv"; do
...
if [ -n "$VENV" ]; then
  PYTHON="$VENV_PYTHON"
elif [ -n "${VIGIL_PYTHON:-}" ] && [ -x "$VIGIL_PYTHON" ] \
    && "$VIGIL_PYTHON" -c 'import pytest' 2>/dev/null; then
  # Guard with an import check: VIGIL_PYTHON may point at the RELEASE
  # venv (no pytest) when inherited from a wrapped `vigil` binary rather
  # than the devShell hook.
  PYTHON="$VIGIL_PYTHON"
  echo "▶ no local venv — using Nix dev venv via VIGIL_PYTHON: $PYTHON"
else
  echo "error: no virtualenv with pytest found in $REPO_ROOT/.venv or $REPO_ROOT/venv," >&2
  echo "       and VIGIL_PYTHON is not a python with pytest (enter the Nix devShell or create a venv)" >&2
  if [ -n "$SKIPPED_VENVS" ]; then
...
if [ -f "$HOME/.vigil/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.vigil"
...
#
#   * VIGIL_TEST_WORKERS / PATHS / FILE_TIMEOUT / FILE_RETRIES / SLICE are
#     read by run_tests_parallel.py at argparse-default time — inside the
#     stripped environment.
#   * VIGIL_TEST_IMAGE is read by tests/docker/conftest.py to skip its
...
#     VIGIL_GIT_SHA build-arg the workflow bakes in).
#
# These are test-infrastructure knobs, not credentials — same class as the
# VIGIL_RUN_SLOW_PET_TESTS / VIGIL_E2E_BROWSER opt-ins already forwarded.
# Keep this an explicit allowlist (no VIGIL_TEST_* glob) so the "no
# credential can leak" property stays auditable at a glance.
TEST_ENV=()
for _test_var in VIGIL_TEST_IMAGE VIGIL_TEST_WORKERS VIGIL_TEST_PATHS \
  VIGIL_TEST_FILE_TIMEOUT VIGIL_TEST_FILE_RETRIES VIGIL_TEST_SLICE; do
  if [ -n "${!_test_var:-}" ]; then
...
exec env -i \
  ...
  PYTHONHASHSEED=0 \
  PYTHONUTF8=1 \
  ${VIGIL_RUN_SLOW_PET_TESTS:+VIGIL_RUN_SLOW_PET_TESTS="$VIGIL_RUN_SLOW_PET_TESTS"} \
  ${VIGIL_E2E_BROWSER:+VIGIL_E2E_BROWSER="$VIGIL_E2E_BROWSER"} \
