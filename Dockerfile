...
# Non-root user for runtime; UID can be overridden via VIGIL_UID at runtime
...
# tree), NOT into $VIGIL_HOME. $VIGIL_HOME (/opt/data) is a shared data
...
# `s6-setuidgid hermes` in its run script. If VIGIL_UID is unset, services
...
#
# Fix: write the commit SHA passed via the VIGIL_GIT_SHA build-arg to
...
ARG VIGIL_GIT_SHA=
RUN if [ -n "${VIGIL_GIT_SHA}" ]; then \
        printf '%s\n' "${VIGIL_GIT_SHA}" > /opt/hermes/.hermes_build_sha; \
    fi
...
# slots from $VIGIL_HOME/profiles/<name>/ after a container restart
...
# ---------- Runtime ----------
ENV VIGIL_WEB_DIST=/opt/hermes/hermes_cli/web_dist
...
ENV VIGIL_TUI_DIR=/opt/hermes/ui-tui
ENV VIGIL_HOME=/opt/data
ENV VIGIL_WRITE_SAFE_ROOT=/opt/data
ENV VIGIL_DISABLE_LAZY_INSTALLS=1
...
ENV VIGIL_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> hermes ...` they default to root, and any file the
# command writes under $VIGIL_HOME (auth.json, .env, config.yaml) ends
...
# the opt-out env var (VIGIL_DOCKER_EXEC_AS_ROOT=1).
