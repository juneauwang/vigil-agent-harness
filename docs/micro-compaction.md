...
has to be thrown away or summarized. Vigil has always done this in one batch:
...
Vigil folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Vigil's normal session flush is
...
With `micro_compact` unset or `false` Vigil behaves exactly as it always has:
...
Defaults to `$VIGIL_HOME/logs/agent.log`. It reports passes, outcome mix, net
