...
Vigil supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.vigil/config.yaml`:
...
Or set the env var: `VIGIL_KANBAN_DISPATCH_IN_GATEWAY=false`
