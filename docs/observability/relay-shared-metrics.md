...
Vigil includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Vigil and does not require `vigil plugins enable
observability/nemo_relay`. Vigil remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
Vigil execution remains available, while Relay scopes, middleware, plugins,
...
Vigil requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
Vigil installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Vigil process so configured interceptors can
...
Collection remains off unless Vigil policy enables it:
...
Vigil core owns one Relay host and one isolated Relay session scope per Vigil
...
marks do not require Vigil plugin registration. Shared-metrics marks must
...
```text
Vigil turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Vigil shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Vigil sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Vigil used for the logical call, such as
...
counter in any rolling 24-hour window. The metric has no dimensions; Vigil
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Vigil processes share the SQLite latch, so simultaneous
...
the owning Vigil session. The start counter contains only bounded execution
...
provider attempts for the same Vigil API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Vigil tool-call
...
cancellations. Active task ownership follows the task ID if Vigil rotates its
...
outcome, approval outcome, latency, and explicit retry-count buckets. Vigil
...
tool or plugin names. Vigil does not infer retries from repeated tool names or
...
state, and a use-count bucket. Vigil derives reuse and patch-generation
...
```text
$VIGIL_HOME/telemetry/shared_metrics/metrics.sqlite3
$VIGIL_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Vigil version,
...
schema field name, its current scope is one `VIGIL_HOME`, so it is more
...
Deleting `$VIGIL_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `VIGIL_HOME`. To reset it, stop Vigil
processes and remove `$VIGIL_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Vigil CLI turn against the deterministic local model server:
