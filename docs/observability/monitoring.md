...
Vigil gateway daemon, exported over OTLP/HTTP to an operator-configured
...
a plain `vigil gateway run`: the exporter lives in the gateway process, so
...
Vigil-owned Relay transport health remains in scope. A separate gateway
...
```bash
vigil monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `vigil monitoring` CLI intentionally exposes `status` only. This first
release covers only Vigil-owned service-health and operational-diagnostic
signals, including Vigil-owned Relay transport health. Team Gateway's
