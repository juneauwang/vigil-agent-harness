# Vigil Observer Hooks

Vigil observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Vigil' planner, model providers, memory, tool registry,
...
Vigil also has a first-party NeMo Relay shared-metrics path. It uses these
...
Hook callbacks are fail-open. Vigil catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Vigil only builds
