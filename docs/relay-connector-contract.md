...
This document is the formal interface between the **Vigil gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `vigil gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`vigil gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `vigil` CLI.
