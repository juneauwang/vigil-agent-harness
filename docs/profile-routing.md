...
`vigil config set gateway.profile_routes ...` writes).
...
| `profile` | yes | Target profile name (must exist under `~/.vigil/profiles/<name>`). |
...
activates the per-profile runtime scope (per-profile `VIGIL_HOME`, secret scope, and
