...
# 5. Symlinks the 'vigil' CLI command into a user-facing bin dir
...
# ============================================================================
# PATH setup — symlink vigil into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up vigil command..."

VIGIL_BIN="$SCRIPT_DIR/venv/bin/vigil"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$VIGIL_BIN" "$COMMAND_LINK_DIR/vigil"
echo -e "${GREEN}✓${NC} Symlinked vigil → $COMMAND_LINK_DISPLAY_DIR/vigil"
    ...
    if [ -n "$SHELL_CONFIG" ]; then
        ...
        if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
            if ! grep -q '\.local/bin' "$SHELL_CONFIG" 2>/dev/null; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Vigil Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
# ============================================================================
# Seed bundled skills into ~/.vigil/skills/
# ============================================================================

VIGIL_SKILLS_DIR="${VIGIL_HOME:-$HOME/.vigil}/skills"
mkdir -p "$VIGIL_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.vigil/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$VIGIL_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  vigil status        # Check configuration"
if is_termux; then
    echo "  vigil gateway       # Run gateway in foreground"
else
    echo "  vigil gateway install # Install gateway service (messaging + cron)"
fi
echo "  vigil cron list     # View scheduled jobs"
echo "  vigil doctor        # Diagnose issues"
echo ""
