 ...
 * load, scrapes the `window.__VIGIL_SESSION_TOKEN__` assignment, and
 * re-injects it into the dev HTML. No-op in production builds.
 */
function hermesDevToken(): Plugin {
  const TOKEN_RE = /window\.__VIGIL_SESSION_TOKEN__\s*=\s*"([^"]+)"/;
  const EMBEDDED_RE =
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__VIGIL_SESSION_TOKEN__="${match[1]}";` +
              `window.__VIGIL_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};`,
          },
        ];
      } catch (err) {
        console.warn(
