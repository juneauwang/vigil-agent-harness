...
/**
 * 详情抽屉（批三十五）：右侧滑出，替代卡片内展开（DetailBox）。
 * 内容 = 顶部标题（实体名 + EnvBadge + StatusPill）+ DetailTree 全宽渲染，
 * 长命令不折行（DetailTree 字符串走 whitespace-pre + 横向滚动）；遮罩点击
 * 关闭。复用 CSS 变量 + 现有 dropdown/审批 modal 的阴影层次。
 */
export default function DetailDrawer({
  ...
  return (
    <div className="fixed inset-0 z-[90] flex justify-end" role="dialog" aria-label={`${entity.name} 详情`}>
      <div className="absolute inset-0 bg-black/50" onClick={onClose} aria-label="关闭详情遮罩" />
