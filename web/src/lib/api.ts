...
export const api = {
   ...
   * redacted (_redact_value). Both are PUBLIC_API_PATHS GETs.
   */
...
// ── Ops dashboard read-only views (UI 壳第一批) ───────────────────────────
// Shapes mirror the sanitized payloads served by web_server.py
// (/api/topology, /api/runbooks). Credential material never appears:
// topology goes through build_view (recursive key-strip + value redaction)
// and runbook detail through _redact_value-level value redaction.
