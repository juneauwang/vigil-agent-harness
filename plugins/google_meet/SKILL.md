...
vigil meet install                 # pip deps + Chromium (transcribe only)
vigil meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
vigil meet auth                    # optional; skips guest-lobby wait
vigil meet setup                   # preflight checks
```

`vigil meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or VIGIL_MEET_REALTIME_KEY in ~/.vigil/.env
```
...
pip install playwright websockets && python -m playwright install chromium
vigil plugins enable google_meet
vigil meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
vigil meet node approve my-mac ws://<mac-ip>:18789 <token>
vigil meet node ping my-mac                   # confirm reachable
```

Run `vigil meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Vigil agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `vigil meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `VIGIL_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
Local:
```
$VIGIL_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$VIGIL_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `vigil meet node approve`.
