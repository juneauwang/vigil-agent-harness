...
Let the vigil agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where vigil runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `vigil meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `vigil meet` CLI |
...
| `cli.py` | `vigil meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$VIGIL_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `vigil meet node {run,list,approve,remove,status,ping}` |
...
vigil meet install                                      # pip + Chromium
vigil meet setup                                        # preflight
vigil meet auth                                         # optional
vigil meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
vigil meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.vigil/.env
vigil meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
vigil meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
vigil meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.vigil/.env
vigil meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, vigil will **not** switch your system audio input automatically — the
...
vigil plugins enable google_meet
vigil meet node run --display-name my-mac --host 0.0.0.0 --port 18789
