"""nemo_relay — optional Vigil plugin for NeMo Relay observability."""
...
class _Settings:
    ...
    atif_agent_name: str = "Vigil Agent"
        ...
        with self._lock:
            ...
            if self._owners:
                ...
                logger.warning(
                    ...
                    "Vigil profile; keeping the existing process-global configuration "
...
class _Runtime:
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if relay_session is None:
            raise RuntimeError("Vigil core Relay session is unavailable")
...
def register(ctx) -> None:
    ...
    # Activate dynamic plugins before Vigil installs the managed execution
    ...
    with _LOCK:
        ...
        try:
            ...
            if resolved_host is None:
                raise RuntimeError("Vigil core Relay runtime is unavailable")
            runtime = _Runtime(
...
def _load_settings() -> _Settings:
    plugins_toml_path = _env("VIGIL_NEMO_RELAY_PLUGINS_TOML")
    ...
    return _Settings(
        ...
        atof_enabled=_env_bool("VIGIL_NEMO_RELAY_ATOF_ENABLED"),
        atof_output_directory=_env("VIGIL_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY"),
        atof_filename=_env("VIGIL_NEMO_RELAY_ATOF_FILENAME") or "hermes-atof.jsonl",
        atof_mode=_env("VIGIL_NEMO_RELAY_ATOF_MODE") or "append",
        atif_enabled=_env_bool("VIGIL_NEMO_RELAY_ATIF_ENABLED"),
        atif_output_directory=_env("VIGIL_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY"),
        atif_filename_template=_env("VIGIL_NEMO_RELAY_ATIF_FILENAME_TEMPLATE") or "hermes-atif-{session_id}.json",
        atif_subagent_export_mode=_atif_subagent_export_mode(),
        atif_agent_name=_env("VIGIL_NEMO_RELAY_ATIF_AGENT_NAME") or "Vigil Agent",
        atif_agent_version=_env("VIGIL_NEMO_RELAY_ATIF_AGENT_VERSION") or "unknown",
        atif_model_name=_env("VIGIL_NEMO_RELAY_ATIF_MODEL_NAME") or "unknown",
    )
...
def _dynamic_plugin_specs(
    ...
    if plugins_section is not None:
        ...
        if plugins_section:
            logger.error(
                "Vigil cannot activate Relay gateway [[plugins.dynamic]] records because "
...
def _atif_subagent_export_mode() -> str:
    mode = _env("VIGIL_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE").lower()
