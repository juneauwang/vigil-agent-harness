...
This plugin connects Vigil to iMessage (and other Spectrum
...
TypeScript-only, Vigil runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Vigil channel,
...
Run `vigil photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.vigil/.env` (the same place every other
...
Management metadata lives in `~/.vigil/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `vigil photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Vigil
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Vigil coalesces
...
- **Native polls are supported.** Vigil posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `vigil send`) cannot spawn the sidecar;
...
6. Verify end-to-end: `vigil photon status`, a DM and a group roundtrip,
