"""
Photon Spectrum (iMessage) platform adapter for Vigil Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Vigil dep
...
#
# Out-of-process senders (cron subprocesses, `vigil send`, the dashboard)
...
def _runtime_record_path() -> Path:
    # get_hermes_home() honors profile overrides — never hardcode ~/.vigil.
...
# this module (vigil status, test collection, plugin discovery).
...
# artifacts so Vigil sees the link once, not a follow-up "(attachment)" prompt.
...
def sidecar_deps_installed() -> bool:
    ...
    _start_sidecar(), and `vigil photon status` so all three agree on
...
def check_requirements() -> bool:
    ...
    if not sidecar_deps_installed():
        ...
        # user has no CLI to run `vigil photon setup`, so the connect path
        # must self-heal). Otherwise keep returning False so
        # `vigil setup` / status surface the missing-deps state.
        ...
        # (load_gateway_config, vigil status, GET /api/status polling) —
...
def _sidecar_deps_stale() -> bool:
    ...
    `vigil update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``vigil photon install-sidecar``: ``npm ci`` for an exact,
...
def _richlink_candidate(text: str) -> Optional[str]:
    ...
    markdown/text path so Vigil does not drop labels or rewrite intent.
    """
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Vigil defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Vigil-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Vigil checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not sidecar_deps_installed():
            ...
            # `vigil photon setup`, so the connect path must be able to
        ...
        # A `vigil update` that bumps the spectrum-ts pin rewrites
            ...
            while time.time() < deadline:
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        # (cron, `vigil send`) can reach this sidecar
...
async def _standalone_send(
    ...
    *,
    ...
    if not token:
        ...
        # the gateway process env otherwise, so cron/`vigil send` would be
        ...
        if not token:
            return {
                "error": (
                    ...
                    "Start the Vigil gateway (which spawns the sidecar and "
    ...
    headers = {"X-Vigil-Sidecar-Token": token}
