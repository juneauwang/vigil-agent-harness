// Vigil Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Vigil): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Vigil -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Vigil-Sidecar-Token: ${TOKEN}`):
...
const PROBE_MSG_PREFIX = "vigil-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Vigil'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Vigil allowlists cannot cause or fix this:
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Vigil supplied them so a known-good
