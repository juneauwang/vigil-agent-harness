...
Small Node helper that bridges Vigil to Photon's Spectrum SDK
(`spectrum-ts`).  Vigil is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Vigil-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Vigil can receive inbound messages
...
`Space.send(...)` API. Vigil is Python, so replies go through this sidecar
