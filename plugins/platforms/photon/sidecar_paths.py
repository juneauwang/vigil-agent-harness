...
under an immutable ``/opt/vigil`` (read-only for the vigil user), which
...
4. Source dir read-only and deps missing or stale → mirror the sidecar
   source files to ``$VIGIL_HOME/photon/sidecar`` (the durable data volume,
