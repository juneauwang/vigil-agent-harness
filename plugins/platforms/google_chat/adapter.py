...
# ``model_tools`` import time, so ``vigil status``, ``vigil chat``, etc.
...
#     fresh "Vigil is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a vigil MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${VIGIL_HOME}/google_chat_thread_counts.json so
        ...
        # they don't sit in the chat forever as "Vigil is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("VIGIL_HOME", str(_Path.home() / ".vigil"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${VIGIL_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a vigil MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Vigil is thinking…' marker message.
        ...
        "Vigil is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Vigil is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Vigil is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Vigil is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``vigil setup``.
    ...
    prompt for env vars, persist them to ``~/.vigil/.env`` so the next
    ...
    print()
    print_success("Google Chat configuration saved to ~/.vigil/.env")
    print_info("Restart the gateway: vigil gateway restart")
...
def register(ctx) -> None:
    """Plugin entry point — called by the Vigil plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `vigil setup` to install Google Chat support.",
        ...
        # the "Vigil is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Vigil is thinking…' marker message "
