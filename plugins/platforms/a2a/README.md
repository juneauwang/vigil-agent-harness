# A2A — Agent-to-Agent protocol for Vigil
...
peer (another Vigil, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
vigil gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Vigil serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.vigil/a2a_audit.jsonl`.
- Conversations persist to `~/.vigil/a2a_conversations/` — they survive context
