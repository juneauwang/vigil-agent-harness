...
# trigger its own recovery path. Refs: juneauwang/vigil-agent-harness#59614
...
# _UPDATER_STOP_TIMEOUT. Refs: juneauwang/vigil-agent-harness#66377
_DRAIN_TIMEOUT = 15.0
...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = env_float("VIGIL_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", 0.8)
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "VIGIL_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "VIGIL_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    # older PTB/clients. Streaming edits stay on Vigil' existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        Vigil must skip rich delivery up front and use the legacy MarkdownV2
        ...
        try:
            if app and app.updater and app.updater.running:
                try:
                    ...
                    # Refs: juneauwang/vigil-agent-harness#58270
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # readable by the Vigil process (shared mount, same machine, etc.).
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("VIGIL_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("VIGIL_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("VIGIL_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("VIGIL_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("VIGIL_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            ...
            disable_fallback = (os.getenv("VIGIL_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            _max_connect = 8
            _init_timeout = _env_float("VIGIL_TELEGRAM_INIT_TIMEOUT", 30.0)
                ...
                try:
                    ...
                    if asyncio.get_running_loop().time() >= _total_deadline:
                        raise OSError(
                            ...
                            f"or set VIGIL_TELEGRAM_HTTP_CONNECT_TIMEOUT / "
                            f"VIGIL_TELEGRAM_INIT_TIMEOUT to a lower value."
                        )
                    logger.warning(
                    ...
                    if _attempt < _max_connect - 1:
                        ...
                        raise OSError(
                            ...
                            f"or set VIGIL_TELEGRAM_HTTP_CONNECT_TIMEOUT to a lower value."
                        )
    ...
    # Scripts live in ~/.vigil/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".vigil" / "scripts" / "gmail-triage" / script_name
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Vigil bot profiles. A message like
    ...
    def _is_own_message(self, message: Message) -> bool:
        ...
        counted as incoming unread messages in the Vigil inbox.
        """
        if not self._bot:
            return False
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        # unread in the Vigil inbox (#52363).
        #
...
def _resolve_notifications_mode() -> str:
    ...
    mode = os.getenv("VIGIL_TELEGRAM_NOTIFICATIONS", "")
    if not mode:
        try:
