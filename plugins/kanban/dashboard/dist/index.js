/**
 * Vigil Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.vigil/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__VIGIL_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const DOCS_URL = "https://github.com/juneauwang/vigil-agent-harness";
  const DOCS_TUTORIAL_URL = "https://github.com/juneauwang/vigil-agent-harness";
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__VIGIL_SESSION_TOKEN__ directly (the old
  ...
  function DocsLink() {
    return h("a", {
      ...
      title: "Open Vigil Kanban docs in a new tab",
      "aria-label": "Vigil Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `vigil kanban decompose <id>`) and tasks
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Vigil profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "hermes-kanban-bulk" },
      ...
      h("div", { className: "hermes-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Vigil profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
            ...
            h("div", { className: "flex flex-col gap-1 flex-1" },
              ...
              h(Input, {
                ...
                title: props.columnName === "triage"
                  ? "Vigil profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
                  : "Vigil profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  if (window.__VIGIL_PLUGINS__ && typeof window.__VIGIL_PLUGINS__.register === "function") {
    window.__VIGIL_PLUGINS__.register("kanban", KanbanPage);
  }
})();
