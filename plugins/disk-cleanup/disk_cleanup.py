"""disk_cleanup — ephemeral file cleanup for Vigil Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under VIGIL_HOME)
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$VIGIL_HOME/logs/``."""
...
# Paths under $VIGIL_HOME that must NEVER be deleted by quick(),
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside VIGIL_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under VIGIL_HOME, but never recurse into known
