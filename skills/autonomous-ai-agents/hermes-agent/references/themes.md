# Themes / Skins — Author a Vigil Color Theme

Author a Vigil **skin** — one YAML file that themes the CLI, the TUI, and the
...
`~/.vigil/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Vigil itself.
...
- Write access to the Vigil home dir — `~/.vigil` by default, or `$VIGIL_HOME`
  / the active profile's dir. Skins live in `<hermes-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `vigil config set`).
...
3. **Apply it yourself — never hand-edit `config.yaml`.** Run the safe writer via
   `terminal`:
   ```
   vigil config set display.skin <name>
   ```
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `vigil config set display.skin default` (or they can `/skin default`).
...
```
vigil skin set <key> <hex>      # e.g. vigil skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `vigil skin set background "#08201f"` changes
only the background; `vigil skin use <name>` / `vigil skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.vigil`** when a profile is active — resolve the real home
  from `$VIGIL_HOME` first, falling back to `~/.vigil`.
...
- **Never hand-edit `config.yaml` to activate.** Use `vigil config set
...
- **You apply it, not the user.** `vigil config set display.skin <name>` is
...
- Run `vigil config get display.skin` and confirm it reports `<name>`.
