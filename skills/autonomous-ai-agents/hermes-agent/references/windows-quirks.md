...
Vigil runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`vigil config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Vigil's env scrubber dropping
...
every Vigil tool and most Windows APIs. Prefer forward slashes in code
