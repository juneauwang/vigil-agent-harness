...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Vigil agent runs by POSTing events to a URL.
...
All management is via the `vigil webhook` CLI command:

### Create a subscription
```bash
vigil webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.vigil/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.vigil/scripts/` (path traversal is blocked).

```bash
vigil webhook subscribe todoist-vigil \
  --prompt "Task changed: {payload.content}" \
  --script "todoist-hermes-label.py" \
  --deliver telegram --deliver-chat-id "12345"
```

Full filter syntax: https://github.com/juneauwang/vigil-agent-harness
...
### GitHub: PR reviews
```bash
vigil webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
vigil webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
vigil webhook subscribe alerts \
...
- Subscriptions persist to `~/.vigil/webhook_subscriptions.json`

## How It Works

1. `vigil webhook subscribe` writes to `~/.vigil/webhook_subscriptions.json`
...
3. **Check gateway logs:** `grep webhook ~/.vigil/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `vigil webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `vigil webhook test <name>` to verify the route works.
