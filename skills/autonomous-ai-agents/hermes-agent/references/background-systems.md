...
the `cronjob` tool, the `vigil cron` CLI (`list`, `add`, `edit`,
...
User docs: https://github.com/juneauwang/vigil-agent-harness
...
- **CLI:** `vigil curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `vigil curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.vigil/skills/.usage.json` holds
...
Users drive it via `vigil kanban <verb>`; dispatcher-spawned workers
see a focused `kanban_*` toolset gated by `VIGIL_KANBAN_TASK`, and
...
- **Isolation:** board is the hard boundary (workers get
  `VIGIL_KANBAN_BOARD` pinned in env); tenant is a soft namespace
