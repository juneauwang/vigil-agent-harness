...
## Layer 1 — Is this thing a Vigil plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Vigil process**. Its LLM calls go through Vigil's already-configured provider. | Already uses Portal if user's Vigil is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Vigil. That LLM call is already authenticated
via Vigil's provider config. The plugin's own server auth (e.g.
...
What Vigil has that *feels* like OAuth — `vigil login --provider nous`
opening a browser, user signs in, token lands in `~/.vigil/auth.json` — is a
**Hermes-specific browser flow**. Under the hood it produces a credential
Vigil uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Vigil's existing Portal credential out of `~/.vigil/auth.json`.
...
rides on the credential Vigil already holds.

Where this could live in Vigil:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Vigil itself.
...
1. First decide: Vigil plugin (runs inside Vigil) or separate app? If plugin,
   it already uses Portal via Vigil's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Vigil agent itself") is fully supported. Vigil treats agent
...
pitch in the Tool Gateway announcement is specifically about Vigil's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Vigil is configured to use Portal as its provider. It is
