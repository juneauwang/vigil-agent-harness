...
Author widget apps for the Vigil TUI (`vigil --tui`): glanceable ambient
...
- The TUI must be in use (`vigil --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.vigil/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.vigil/logs/tui_gateway_crash.log` if a widget never appears.
