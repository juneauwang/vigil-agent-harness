...
Vigil injects project-level instructions into the system prompt by reading context files from the working directory. The discovery order is **first match wins** — only one project context source is loaded per session.
...
| `AGENTS.md` / `agents.md` | **Cwd only** — subdirectory and parent copies are ignored | You want portable agent instructions that work the same in Vigil, Claude Code, Codex, etc. |
| `CLAUDE.md` / `claude.md` | Cwd only | Same as AGENTS.md, Claude-flavored |
| `.cursorrules` / `.cursor/rules/*.mdc` | Cwd only | Migrating from Cursor |

`SOUL.md` (in `$VIGIL_HOME`) is independent and always loaded when present — it sets the agent's identity, not project rules.
...
- **Don't put project rules in `~/.vigil/AGENTS.md`** (or any other home-level location). When Vigil runs with that directory as cwd, the file loads — but only for that one directory. For cross-project context, use `SOUL.md` (in `$VIGIL_HOME`, identity-only) or install a skill via `vigil skills install`.
...
`vigil --ignore-rules` skips auto-injection of all project context files (`.hermes.md`, `AGENTS.md`, `CLAUDE.md`, `.cursorrules`) **and** `SOUL.md` identity, plus user config, plugins, and MCP servers. Use it to isolate whether a problem is your setup or Vigil itself.
...
Vigil: when working in this repo, follow these rules.
...
That file at `/home/me/projects/myrepo/.hermes.md` is auto-loaded when Vigil runs in any subdirectory of `/home/me/projects/myrepo`, but not when it runs in `/home/me/other-project`.
