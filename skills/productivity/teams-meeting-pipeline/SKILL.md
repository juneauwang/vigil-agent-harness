...
Everything operator-facing is a `vigil teams-pipeline` subcommand run via the terminal tool. There are no new model tools for this pipeline — the CLI is the surface.
...
Before using the pipeline, verify these are set in `${VIGIL_HOME:-~/.vigil}/.env`:
...
```bash
vigil teams-pipeline validate              # config snapshot — run first after any change
vigil teams-pipeline token-health          # Graph token status
vigil teams-pipeline token-health --force-refresh   # force a fresh token acquisition
vigil teams-pipeline list                  # recent meeting jobs
vigil teams-pipeline list --status failed  # only failed jobs
vigil teams-pipeline show <job-id>         # full detail of one job
vigil teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
vigil teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
vigil teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
vigil teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```
...
vigil teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
vigil teams-pipeline delete-subscription <sub-id>
vigil teams-pipeline maintain-subscriptions            # renew near-expiry ones
vigil teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
...
1. Run `vigil teams-pipeline subscriptions` — if it's empty or all entries show `expirationDateTime` in the past, that's the cause.
2. Recreate with `subscribe` as shown above.
3. **Set up automated renewal immediately** via `vigil cron add`, a systemd timer, or plain crontab. The operator runbook at `/docs/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production` has all three options. 12-hour interval is safe (6x headroom against the 72h limit).
