...
## 5. Vigil Configuration

### Config location

`$VIGIL_HOME/config.yaml` (defaults to `~/.vigil/config.yaml` when `VIGIL_HOME` is unset)
...
Restart the Vigil session for changes to take effect. The MCP connection is
