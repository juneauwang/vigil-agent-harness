...
VIGIL_HOME_DIR="${VIGIL_HOME:-$HOME/.vigil}"
VIGIL_CFG="${VIGIL_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Vigil config has twozero_td MCP entry ──
if [[ ! -f "$VIGIL_CFG" ]]; then
    echo -e " ${FAIL} Hermes config not found at ${VIGIL_CFG}"
    manual_steps+=("Create ${VIGIL_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$VIGIL_CFG" 2>/dev/null; then
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${VIGIL_CFG} manually"); }
