"""Slash command definitions and autocomplete for the Vigil CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Vigil config/state", "Session",
    ...
    CommandDef("goal", "Set a standing goal Vigil works on across turns until achieved", "Session",
    ...
    CommandDef("wake", "Toggle the 'Hey Vigil' wake word listener", "Configuration",
               cli_only=True, args_hint="[on|off|status]",
               subcommands=("on", "off", "status")),
    CommandDef("busy", "Control what Enter does while Vigil is working", "Configuration",
    ...
    CommandDef("update", "Update Vigil to the latest version", "Info",
               busy_policy="dispatch"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    Telegram command menu, in Slack's ``/vigil`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Vigil ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Vigil
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``vigil skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``vigil skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``vigil skills list`` and
...
# unrelated command landed. These claim their slots right after /vigil,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /vigil <command>).
...
# ``/vigil <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/vigil" decision, not a silent clamp.
#   - debug: the log/report upload surface; reached via /vigil debug on Slack.
#   - egress: Docker-only proxy status; reachable as /vigil egress on Slack.
#   - init: repo-scan AGENTS.md bootstrap — a cwd-centric dev command that is
#     rare from Slack; reachable as /vigil init. Without this entry, adding
#     /init clamps /version off the native list and breaks Telegram parity.
#   - version: low-frequency info command; reachable as /vigil version on
...
#   - diff: git working-tree diff; reached via /vigil diff on Slack so it
...
#     /vigil update on Slack. Demoted to free the native slot /approvals now
...
#   - heartbeat: session heartbeat management; reached via /vigil heartbeat
#     on Slack. Added at the 50-cap — a native slot would clamp /insights.
#   - refine: on-demand memory/skill review; reached via /vigil refine on
#     Slack. Added at the 50-cap — a native slot would clamp an existing
#     native slash.
_SLACK_VIA_VIGIL_ONLY = frozenset({"debug", "egress", "init", "version", "diff", "update", "heartbeat", "refine"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/vigil <verb>`` subcommand.
    ...
    can still reach them via ``/vigil <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/vigil`` is always reserved as the first entry so the
    legacy ``/vigil <subcommand>`` form keeps working for anything that
    ...
    # Reserve /vigil as the catch-all top-level command.
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_VIGIL_ONLY:
            # Intentionally Slack-via-/vigil only (see _SLACK_VIA_VIGIL_ONLY).
            return
    ...
    # everything except /vigil, so a new canonical command can never silently
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /vigil handler.

    Maps both canonical names and aliases so /vigil bg do stuff works
    the same as /vigil background do stuff.

    Plugin-registered slash commands are included so ``/vigil <plugin-cmd>``
