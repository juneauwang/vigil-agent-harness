...
_STATUS_LABELS = {
    "active": "active（沉淀完成，下次跳过审批）",
    "pending": "pending（计数中）",
    "banned": "banned（被拒过，永不自动沉淀）",
    "inactive": "inactive（已撤销，回到弹审批）",
}
...
def run_list(args) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not rows:
        print("（空：approval_memory.yaml 不存在或无数目——仅内置种子在运行时生效）")
        return 0
    header = f"{'模板':<16}{'状态':<46}{'次数':>4}  {'免问':<5}{'沉淀时间':<28}{'来源'}"
    print(header)
    print("-" * len(header))
    for row in rows:
        ...
        print(
            f"{row['template']:<16}{_STATUS_LABELS.get(row['status'], row['status']):<46}"
            f"{count_s:>4}  {opt:<5}{approved_at:<28}{_row_kind(row)}"
        )
    print()
    print("说明：active = 沉淀完成下次跳过审批；banned = 被用户拒过永不自动沉淀"
...
def run_forget(args) -> int:
    ...
    try:
        ...
        if not template:
            print("✗ 需要指定要撤销的命令模板（如：vigil approvals forget lscpu）",
                  file=sys.stderr)
            return 2
        ok = forget(template, home=_memory_home())
    except Exception as exc:
        print(f"✗ 撤销失败：{exc}", file=sys.stderr)
        return 2
    if not ok:
        print(f"✗ 模板 {template!r} 不在白名单（非种子也非已沉淀条目），无需撤销。",
              file=sys.stderr)
        return 1
    print(
        f"✓ 已撤销 {template!r}（置 inactive、计数清零）：该命令回到弹审批状态。"
