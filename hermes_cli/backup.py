"""
Backup and import commands for vigil CLI.

`vigil backup` creates a zip archive of the entire ~/.vigil/ directory
(excluding the hermes-agent repo and transient files).

`vigil import` restores from a backup zip, overlaying onto the current
VIGIL_HOME root.
"""
...
# VIGIL_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    ...
    # Python dependency trees (plugin / MCP-server venvs under VIGIL_HOME) —
...
# File names that ``vigil import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE VIGIL_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Vigil backup slot."""
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Vigil backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside VIGIL_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to vigil root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside VIGIL_HOME.
...
# to the user as a hung `vigil update` (#70553 follow-up). Sessions databases
...
def run_backup(args) -> None:
    """Create a zip backup of the Vigil home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: Vigil home directory not found at {hermes_root}")
        sys.exit(1)
...
def _run_backup_locked(args, hermes_root: Path) -> None:
    ...
    # outside VIGIL_HOME, so the walk above never sees it. Ask the active
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Vigil backup.
    ...
    # Look for telltale files that a vigil home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Vigil backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.vigil/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a vigil dir name
        if prefix in {".vigil", ".hermes", "hermes", "vigil"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Vigil configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under VIGIL_HOME.
        ...
        print("Done. Your Vigil configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and vigil backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to VIGIL_HOME).
...
# — `vigil update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $VIGIL_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside VIGIL_HOME and the entry is silently
...
# Relative path of the cron job database inside VIGIL_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``vigil update``.
    ...
    Args:
        ...
        hermes_home: Override for the Vigil home directory (tests).
...
def run_quick_backup(args) -> None:
    """CLI entry point for vigil backup --quick."""
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `vigil claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of VIGIL_HOME under ``backups/`` before a
    ``vigil claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``vigil import <archive>``.  Writes to
    ``<VIGIL_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `vigil import` and the
