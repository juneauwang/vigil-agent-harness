"""
vigil fallback — manage the fallback provider chain.
...
Subcommands:
  ...
  vigil fallback add      Pick provider + model via the same picker as `vigil model`,
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `vigil model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  vigil fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `vigil model`, then append the selection to the chain."""
    ...
    print("  `vigil model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `vigil fallback list` to view, or `vigil fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``vigil fallback [subcommand]``."""
