...
on Japanese locales, etc.).  Vigil's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Vigil is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``VIGIL_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("VIGIL_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Vigil-managed Git install directories so
    ...
    # already-running shells don't see the change — which means vigil
    ...
    # install without Vigil-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Vigil's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Vigil's
    ``vigil config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Vigil (User env var in Windows Settings, or
    export in a PowerShell profile) and Vigil picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Vigil-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\vigil\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``vigil`` from right after install) retain their old PATH.

    Any subprocess Vigil spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Vigil-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "vigil", "git", "cmd"),
        os.path.join(local_appdata, "vigil", "git", "bin"),
        os.path.join(local_appdata, "vigil", "git", "usr", "bin"),
        # Windows launcher venv Scripts directory — host of the legacy launcher shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes vigil, but harmless to include.
        os.path.join(local_appdata, "vigil", "hermes-agent", "venv", "Scripts"),
