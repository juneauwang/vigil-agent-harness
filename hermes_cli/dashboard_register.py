"""``vigil dashboard register`` — register a self-hosted dashboard OAuth client.
...
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.vigil/.env``
as ``VIGIL_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     (``~/.vigil/auth.json``), refreshing it if needed. Fails fast with a
     "run `vigil setup`" hint when the user isn't logged in.
  ...
  3. Writes ``VIGIL_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``VIGIL_DASHBOARD_PORTAL_URL`` into ``~/.vigil/.env`` idempotently.
...
def _resolve_portal_base_url(override: Optional[str] = None) -> str:
    ...
    Precedence:
      1. ``override`` — explicit ``--portal-url`` flag or
         ``VIGIL_DASHBOARD_PORTAL_URL`` env (used for testing against a
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``vigil dashboard register`` idempotent. The portal
...
def _print_post_register_hint(
    *,
    ...
    print("    VIGIL_DASHBOARD_OAUTH_CLIENT_ID=" + str(_cid))
    if wrote_portal_url:
        print("    VIGIL_DASHBOARD_PORTAL_URL=" + str(portal_base_url))
    if public_url:
        print("    VIGIL_DASHBOARD_PUBLIC_URL=" + str(public_url))
    print()
    print(
...
def cmd_dashboard_register(args) -> None:
    ...
    # stamped in by the orchestrator (NAS sets VIGIL_DASHBOARD_OAUTH_CLIENT_ID
    ...
    # VIGIL_DASHBOARD_PORTAL_URL env var, else the stored login's portal.
    #
    ...
    # client_id locally (VIGIL_DASHBOARD_OAUTH_CLIENT_ID). Re-send it so the
    ...
    try:
        existing_client_id = get_env_value("VIGIL_DASHBOARD_OAUTH_CLIENT_ID")
    except Exception:
        existing_client_id = None
    ...
    try:
        save_env_value("VIGIL_DASHBOARD_OAUTH_CLIENT_ID", client_id)
    except Exception as exc:
        print(f"✗ Failed to write VIGIL_DASHBOARD_OAUTH_CLIENT_ID to .env: {exc}")
        print(f"  Set it manually:  VIGIL_DASHBOARD_OAUTH_CLIENT_ID={client_id}")
        sys.exit(1)

    # Persist the portal URL. Two cases:
    #   a) The user explicitly supplied a custom portal (--portal-url flag or
    #      VIGIL_DASHBOARD_PORTAL_URL env). That's an intentional choice we
    ...
    try:
        existing_portal = get_env_value("VIGIL_DASHBOARD_PORTAL_URL")
    except Exception:
        existing_portal = None
    ...
    if should_write_portal:
        try:
            save_env_value("VIGIL_DASHBOARD_PORTAL_URL", portal_base_url)
    ...
    # that same callback by taking VIGIL_DASHBOARD_PUBLIC_URL and appending
    ...
    # the supplied redirect URI and persist it as VIGIL_DASHBOARD_PUBLIC_URL so
    ...
    if public_url:
        ...
        try:
            existing_public_url = get_env_value("VIGIL_DASHBOARD_PUBLIC_URL")
        except Exception:
            existing_public_url = None
        if existing_public_url != public_url:
            try:
                save_env_value("VIGIL_DASHBOARD_PUBLIC_URL", public_url)
