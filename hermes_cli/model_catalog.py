...
The Vigil docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.vigil/cache/model_catalog.json``.
...
# Fallback fetch chain. The manifest is published from the Vigil GitHub repo;
# the raw URL is not bot-gated and is the same file the docs site serves.
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://github.com/juneauwang/vigil-agent-harness/raw/main/website/static/api/model-catalog.json",
)
DEFAULT_TTL_HOURS = 1
DEFAULT_FETCH_TIMEOUT = 8.0
SUPPORTED_SCHEMA_VERSION = 1

_VIGIL_USER_AGENT = f"vigil-cli/{_VIGIL_VERSION}"
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Vigil silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`vigil update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``vigil update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.vigil/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``vigil model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
