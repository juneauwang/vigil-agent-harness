"""Default configuration data for Vigil Agent.
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Vigil-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Vigil
        ...
        # identity slot (SOUL.md). Empty by default. The VIGIL_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Vigil adds a coding operating brief
        ...
        # ``VIGIL_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             VIGIL_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force VIGIL_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Vigil auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Vigil sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Vigil
        ...
        #  "/home/user/.vigil/cache/documents:/output"]
        ...
        # bundled Vigil image, which drops to the `vigil` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Vigil sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Vigil should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``vigil chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.vigil/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: vigil sweeps the checkpoint base at startup
        ...
        # ``vigil checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # AGENTS.md, CLAUDE.md, .hermes.md, or .cursorrules before Vigil applies
    ...
    # Single-query (``vigil -q/-z "..."``) variant of mcp_discovery_timeout.
    ...
    # single read_file page exceeds these limits, Vigil truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Vigil' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); vigil =
                                      # Vigil' compression threshold triggers
    ...
    "auxiliary": {
        ...
        # Invoked by ``vigil kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``vigil kanban decompose`` and the kanban
        ...
        # ``vigil profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `vigil model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `vigil` (and `vigil chat`) launches by default:
        ...
        # REPL and `--tui` (or VIGIL_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `vigil --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `vigil -c` muscle memory.  Default off so existing
        # users aren't surprised.  VIGIL_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `vigil --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to VIGIL_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/vigil`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `vigil pets`. Disabled until
    ...
    "dashboard": {
        ...
        # ``VIGIL_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``VIGIL_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``VIGIL_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``VIGIL_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``VIGIL_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/vigil``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Vigil appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.vigil/cache/piper-voices/
    ...
    # "Hey Vigil" hands-free wake word. Always-on, on-device hotword
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.vigil/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.vigil/cache/delegation/
    ...
    # response. If not, Vigil feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Vigil auto-pauses the goal and
    ...
    # always goes to ~/.vigil/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${VIGIL_SKILL_DIR} and ${VIGIL_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `vigil curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `vigil curator run --consolidate` overrides
        ...
        # normally restored on every `vigil update`, so pruning them only
        ...
        # skipped), snapshot ~/.vigil/skills/ into
        # ~/.vigil/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `vigil curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Vigil Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (VIGIL_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Vigil' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.vigil/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or VIGIL_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `vigil doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``hermes_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Vigil to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $VIGIL_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via VIGIL_CRON_MAX_PARALLEL env var.
        ...
        # VIGIL_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `vigil -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `vigil` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `vigil kanban decompose <id>` or
    ...
    #
    # Core Vigil tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.vigil/logs/.
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://raw.githubusercontent.com/juneauwang/vigil-agent-harness/main/website/static/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `vigil model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal VIGIL_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.vigil/sessions/sessions.json entirely.
        ...
        # the VIGIL_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # ``VIGIL_GATEWAY_MAX_STARTS`` / ``VIGIL_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.vigil/.env,
        ...
        # behavior: files must live under the Vigil cache, under
        ...
        # user. Bridged to VIGIL_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Vigil
        # cache (~/.vigil/cache/{documents,images,audio,video,screenshots})
        ...
        # to VIGIL_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # VIGIL_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to VIGIL_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.vigil/state.db.
    ...
    "sessions": {
        ...
        # the default of ``vigil sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.vigil/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `vigil sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `vigil update` prints a one-line notice with
        ...
        # ~/.vigil/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to VIGIL_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # VIGIL_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
    },
    ...
    "updates": {
        ...
        #     DBs) into <VIGIL_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``vigil backup``-style zip
        #     of VIGIL_HOME into <VIGIL_HOME>/backups/, restorable with
        #     ``vigil import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `vigil update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<VIGIL_HOME>/lsp/bin/`` on first use.
    ...
    # `vigil tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.vigil/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.vigil/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.vigil/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Vigil may use this encrypted
            ...
            # ~/.vigil/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.vigil/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Vigil disables it for our users unless they
        # explicitly opt in here. When false (default), Vigil sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Vigil also calls
    ...
    #
    # Configure with `vigil egress setup`.  Disabled by default — the rest of
    # Vigil works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.vigil/bin/ on
    ...
    # Vigil Desktop (Electron app) launch options. These only affect
    # `vigil desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the VIGIL_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Vigil mints short-lived tokens from. Falls back to "
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Vigil' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Vigil, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. VIGIL_BOT)",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Vigil API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Vigil API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Vigil API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # VIGIL_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to VIGIL_TOOL_PROGRESS_MODE for backward
    ...
    # VIGIL_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "VIGIL_PREFILL_MESSAGES_FILE": {
