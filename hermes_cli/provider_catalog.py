...
The provider list shown by ``vigil model`` (CLI/TUI) and the desktop Settings
...
the same universe ``vigil model`` renders (``CANONICAL_PROVIDERS``), joining:
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``vigil model`` shows.
"""
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `vigil model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``vigil model`` universe.
    ...
    # Vigil overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = VIGIL_OVERLAYS.get(slug)

        # auth_type: registry is authoritative; fall back to profile, then the
        # Vigil overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
