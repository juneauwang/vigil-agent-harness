...
configured for "cli" in `vigil tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (VIGIL_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `vigil chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"vigil -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "vigil -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"vigil -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "vigil -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "vigil -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to VIGIL_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("VIGIL_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "vigil -z: --provider requires --model (or VIGIL_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"vigil -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("vigil -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``vigil -z`` / oneshot mode.
...
def _run_agent(
    ...
    # Imports are local so they don't run when vigil is invoked for
    ...
    env_model = os.getenv("VIGIL_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("VIGIL_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    try:
        ...
        agent = AIAgent(
            ...
            #                VIGIL_INTERACTIVE which we never set
            #   - shell-hook approval → auto-approved via VIGIL_ACCEPT_HOOKS=1
            #                (set above); also falls back to deny on non-tty
            #   - dangerous-command approval → bypassed via VIGIL_YOLO_MODE=1
