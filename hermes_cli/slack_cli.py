"""``vigil slack ...`` CLI subcommands.

Today only ``vigil slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ vigil slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Vigil. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Vigil deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Vigil on Slack's older Assistant messaging
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Vigil in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Vigil in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Vigil agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hermes_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$VIGIL_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Vigil")
    ...
    """
    name = getattr(args, "name", None) or "Vigil"
    description = getattr(args, "description", None) or "Your Vigil agent on Slack"
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Vigil app\n"
