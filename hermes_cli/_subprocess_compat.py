...
Vigil is developed on Linux / macOS and tested natively on Windows too.
...
# Electron exits, so the gateway never gets respawned after a `vigil
...
def suppress_platform_ver_console() -> None:
    ...
    *visible* console: one flashing ``cmd`` window per process, triggered by
    ...
    during a dependency's import, not from Vigil' own code.
    """
    if not IS_WINDOWS:
        return
    try:
...
def noninteractive_git_env(
    ...
    Vigil shells out to git from many non-interactive contexts — MCP catalog
