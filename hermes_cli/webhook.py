...
def _setup_hint() -> str:
  ...
  Then start the gateway: vigil gateway run
"""
...
def webhook_command(args):
    ...
    if not sub:
        print("Usage: vigil webhook {subscribe|list|remove|test}")
        print("Run 'vigil webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (vigil gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: vigil webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from vigil webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (vigil gateway run)")
