...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``vigil doctor --ack <id>``; the ack is persisted to
...
1. ``vigil doctor`` (and ``vigil doctor --ack <id>``)
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``vigil doctor``.
