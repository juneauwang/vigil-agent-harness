...
class CLICommandsMixin:
    ...
    def _handle_diff_command(self, command: str):
        ...
        Syntax:
            ...
            /diff session          — everything Vigil changed (checkpoint baseline)
    ...
    def _print_session_diff(self, cwd: str, stat_only: bool):
        ...
        if result.get("empty") or (not stat and not diff):
            print("  No changes — Vigil hasn't edited any files here yet.")
            return
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Vigil config/state.
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `vigil journey`).
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        # same directory as a startup `vigil -c`/`--resume`. The startup resume
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to hermes_cli.curator so the CLI and the `vigil curator`
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``vigil bundles list`` but renders inside the running
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Vigil-managed isolated debug "
    ...
    def _handle_heartbeat_command(self, cmd: str) -> None:
        ...
        cross-process schedules use `vigil cron`.
        """
        ...
        _cprint(
            ...
            f"lives only while this Vigil process runs — use `vigil cron` for "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Vigil keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Vigil is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Vigil is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will redirect the current run while Vigil is busy; /stop still cancels it."
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        ...
        ``nous`` was removed: Vigil has no Nous-internal storage, so a nous
        upload would leak the debug bundle to a storage we don't own. The word
        is tolerated but never enables nous (always False); typing it prints a
        reminder and falls back to the default paste path.
        """
        ...
        if "nous" in words:
            print(
                "/debug nous 已移除（Vigil 不提供 Nous 内部存储上传）；本次按默认上传"
                " paste 服务处理，或使用 /debug local 本地渲染。"
            )
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Vigil Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``vigil update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Vigil Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Vigil Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Vigil Agent",
    ...
    def _handle_wake_command(self, command: str):
        """Handle /wake [on|off|status] — the 'Hey Vigil' hotword listener.
