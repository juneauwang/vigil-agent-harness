"""
Profile management for multiple isolated Vigil instances.

Each profile is a fully independent VIGIL_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.vigil/profiles/<name>/`` by default.

The "default" profile is ``~/.vigil`` itself — backward compatible,
zero migration needed.

Usage::

    vigil profile create coder          # fresh profile + bundled skills
    vigil profile create coder --clone  # also copy config, .env, SOUL.md, skills
    vigil profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    vigil -p coder chat                 # or via flag
    vigil profile use coder             # set as sticky default
    vigil profile delete coder          # remove profile + alias + service
"""
...
# default profile (``~/.vigil``).  Named profiles never contain these
...
# Marker file written by `vigil profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `vigil
...
# `vigil skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Vigil infrastructure directories that only the default profile
         (``~/.vigil``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.vigil) profile.
...
# Allow-list for ``export_profile("default")``: when VIGIL_HOME equals the
...
# into the export. The set below identifies the *known Vigil profile
# artifacts* at the root of VIGIL_HOME; everything else is excluded.
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the vigil root, NOT to the current VIGIL_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where VIGIL_HOME points outside
    ``~/.vigil``, profiles live under ``VIGIL_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) VIGIL_HOME path.

    In standard deployments this is ``~/.vigil``.
    In Docker/custom deployments where VIGIL_HOME is outside ``~/.vigil``
    (e.g. ``/opt/data``), returns VIGIL_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Vigil installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its VIGIL_HOME directory."""
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, vigil subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _VIGIL_SUBCOMMANDS:
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            vigil_exe = shutil.which("vigil") or shutil.which("argus") or shutil.which("hermes") or "vigil"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(vigil_exe)} -p {profile} "$@"\n', encoding="utf-8")
...
def remove_wrapper_script(name: str) -> bool:
    ...
    for wrapper_path in candidates:
        if wrapper_path.exists():
            try:
                # Verify it's our wrapper before removing (vigil -p or legacy hermes -p)
                content = wrapper_path.read_text(encoding="utf-8")
                if "vigil -p" in content or "argus -p" in content or "hermes -p" in content:
                    wrapper_path.unlink()
                    return True
            except Exception:
                pass
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Vigil. If we
    ...
    try:
        ...
        # not be migrated. The next `vigil doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``vigil -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``vigil profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``vigil -p X``)
...
def build_alias_map() -> dict[str, str]:
    ...
    if not wrapper_dir.is_dir():
        return result
    is_windows = sys.platform == "win32"
    # Wrappers may target any entry point: `vigil -p` (current), `argus -p` (pre-Vigil), or
    # `hermes -p` (legacy wrappers from before the Vigil rename).
    prefixes = ("vigil -p ", "argus -p ", "hermes -p ")
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``vigil profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``VIGIL_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Vigil config
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own VIGIL_HOME.
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``vigil update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.vigil)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.vigil/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `vigil -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Vigil profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `vigil update`'s
    ...
    # Cloned configs can be older than the running Vigil (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `vigil profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `vigil -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches VIGIL_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``vigil profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "VIGIL_HOME": str(profile_dir)},
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Vigil *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Vigil backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``VIGIL_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `vigil -p <canon> profile
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Vigil process: either an entrypoint marker in argv, or
            # a resolved executable named `vigil`.
            joined = " ".join(argv)
            ...
            # ...or by VIGIL_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("VIGIL_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.vigil).\n"
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``VIGIL_HOME`` and binds the port resolved by ``gateway/config.py``
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set VIGIL_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("VIGIL_HOME")
    try:
        os.environ["VIGIL_HOME"] = str(profile_dir)
        ...
        if old_home is not None:
            os.environ["VIGIL_HOME"] = old_home
        elif "VIGIL_HOME" in os.environ:
            del os.environ["VIGIL_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.vigil/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
...
def get_active_profile_name() -> str:
    """Infer the current profile name from VIGIL_HOME.

    Returns ``"default"`` if VIGIL_HOME is not set or points to ``~/.vigil``.
    Returns the profile name if VIGIL_HOME points into ``~/.vigil/profiles/<name>``.
    Returns ``"custom"`` if VIGIL_HOME is set to an unrecognized path.
    """
    from hermes_constants import get_hermes_home
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      VIGIL_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Vigil file the
      user may have lying alongside VIGIL_HOME (#58394).
...
def export_profile(name: str, output_path: str, extra_files: Optional[Dict[str, str]] = None) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.vigil itself — its parent is ~/ and its
        # directory name is ".vigil", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.vigil itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.vigil). "
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a VIGIL_HOME path string.

    Called early in the CLI entry point, before any vigil modules
    are imported, to set the VIGIL_HOME environment variable.
    """
