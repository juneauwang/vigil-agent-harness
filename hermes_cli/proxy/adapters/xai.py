...
# xAI's public API is OpenAI-compatible for the endpoints Vigil commonly
# uses. The Responses endpoint is included because Vigil' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Vigil-managed OAuth credentials."""

    auth_hint = "vigil auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`vigil auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`vigil auth reset xai-oauth` or re-authenticate with "
                    "`vigil auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `vigil auth add xai-oauth --type oauth`."
            )
