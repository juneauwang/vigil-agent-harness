...
prefix (e.g. ``mission-control.tilos.com/vigil/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /vigil`` so the backend can
reconstruct prefixed URLs (Location: headers, OAuth redirect_uri,
cookie Path attributes, SPA asset URLs).

This module is also the home of the ``VIGIL_DASHBOARD_PUBLIC_URL`` /
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    "I set VIGIL_DASHBOARD_PUBLIC_URL but the OAuth callback is still
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/vigil"`` (no trailing slash) or ``""``
...
# ---------------------------------------------------------------------------
# VIGIL_DASHBOARD_PUBLIC_URL / dashboard.public_url
# ---------------------------------------------------------------------------
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``VIGIL_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
    ...
    """
    env_raw = os.environ.get("VIGIL_DASHBOARD_PUBLIC_URL", "")
    env_clean = _normalise_public_url(env_raw)
    if env_clean:
        return env_clean
    _warn_if_malformed("VIGIL_DASHBOARD_PUBLIC_URL env var", env_raw)
