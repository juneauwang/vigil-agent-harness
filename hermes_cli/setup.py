"""
Interactive setup wizard for Vigil Agent.
...
# display_hermes_home imported lazily at call sites (stale-module safety during vigil update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("◉ Vigil Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Vigil using environment variables or config commands:")
    print_info("  vigil config set model.provider custom")
    print_info("  vigil config set model.base_url http://localhost:8080/v1")
    print_info("  vigil config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'vigil setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'vigil setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Vigil cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    vigil model            (pick any provider/model)")
        print_info("    vigil setup --portal   (Nous Portal OAuth, no API key)")
    ...
    # Video generation — opt-in via `vigil tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'vigil setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'vigil setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'vigil tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'vigil setup terminal'"))
    ...
    # Spotify (OAuth via vigil auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'vigil setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('vigil setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('vigil setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('vigil setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('vigil setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('vigil setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('vigil config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('vigil config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('vigil config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('vigil', Colors.GREEN)}              Start chatting")
    print(f"   {color('vigil gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('vigil doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `vigil tools` and `vigil setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``vigil model``)
    ...
    provider added to ``vigil model`` is automatically available here.
    ...
    # Delegate to the shared vigil model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: vigil model")
    ...
    # on demand via `vigil auth add`, `vigil setup` vision, and
    # `vigil setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``vigil model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Vigil auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via vigil setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'vigil setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Vigil runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `vigil setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `vigil setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `vigil egress setup` then `vigil egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `vigil egress setup`."
            )
        ...
        try:
            ...
            # which() misses the uv Vigil installed. Bootstrapping one is
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `vigil setup agent` later to customize.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Vigil to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  vigil config edit")
    print_info("   Open config in your editor:  vigil config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Vigil from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'vigil setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     vigil config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: vigil gateway install --system")
                print_info("  Or run in foreground:  vigil gateway")
        else:
            from hermes_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   vigil gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   vigil gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `vigil setup tools` and `vigil tools` use the same flow:
...
def setup_telemetry(config: dict):
    ...
    print_info("Packages stay under this Vigil profile and are not uploaded.")
...
# Config values may have different semantics between OpenClaw and Vigil.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Vigil to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Vigil at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Vigil at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Vigil at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Vigil at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Vigil equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Vigil config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Vigil.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Vigil's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Vigil can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: vigil claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: vigil claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Vigil configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Vigil config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Vigil (use vigil claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``vigil setup --portal`` and ``vigil portal``. This is the
    ...
    Vigil session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``vigil setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``vigil model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``vigil portal`` always offers a model picker,
    ...
    print(color("│     ◉ Vigil Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `vigil portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `vigil portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `vigil portal`: %s", exc)
        print()
    ...
    print_info("  Run `vigil portal info` to inspect routing.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      vigil setup           — full or quick (auto-detected)
      vigil setup model     — just model/provider
      vigil setup tts       — just text-to-speech
      vigil setup terminal  — just terminal backend
      vigil setup gateway   — just messaging platforms
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ◉ Vigil Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    if is_existing:
        ...
        print_success("You already have Vigil configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'vigil setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Vigil?",
            [
    ...
    print()
    print_info("You can edit these files directly or use 'vigil config edit'")
    ...
    # Tune later with `vigil setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``vigil setup <section>``
    or switch providers with ``vigil model``.
    """
    from hermes_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'vigil setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    vigil setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  vigil setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``vigil tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``vigil setup agent`` /
    ``vigil config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `vigil update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        vigil tools")
        print_info("  Seed skills:         vigil skills opt-in --sync")
        print_info("  Add MCP servers:     vigil mcp add")
        print_info("  Enable plugins:      vigil plugins")
        print_info("  Tune agent settings: vigil setup agent")
        print()
        _print_setup_summary(config, hermes_home)
        return
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`vigil update` runs from re-injecting them. Opt back in any")
            print_info("time with `vigil skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `vigil tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `vigil plugins list` / `vigil plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `vigil plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `vigil mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `vigil mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   vigil tools")
    print_info("  Seed skills:         vigil skills opt-in --sync")
    print_info("  Add MCP servers:     vigil mcp add")
    print_info("  Tune agent settings: vigil setup agent")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'vigil setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Vigil to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'vigil setup gateway'.")
