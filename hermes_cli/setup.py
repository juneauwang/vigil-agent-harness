...
def is_noninteractive() -> bool:
    ...
    ``VIGIL_NONINTERACTIVE=1`` (see ``hermes_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("VIGIL_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``VIGIL_NONINTERACTIVE=1`` or a closed/redirected
...
def _print_setup_summary(config: dict, hermes_home):
    """Print the setup completion summary (ops-focused tool surface)."""
    ...
    # Tool availability summary — deliberately ops-focused. Consumer tools
    # remain available via `vigil tools`, but setup does not enumerate them or
    # push the user to configure unrelated API keys here.
    print()
    print_header("Tool Availability Summary")

    tool_status = []

    # Vision — keep for operational screenshots/image triage.
    try:
    ...
    if disabled_tools:
        print_warning("Some tools are disabled. Run 'vigil setup' to configure them.")
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    # Tool Gateway prompt is already shown by the model flow above.
    save_config(config)
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        try:
            ...
            # Managed uv first: $VIGIL_HOME/bin is never on PATH, so a bare
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into VIGIL_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("VIGIL_MAX_ITERATIONS")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # VIGIL_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.vigil/logs/tool_calls.log (gateway only)")
...
def _profile_name_from_hermes_home(hermes_home) -> str | None:
    """Return the active profile name when VIGIL_HOME is a profile dir."""
...
def _setup_webhooks():
    ...
    print()
    print_info("   See the webhook documentation in the Vigil repo for route examples.")
    print()
    ...
    print()
    print_info("   Route configuration guide: edit webhook routes in config.yaml.")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    # 消费级工具（category=tool）不再在 setup 出现：运维用户直给默认就绪的
    # 工具面（terminal/todo/skills/topo/runbook），想配 web/browser/voice 等
    # 走 `vigil tools`（_DEFAULT_OFF_TOOLSETS 语义不变，见批次十二 D1）。
    missing_messaging = [
        v
