...
def _scan_dashboard_processes(
    *,
    ...
    ``vigil dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``vigil update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Vigil Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``vigil serve`` as
    ...
    environment variable ``VIGIL_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        ...
        # The headless backend (`vigil serve`) is the same long-lived server
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "vigil.*dashboard"`) keeps us consistent
...
def _kill_stale_dashboard_processes(
    ...
    *,
    ...
    """Kill running ``vigil dashboard`` / ``vigil serve`` processes.

    Called at the end of ``vigil update`` (default ``reason``) and also
    from ``vigil dashboard --stop`` (which overrides ``reason``).  The
    ...
    When ``restart_managed`` is true (the ``vigil update`` path), a detected
    ...
    # When the Vigil Desktop Electron app spawns this dashboard as a
    # backend child, it sets VIGIL_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("VIGIL_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    # caller asked for restarts (the `vigil update` path) — `--stop` must
    ...
    #    remote backend (vigil serve) under Restart=on-failure never comes
...
def _detect_concurrent_hermes_instances(
    ...
    The Vigil Desktop Electron app spawns ``hermes.EXE`` as a backend child,
    so during ``vigil update`` the user-invoked process and the desktop's
    ...
    chain are excluded so the running ``vigil update`` invocation never
    ...
    # file lock) is the parent. Without excluding it, every ``vigil update``
    ...
    #      hermes.exe sitting *under* a non-Vigil parent (e.g. a Vigil
