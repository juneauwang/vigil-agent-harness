...
``$VIGIL_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
(the stage2 hook) has chowned the volume and seeded $VIGIL_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$VIGIL_HOME``,
    not under ``profiles/``). The dispatcher in ``hermes_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``vigil gateway start`` (no ``-p``) targets. Without it,
    bare ``vigil gateway start`` inside the container would land on
    ...
    ``$VIGIL_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        hermes_home: The container's VIGIL_HOME (typically /opt/data).
    ...
    # ``vigil gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `vigil profile create` (config.yaml
            # is not — that comes later via `vigil setup`). Use it as the
            ...
            # via ``vigil -p default-named gateway start`` if they
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    if os.environ.get("VIGIL_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off the container argv, leaving the vigil args.

    Two container-command argv shapes are handled:

    * **s6-overlay v2 / tini:** PID 1 argv is
      ``/init /opt/vigil/docker/main-wrapper.sh <subcommand> [args...]``.
    * **s6-overlay v3:** PID 1 is ``s6-svscan`` and the command lives on the
      rc.init-launched process as ``/bin/sh -e
      /run/s6/basedir/scripts/rc.init top /opt/vigil/docker/main-wrapper.sh
    ...
    ``vigil`` invocations carry no wrapper, so fall back to peeling a bare
    ``init`` prefix. The wrapper re-execs ``vigil <subcommand>``, so an
    explicit leading ``vigil`` is peeled too. Shared by the legacy-gateway
    ...
    # The wrapper re-execs `vigil <subcommand>`; peel an explicit hermes.
    if args and Path(args[0]).name in {"hermes", "vigil"}:
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``vigil dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted VIGIL_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `vigil -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit vigil
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the vigil user) won't hit EACCES. See
...
def _write_reconcile_log(
    ...
    """Append one line per profile to $VIGIL_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted VIGIL_HOME, both race to flock() the same s6-log lock
