"""
Multi-provider authentication system for Vigil Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.vigil/auth.json with cross-process file locking.
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from hermes_cli import __version__ as _VIGIL_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _VIGIL_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"hermes-cli/{_VIGIL_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://github.com/juneauwang/vigil-agent-harness"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

OAUTH_OVER_SSH_DOCS_URL = "https://github.com/juneauwang/vigil-agent-harness"
...
def normalize_actual_base_url(base_url: str) -> str:
    ...
    surface for Vigil' Responses transport.
    """
...
def get_anthropic_key() -> str:
    ...
    ``~/.vigil/.env`` so a deliberate key rotation isn't shadowed by a stale
...
def _resolve_api_key_provider_secret(
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.vigil/.env over os.environ so a deliberate key rotation
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``vigil auth``.
    """
    return (
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("VIGIL_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.vigil/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_hermes_home() / "auth.json"
    # Seat belt: if pytest is running and VIGIL_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch VIGIL_HOME, tests invoked without the
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved in real_home_auths:
            raise RuntimeError(
                ...
                "Set VIGIL_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom VIGIL_HOME that is not a profile).
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.vigil/auth.json`` even when VIGIL_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_hermes_root()`` for a profile-shaped VIGIL_HOME can
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``vigil auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``vigil auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``vigil auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    try:
        ...
        # ``provider: anthropic`` as a MoA advisor/aggregator has opted Vigil
    ...
    # not by the user explicitly configuring anthropic in Vigil.
    ...
    # the user initiated inside Vigil (manual add / device-code / PKCE), plus
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `vigil logout`.
...
def has_any_provider_configuration(
    *,
    ...
    # OpenRouter credential pool (manual ``vigil auth add openrouter``).
    try:
        from agent.credential_pool import load_pool
...
def resolve_provider(
    ...
    *,
    ...
    # Auto-detect an OpenRouter credential added via `vigil auth add openrouter`
    ...
    # `vigil auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, "")):
                ...
                # OAuth but has a stale key in ~/.vigil/.env isn't silently
    ...
    raise AuthError(
        ...
        "OPENAI_API_KEY, etc.) in ~/.vigil/.env.",
...
def _nous_portal_env_override() -> Optional[str]:
    ...
    Mirrors ``_nous_inference_env_override()``: ``VIGIL_PORTAL_BASE_URL`` /
    ``NOUS_PORTAL_BASE_URL`` are the documented dev/staging escape hatch for
    pointing Vigil at a non-production Nous Portal (e.g. a hosted agent
    provisioned on nous-account-service's `staging` environment, which stamps
    ``VIGIL_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` into
    ...
    return _optional_base_url(
        os.getenv("VIGIL_PORTAL_BASE_URL") or os.getenv("NOUS_PORTAL_BASE_URL")
    )
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("VIGIL_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `vigil model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.vigil/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("VIGIL_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set VIGIL_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("VIGIL_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("VIGIL_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("VIGIL_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.vigil/.env, and return it.
    ...
    # Persist so subsequent `vigil auth spotify` runs skip the wizard.
    save_env_value("VIGIL_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("VIGIL_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved VIGIL_SPOTIFY_CLIENT_ID to ~/.vigil/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "VIGIL_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Vigil:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Vigil is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.vigil/auth.json (not ~/.codex/)
#
# Vigil maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Vigil auth store (~/.vigil/auth.json).
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``vigil setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``vigil auth add openai-codex``
      ...
      ``vigil auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Vigil auth store (~/.vigil/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``vigil auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Vigil auth, if available."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Vigil auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run vigil auth" prompt (see issue #32790).
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Vigil auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Vigil keeps its OWN
        ...
        # the Codex CLI (or another Vigil process) rotates the shared token,
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Vigil's own Codex token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("VIGIL_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("VIGIL_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Vigil processes
...
def _probe_codex_quota_restored(
    ...
    *,
    ...
    Vigil persists a Codex 429's ``reset_at`` locally and freezes the
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.vigil/auth.json
# =============================================================================
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.vigil/auth.json even when VIGIL_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``vigil auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.vigil/auth.json``. A single MITM during initial
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``VIGIL_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    # written by an older Vigil (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `vigil model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``vigil model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("VIGIL_XAI_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("VIGIL_XAI_BASE_URL", "").strip().rstrip("/")
...
# so a new `vigil --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${VIGIL_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<hermes-root>/shared/nous_auth.json`` where ``<hermes-root>`` is what
# ``get_default_hermes_root()`` returns — ``~/.vigil`` on Linux/macOS,
# ``%LOCALAPPDATA%\vigil`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's VIGIL_HOME so named profiles (which
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``VIGIL_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<hermes-root>/shared/``, where ``<hermes-root>`` is what
    :func:`hermes_constants.get_default_hermes_root` returns — so
    Linux/macOS classic installs land at ``~/.vigil/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\vigil\\shared\\``, and
    Docker / custom ``VIGIL_HOME`` deployments at
    ``<VIGIL_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("VIGIL_SHARED_AUTH_DIR", "").strip()
    if override:
...
def _nous_shared_store_path() -> Path:
    ...
    # real user's Vigil root, refuse rather than silently corrupt cross-profile
    # state. Tests must set VIGIL_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set VIGIL_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No VIGIL_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _quarantine_nous_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Vigil
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with Vigil's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Vigil's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another Vigil install sharing "
            "~/.vigil/auth.json) called POST /api/oauth/token with Vigil's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only Vigil may call the "
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Vigil models — they're not reliable for agentic tool-calling
...
def resolve_nous_access_token(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError(
                "Vigil is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )

        # VIGIL_PORTAL_BASE_URL / NOUS_PORTAL_BASE_URL is the trusted
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``vigil auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``vigil auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `vigil auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _provider_state_transaction("nous") as (
        ...
        if not state:
            raise AuthError("Vigil is not logged into Nous Portal.",
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            portal_url = (
                _optional_base_url(state.get("portal_base_url"))
                or os.getenv("VIGIL_PORTAL_BASE_URL")
...
# ~350ms even on the failure path, and read-only UI surfaces (`vigil tools`, status panels,
# subscription-feature checks) call it many times per render — `vigil tools` → "All Platforms"
...
# `vigil auth login/logout/add/remove` invalidate naturally on the next call.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `vigil auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `vigil auth` and
    # `vigil model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("VIGIL_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("VIGIL_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``vigil doctor`` runs the live
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Vigil' "
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set VIGIL_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `vigil logout` historically keyed off auth.json.active_provider only.
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.vigil/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Vigil-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Vigil auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Vigil will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("VIGIL_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Vigil will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Vigil gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Vigil creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Vigil auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Vigil auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Vigil creates its own local OAuth session)")
    print()
    ...
    # xAI credential re-enabled. ``vigil auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``vigil auth list`` would show
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Vigil auth store (~/.vigil/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Vigil login via MiniMax ({region}) OAuth...")
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``vigil status`` that just want to know
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Vigil login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
...
def step_up_nous_billing_scope(
    *,
    ...
    targets the same deployment (incl. a preview via ``VIGIL_PORTAL_BASE_URL`` set
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Vigil builds still see
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Vigil will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `vigil model` or configure an API key to use Vigil.")
        else:
