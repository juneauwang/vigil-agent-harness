...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Vigil Agent. Use 'vigil debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    vigil debug share              Upload debug report (asks for confirmation)
    vigil debug share --yes        Skip confirmation (for scripts/CI)
    vigil debug share --lines 500  Include more log lines
    vigil debug share --expire 30  Keep paste for 30 days
    vigil debug share --local      Print report locally (no upload)
    vigil debug share --no-redact  Disable upload-time secret redaction
    vigil debug share --nous       Upload to Nous-internal storage (private)
    vigil debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'vigil debug share'",
    )
