...
manual BotFather token copy-paste. Vigil talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.github.com/juneauwang/vigil-agent-harness"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Vigil Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Vigil Telegram onboarding service.")
