...
def show_status(args):
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: vigil portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: vigil model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: vigil auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: vigil auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: vigil model)"
    ...
    try:
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   vigil gateway")
    ...
    print(color("  Run 'vigil doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'vigil setup' to configure", Colors.DIM))
    print()
