#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the Vigil Skills Hub.

Powers both:
  - `vigil skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    if len(exact) > 1:
        ...
        # so users can copy it for `vigil skills install`.
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.vigil/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.vigil/skills/<name>/)[/]"
        )
    try:
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    # into `vigil skills install` fail.
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # The identifier is what you pass to `vigil skills install`. Browse used
...
def do_list(source_filter: str = "all",
    ...
    config — ``vigil -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``VIGIL_HOME`` at process
...
def do_list_modified(console: Optional[Console] = None,
    ...
    """List bundled skills the user has edited (which `vigil update` keeps)."""
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Vigil Skills Hub.\n\n"
                        f"This skill was scanned by the Vigil Skills Guard before submission.",
...
def skills_command(args) -> None:
    """Router for `vigil skills <subcommand>` — called from hermes_cli/main.py."""
