...
This document describes Vigil's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/juneauwang/vigil-agent-harness)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Vigil does not operate a bug
bounty program.**
...
- Environment details (`vigil version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Vigil access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Vigil's documentation
...
Vigil supports two OS-level isolation postures. They address
...
Vigil supports this in two ways:

- **Vigil's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Vigil's in-process heuristics
...
Vigil filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Vigil
itself. Bugs in Vigil's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Vigil does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Vigil's own documentation, or reasonable
  operator expectations would predict — including cases where
  Vigil has documented a stance about how its output should
...
- Vigil includes supply-chain guards for MCP server
