...
Registry and ABC for every inference provider Vigil knows about.
...
`$VIGIL_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
under `$VIGIL_HOME/plugins/model-providers/` for a private plugin).
