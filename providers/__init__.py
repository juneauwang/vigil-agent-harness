...
2. User plugins: ``$VIGIL_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$VIGIL_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$VIGIL_HOME/plugins/model-providers/`` if it exists."""
    try:
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple VIGIL_HOME profiles don't alias each other.
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$VIGIL_HOME/plugins/model-providers/<name>/``
    ...
    # 2. User plugins — under $VIGIL_HOME/plugins/model-providers/<name>/.
