...
How to build Vigil Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Vigil surface can also
...
- A **runtime home change** (switching the underlying `VIGIL_HOME` profile) is
...
its shape. "Plugin" means several unrelated things across Vigil — do not assume
