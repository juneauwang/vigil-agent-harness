...
declare global {
  interface Window {
    hermesDesktop: {
      ...
      // Vigil Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new VIGIL_HOME (reloads the window). Pass null to
      ...
      // Local Desktop runtime-plugin root (<VIGIL_HOME>/desktop-plugins),
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Vigil Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Vigil Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected Vigil Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the vigil gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Vigil Cloud agent — the trimmed DTO from NAS GET /api/agents.
