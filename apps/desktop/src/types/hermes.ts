...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Vigil doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // this provider identity — the SAME one `vigil model` uses — instead of
...
export interface ModelOptionProvider {
   ...
   *  the `vigil model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// non-desktop) Vigil simply carries none of it. See store/profile-share.ts.
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Vigil, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `vigil tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Vigil); elsewhere, driver health from `cua-driver doctor`. `null`
