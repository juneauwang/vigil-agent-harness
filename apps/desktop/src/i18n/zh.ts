...
export const zh: Translations = {
  ...
  settings: {
    ...
    gateway: {
      ...
      envOverrideDesc: '取消设置 VIGIL_DESKTOP_REMOTE_URL 和 VIGIL_DESKTOP_REMOTE_TOKEN 后才会使用下面保存的设置。',
      modeTitle: '连接模式',
      localTitle: '本地网关',
      ...
      sshHermesPathDesc: '远程 vigil 可执行文件的完整路径。留空 = 自动检测。',
      ...
      sshErrNotInstalled:
        '远程主机上未安装 Hermes。请在远程安装（curl -fsSL https://github.com/juneauwang/vigil-agent-harness | sh）或设置 Vigil 路径。',
  ...
  skills: {
    ...
    skillArchivedMessage: '可通过 vigil curator restore 恢复。',
    hub: {
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      weixin:
        '运行 `vigil gateway setup`，选择 Weixin，然后使用个人微信账号扫描并确认二维码。Hermes 会通过腾讯 iLink Bot API 连接并保存凭据。',
  ...
  assistant: {
    ...
    approval: {
      ...
      alwaysDescription: pattern =>
        `这会将“${pattern}”模式加入永久允许列表 (~/.vigil/config.yaml)。Hermes 对类似命令将不再询问，包括当前会话和未来会话。`,
      alwaysAllow: '始终允许'
    },
    clarify: {
      notReady: '澄清请求尚未就绪',
