...
<p align="center">
  <a href="https://github.com/juneauwang/vigil-agent-harness"><img src="https://img.shields.io/badge/Download-macOS%20%C2%B7%20Windows%20%C2%B7%20Linux-FFD700?style=for-the-badge" alt="Download"></a>
...
**The native desktop app for [Vigil](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Vigil surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Vigil and hear it back.</td></tr>
...
### Install with Vigil (recommended)
...
It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Vigil gateway or install Vigil locally. Local onboarding then walks you through choosing a provider and model.

### Prebuilt installers

Prebuilt installers are built and distributed via [the Vigil Desktop website.](https://github.com/juneauwang/vigil-agent-harness).
...
```bash
# throwaway VIGIL_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Vigil runtime into `VIGIL_HOME`
(`~/.vigil`, or `%LOCALAPPDATA%\hermes` on Windows), using the same layout as a
CLI install.
...
- **Vigil** runs as a headless `vigil serve` process and exposes the
...
1. `VIGIL_DESKTOP_VIGIL_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `VIGIL_DESKTOP_HERMES`, or `vigil` on `PATH`
5. a system Python that can import the Vigil runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Vigil
...
screen offers **Connect to existing Vigil** before starting the local installer.
...
terminal commands, and file operations run against the remote Vigil host, not
...
Boot logs land in `VIGIL_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
> The default Vigil home on Windows is `%LOCALAPPDATA%\hermes`. Set the `VIGIL_HOME` env var if you've relocated it.
...
- 📖 [Documentation](https://github.com/juneauwang/vigil-agent-harness)
