 ...
 * packaged binary) with a real `vigil serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → vigil serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → vigil serve is spawned → WS connects → config
