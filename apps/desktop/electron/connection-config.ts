 ...
 *     `X-Vigil-Session-Token` header; WS uses `?token=`.
...
//     reuse-detected — Portal NAS #293 / vigil #37247). When the AT cookie
...
// The Nous portal (NAS) does NOT use Vigil gateway session cookies — it is a
...
// Vigil Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  // name used by the remote Vigil installation. Preserve an explicit mapping
  // when it is a valid Vigil profile identifier; otherwise fall back to the
 ...
 *     `--profile`, so its `VIGIL_HOME` scopes it.
 *
...
/**
 * True if any cookie in `cookies` is a vigil session ACCESS-token cookie
 ...
 * the Vigil gateway session cookies, so cloud sign-in / discovery liveness
