 ...
 * Background
 * ----------
 * After `vigil update` + `vigil desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything VIGIL_DESKTOP_* is preserved
// plus VIGIL_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['VIGIL_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['VIGIL_DESKTOP_']
