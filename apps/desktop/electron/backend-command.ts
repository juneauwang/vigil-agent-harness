// Backend subcommand routing for the desktop-managed Vigil process.
//
// The desktop app launches its own headless backend via `vigil serve` — it
...
// app hasn't updated yet, or an older `vigil` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Vigil profile to pin via `--profile`.
 */
