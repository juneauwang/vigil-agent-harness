...
// `vigil serve` announces VIGIL_BACKEND_READY; the legacy `vigil dashboard`
// backend announces VIGIL_DASHBOARD_READY. Accept either so the desktop spawn
// works against both the headless backend and old/dashboard runtimes.
const _READY_RE = /^VIGIL_(?:BACKEND|DASHBOARD)_READY port=(\d+)/m
...
/**
 * Watch a child process's stdout for the `VIGIL_(BACKEND|DASHBOARD)_READY
