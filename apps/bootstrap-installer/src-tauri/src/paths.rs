...
//!   macOS:   ~/.vigil
//!   Linux:   ~/.vigil  (override via $VIGIL_HOME)
//!
//! NOTE (macOS): Python's get_hermes_home(), scripts/install.sh, and the
//! Electron desktop's resolveHermesHome() ALL use ~/.vigil on macOS — there
...
/// Returns the canonical Vigil home directory, respecting $VIGIL_HOME if set.
pub fn hermes_home() -> PathBuf {
    ...
    // macOS + Linux + fallback: ~/.vigil (matches Python get_hermes_home(),
...
/// VIGIL_HOME so it survives repo checkout deletion (unlike anything under
/// hermes-agent/).
///
...
///
/// Lives directly under VIGIL_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves VIGIL_HOME identically and pins it into
...
pub fn copy_self_to_hermes_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to VIGIL_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_VIGIL_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".hermes-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under VIGIL_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("VIGIL_BOOTSTRAP_LOG")
