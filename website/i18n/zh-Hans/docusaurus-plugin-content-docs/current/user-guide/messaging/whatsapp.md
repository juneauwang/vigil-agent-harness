...
sidebar_position: 5
title: "WhatsApp"
description: "通过内置 Baileys 桥接将 Vigil 设置为 WhatsApp 机器人"
---

# WhatsApp 配置

Vigil 通过基于 **Baileys** 的内置桥接连接到 WhatsApp。其工作原理是模拟 WhatsApp Web 会话——**而非**通过官方 WhatsApp Business API。无需 Meta 开发者账号或 Business 认证。
...
发生这种情况时，Vigil 会更新桥接依赖。如果机器人在 WhatsApp 更新后停止工作，
请拉取最新版 Vigil 并重新配对。
:::
...
Baileys 桥接将会话保存在 `~/.vigil/platforms/whatsapp/session` 目录下。这意味着：
...
- Agent 响应默认以"⚕ **Vigil**"为前缀。可在 `config.yaml` 中自定义或禁用：
...
- `~/.vigil/platforms/whatsapp/session` 目录包含完整会话凭证——请像保护密码一样保护它
- 设置文件权限：`chmod 700 ~/.vigil/platforms/whatsapp/session`
