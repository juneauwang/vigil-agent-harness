...
消息 gateway 是一个长期运行的进程，通过统一架构将 Vigil 连接到 20 余个外部消息平台。
...
| 来源 | 提供内容 |
|--------|-----------------|
| `~/.vigil/.env` | API 密钥、bot token、平台凭据 |
| `~/.vigil/config.yaml` | 模型设置、工具配置、显示选项 |
| 环境变量 | 覆盖上述任意配置 |
...
Hook 从 `gateway/builtin_hooks/`（扩展点 — 当前发行版中为空；`_register_builtin_hooks()` 是一个空操作存根）和 `~/.vigil/hooks/`（用户安装）中发现。每个 hook 是一个包含 `HOOK.yaml` 清单和 `handler.py` 的目录。
