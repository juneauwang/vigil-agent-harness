...
完成本指南需要**租户管理员权限**（或由管理员代为授予同意）。请记录收集到的值——最终需要填入 `~/.vigil/.env`。
...
4. 填写以下内容：
   - **Name：**`Vigil Teams Meeting Pipeline`（或任何你能识别的名称）。
...
```powershell
# Create a policy scoped to the Vigil app
New-CsApplicationAccessPolicy `
  -Identity "Hermes-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Vigil meeting pipeline to allow-listed users"
...
Vigil 内置了 Graph 身份验证冒烟测试。在 Vigil 安装目录下执行：
