...
sidebar_position: 3
title: "更新与卸载"
description: "如何将 Vigil 更新至最新版本或将其卸载"
---
...
:::tip
`vigil update` 会自动检测新的配置选项并提示你添加。如果跳过了该提示，可手动运行 `vigil config check` 查看缺失的选项，再运行 `vigil config migrate` 以交互方式添加。
:::

### 更新过程

运行 `vigil update` 时，将依次执行以下步骤：

1. **更新前快照** — 默认保存一份轻量级状态快照（涵盖配对数据、cron 任务、`config.yaml`、`.env`、`auth.json` 及其他运行时修改的状态文件；单个超过 1 GiB 的文件会被跳过，因此大型会话数据库不会拖慢更新）。由 `updates.pre_update_backup` 控制（默认 `quick`，`full` 为整个 `VIGIL_HOME` 的 zip 备份，`off` 为禁用）。可通过 [快照与回滚](../user-guide/checkpoints-and-rollback.md) 中描述的快照恢复流程进行恢复。
...
### 仅预览：`vigil update --check`

想在拉取前确认是否有更新？运行 `vigil update --check` — 它会获取并与 `origin/main` 比较提交。不修改任何文件，不重启 gateway。适合在以"是否有更新"为条件的脚本和 cron 任务中使用。
...
`updates.pre_update_backup` 是单一开关，有三种模式：`quick`（默认 — 上述轻量级状态快照）、`full`（快速快照加上完整的 `VIGIL_HOME` zip 备份；在大型 home 目录上可能增加数分钟）、`off`（完全不做更新前备份 — `--no-backup` 对单次运行有相同效果）。旧版布尔值仍然有效：`true` 等同于 `full`，`false` 等同于 `off`。

### Windows：另一个 `hermes.exe` 正在运行

在 Windows 上，如果 `vigil update` 检测到另一个 `hermes.exe` 进程持有 venv 入口点可执行文件的句柄，它将拒绝运行 — 最常见的情况是 Vigil Desktop 应用启动的后端进程、另一个终端中打开的 `vigil` REPL，或正在运行的 gateway：
...
`vigil update` 处理主要的更新流程，但快速验证可确认一切正常落地：

1. `git status --short` — 若工作树出现意外的脏状态，请在继续前检查
2. `vigil doctor` — 检查配置、依赖项和服务健康状态
3. `vigil --version` — 确认版本已按预期更新
4. 如果使用 gateway：`vigil gateway status`
5. 如果 `doctor` 报告 npm audit 问题：在标记的目录中运行 `npm audit fix`

:::warning 更新后工作树出现脏状态
如果 `vigil update` 后 `git status --short` 显示意外变更，请在继续前停下来检查。这通常意味着本地修改被重新应用到了更新后的代码之上，或依赖步骤刷新了锁文件。
:::

### 终端在更新中途断开连接

`vigil update` 针对意外终端断开进行了保护：
...
你不再需要将 `vigil update` 包裹在 `screen` 或 `tmux` 中来应对终端断开。
...
与 [GitHub releases 页面](https://github.com/juneauwang/vigil-agent-harness) 上的最新版本进行比较。
...
:::warning
如果新增了配置选项，回滚可能导致配置不兼容。回滚后运行 `vigil config check`，如果遇到错误，请从 `config.yaml` 中删除无法识别的选项。
:::
