...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Vigil  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Vigil instantly when PRs are opened or updated.
:::
...
- **Vigil installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  vigil gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.vigil/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Vigil can access GitHub. Start a chat:
...
```
Run: gh pr list --repo juneauwang/vigil-agent-harness --state open --limit 3
```
...
Still in the chat, ask Vigil to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo juneauwang/vigil-agent-harness
```
...
A skill gives Vigil consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.vigil/skills/code-review
```

Create `~/.vigil/skills/code-review/SKILL.md`:
...
Verify it loaded — start `vigil` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Vigil your team's standards:
...
2. Teach Vigil your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
vigil gateway status    # Is the gateway running?
vigil cron list         # Is the job enabled?
```
