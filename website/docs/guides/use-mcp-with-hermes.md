---
sidebar_position: 6
title: "Use MCP with Vigil"
description: "A practical guide to connecting MCP servers to Vigil, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Vigil

This guide shows how to actually use MCP with Vigil in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Vigil tool
- you want Vigil to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Vigil to internal APIs, databases, or company systems without modifying Vigil core

Do not use MCP when:
- a built-in Vigil tool already solves the job well
...
- Vigil remains the agent
- MCP servers contribute tools
- Vigil discovers those tools at startup or reload time
...
If you installed Vigil with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).

If you installed without extras and need to add MCP separately:

```bash
cd ~/.vigil/hermes-agent
...
- Vigil banner/status should show MCP integration when configured
- ask Vigil what tools it has available
...
## WSL2: bridge Vigil in WSL to Windows Chrome

This is the practical setup when:

- Vigil runs inside WSL2
...
In this setup, Vigil does **not** connect to Chrome directly. Instead:

- Vigil runs in WSL
- Vigil starts a local stdio MCP server
...
```text
Vigil (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Vigil stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Vigil core browser transport
...
```bash
vigil mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
vigil mcp test chrome-devtools-win
```

Then start a fresh Vigil session or run:
...
Once loaded, Vigil can use the MCP-prefixed browser tools directly. For example:
...
If Vigil runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Vigil from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Vigil from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Vigil:
...
So Vigil will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Vigil to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Vigil to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Vigil remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
vigil mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
vigil mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `vigil mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Vigil to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Vigil to merge, publish, deploy, or spawn runtimes.
...
Now Vigil can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Vigil core.
...
Because Vigil now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
