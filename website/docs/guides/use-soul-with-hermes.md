---
sidebar_position: 7
title: "Use SOUL.md with Vigil"
description: "How to use SOUL.md to shape Vigil's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Vigil

`SOUL.md` is the **primary identity** for your Vigil instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Vigil to feel like the same assistant every time you talk to it — or if you want to replace the Vigil persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Vigil should be
- what Vigil should avoid stylistically
- how Vigil should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Vigil is and how Vigil speaks
...
Vigil now uses only the global SOUL file for the current instance:

```text
~/.vigil/SOUL.md
```

If you run Vigil with a custom home directory, it becomes:
...
Vigil automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Vigil does not overwrite it
- if the file exists but is empty, Vigil adds nothing from it to the prompt

## How Vigil uses it

When Vigil starts a session, it reads `SOUL.md` from `VIGIL_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Vigil feels.
...
Vigil already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Vigil should sound.

# Avoid
What Vigil should not do.

# Defaults
How Vigil should behave when ambiguity appears.
```
...
Then restart Vigil or start a new session.
...
### I edited SOUL.md but Vigil still sounds the same

Check:
- you edited `~/.vigil/SOUL.md` or `$VIGIL_HOME/SOUL.md`
...
### Vigil is ignoring parts of my SOUL.md
