...
Copy-paste blueprints for common automation patterns. Each blueprint uses Vigil's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`vigil webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `vigil webhook subscribe`) |
...
vigil cron create "0 2 * * *" \
  "You are a project manager triaging the juneauwang/vigil-agent-harness GitHub repo.

1. Run: gh issue list --repo juneauwang/vigil-agent-harness --state open --json number,title,labels,author,createdAt --limit 30
...
vigil cron create "0 9 * * 1" \
  "Scan the juneauwang/vigil-agent-harness repo for documentation drift.

1. Run: gh pr list --repo juneauwang/vigil-agent-harness --state merged --json number,title,files,mergedAt --limit 30
...
1. cd ~/.vigil/hermes-agent && source .venv/bin/activate
...
```python title="~/.vigil/scripts/check-uptime.py"
...
```bash
vigil cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Vigil development." \
