...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `vigil send`."
---

# Pipe Script Output to Messaging Platforms

`vigil send` is a small, scriptable CLI that pushes a message to any
messaging platform Vigil is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | vigil send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `vigil
...
vigil send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
vigil send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
vigil send --list

# Filter by platform
vigil send --list telegram
```
...
Any platform Vigil ships adapters for works as a target:
...
`vigil send` resolves the message body in this order:

1. **Positional argument** — `vigil send --to telegram "hi"`
2. **`--file PATH`** — `vigil send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | vigil send --to telegram`

When stdin is a TTY (no pipe), Vigil does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  vigil send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `vigil send` reuses your Vigil config, the same script works on
any host where Vigil is installed — no need to export bot tokens into
...
`vigil send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  vigil send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | vigil send \
...
```bash
./train.py --epochs 200 && \
  vigil send --to telegram "training done" || \
  vigil send --to telegram "training failed (exit $?)"
```
...
vigil send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(vigil send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `vigil send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `vigil send` calls
the platform's REST endpoint directly using credentials from
`~/.vigil/.env` and `~/.vigil/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `vigil gateway start` and retry.
...
# Machine-readable
vigil send --list --json
```

The listing is built from `~/.vigil/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`vigil gateway
...
| Approach | Multi-platform | Reuses Vigil creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `vigil send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`vigil send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `vigil send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `vigil send` shares with cron delivery.
