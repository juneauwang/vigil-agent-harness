...
This tutorial walks you through setting up a Telegram bot powered by Vigil that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Vigil installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.vigil/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Vigil itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Vigil Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Vigil. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Vigil Gateway...
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `vigil gateway install` to update the plist.
:::
...
```bash
vigil gateway stop && vigil gateway start
```
...
vigil pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.vigil/.env`:
...
Control how much detail the bot shows when using tools. In `~/.vigil/config.yaml`:
...
Customize how the bot communicates by editing `~/.vigil/SOUL.md`:

For a full guide, see [Use SOUL.md with Vigil](/guides/use-soul-with-vigil).
...
```bash
# From the CLI
vigil cron list          # View all scheduled jobs
vigil cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u hermes-gateway` (Linux) or `~/.vigil/logs/gateway.log` (macOS) |
| Cron job output | `~/.vigil/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.vigil/cron/jobs.json` |
| Pairing data | `~/.vigil/pairing/` |
| Session history | `~/.vigil/sessions/` |
