...
If you're expecting jobs to fire automatically, you need a running gateway (`vigil gateway` for foreground, or `vigil gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `vigil cron tick`.
...
```bash
date
vigil cron list   # Compare next_run times with local time
```
...
If delivery fails, the job still runs — it just won't send anywhere. Check `vigil cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `vigil skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `vigil skills list`.
...
3. The job's `last_run` metadata via `vigil cron list`
...
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `vigil skills install <skill-name>`.
...
```bash
vigil cron list                    # Show all jobs, states, next_run times
vigil cron run <job_id>            # Schedule for next tick (for testing)
vigil cron edit <job_id>           # Fix configuration issues
vigil logs                         # View recent Vigil logs
vigil skills list                  # Verify installed skills
```
...
1. Run the job with `vigil cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.vigil/logs/agent.log` for scheduler messages and `~/.vigil/logs/errors.log` for warnings
3. Open an issue at [github.com/juneauwang/vigil-agent-harness](https://github.com/juneauwang/vigil-agent-harness) with:
