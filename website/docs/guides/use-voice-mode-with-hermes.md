---
sidebar_position: 8
title: "Use Voice Mode with Vigil"
description: "A practical guide to setting up and using Vigil voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Vigil
...
- you want Vigil sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Vigil.
...
## Step 1: make sure normal Vigil works first
...
Vigil supports both local and cloud speech stacks.
...
### If you use `vigil setup`

If you choose NeuTTS in the setup wizard, Vigil checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Vigil transcribes and responds
...
- asking Vigil to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Vigil loop.

## Tuning CLI behavior

### Silence threshold

If Vigil starts/stops too aggressively, tune:
...
Vigil stays a normal chat bot, but can speak replies.
...
- you want Vigil to function like a portable research or ops assistant
...
Vigil joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Vigil detects speech boundaries
- transcripts are posted in the associated text channel
- Vigil responds in text and audio
...
1. get text Vigil working
2. run `vigil setup tts` to enable voice support
