...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Vigil new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Vigil how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Vigil installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Vigil to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Vigil but aren't active by default. Install them explicitly:
...
What happens:
1. The skill directory is copied to `~/.vigil/skills/`
...
For how to ship skills in your own plugin, see [Build a Vigil Plugin → Bundle skills](/developer-guide/plugins#bundle-skills).
...
When a skill with config is first loaded, Vigil prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.
...
vigil config get skills.config --json
```
...
```bash
mkdir -p ~/.vigil/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.vigil/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Vigil may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Vigil will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.vigil/skills/devops/`, `~/.vigil/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Vigil to update the skill with what you learned. Skills that aren't maintained become liabilities.
