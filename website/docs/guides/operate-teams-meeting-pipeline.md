...
#### Option 1: Vigil cron (recommended if you already run the Vigil gateway)

Vigil ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.vigil/scripts/`. First create the script:
...
Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.vigil/.env` at the top of a wrapper script that crontab calls.
