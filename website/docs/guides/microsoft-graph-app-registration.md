...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.vigil/.env` at the end.
...
4. Fill in:
   - **Name:** `Vigil Teams Meeting Pipeline` (or any name you'll recognize).
...
Put the three values you collected into `~/.vigil/.env`:
...
Vigil ships a Graph auth smoke-test. From your Vigil install:
