...
Vigil automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.vigil/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Vigil stores session history so it can resume conversations, but it does not
...
Vigil explicitly injects for that turn.
...
For example, if a user sends an image and asks Vigil to make a meme from it,
Vigil may inspect that image once with vision and run an image-processing
...
`vigil sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `vigil sessions optimize` merges FTS5 index segments and
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`vigil` or `vigil chat`) |
...
vigil -c "my project"   # → resumes "my project #3"
```
...
vigil --resume 20250305_091523_a1b2c3d4
vigil -r 20250305_091523_a1b2c3d4

# Resume by title
vigil --resume "refactoring auth"

# Or with the chat subcommand
vigil chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `vigil sessions list`.
...
```bash
vigil --resume 20250305_091523_a1b2c3 --no-restore-cwd
```
...
`vigil sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
vigil sessions list --workspace my-project
vigil sessions list --workspace ~/code/hermes-agent
```

### Conversation Recap on Resume

When you resume a session, Vigil displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Vigil session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.vigil/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `vigil -r "<title>"` from the shell) and pick up where the platform left off.
...
Vigil automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `vigil sessions list` or `vigil sessions browse`.
...
```bash
vigil sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Vigil creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`vigil -c "my project"`), it automatically picks the most recent session in the lineage.
...
Vigil provides a full set of session management commands via `vigil sessions`:
...
# Filter by platform
vigil sessions list --source telegram

# Show more sessions
vigil sessions list --limit 50
```
...
`vigil sessions export` is one surface for every export format, selected with `--format`:
...
vigil sessions export backup.jsonl

# Export sessions from a specific platform
vigil sessions export telegram-history.jsonl --source telegram

# Export a single session
vigil sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
vigil sessions export backup.jsonl --redact
```
...
vigil sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
vigil sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
vigil sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
vigil sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
vigil sessions export --format trace

# One session to a local trace file
vigil sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
vigil sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.vigil/session-exports`).

```bash
# Export one session to Markdown
vigil sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
vigil sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
vigil sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
vigil sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
vigil sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
vigil sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
vigil sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
vigil sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
vigil sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
vigil sessions prune

# Custom age threshold — bare numbers are days
vigil sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
vigil sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
vigil sessions prune --newer-than 5h

# Explicit window with absolute timestamps
vigil sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
vigil sessions prune --source telegram
vigil sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
vigil sessions prune --newer-than 5h --title "smoke test"   # title substring
vigil sessions prune --older-than 30 --max-messages 3        # tiny sessions
vigil sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
vigil sessions prune --model gpt-5 --older-than 1w           # by model (substring)
vigil sessions prune --provider openrouter --older-than 60   # by billing provider
vigil sessions prune --branch feature/old-experiment         # by git branch
vigil sessions prune --user 12345678 --chat-type group       # by messaging origin
vigil sessions prune --max-tokens 500 --older-than 7         # by token usage
vigil sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
vigil sessions prune --newer-than 5h --dry-run

# Skip confirmation
vigil sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `vigil sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `vigil sessions prune` keeps the 90-day cutoff. Every
...
`vigil sessions archive` takes the same filters as `prune` but soft-hides
...
vigil sessions archive --newer-than 5h

# Archive by title substring, preview first
vigil sessions archive --title "dry run" --dry-run
vigil sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `vigil sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`vigil sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
vigil sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`vigil insights`](/reference/cli-commands#hermes-insights).
...
When Vigil cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Vigil uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Vigil in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.vigil/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.vigil/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.vigil/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.vigil/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.vigil/sessions/sessions.json` is a **legacy mirror** of it,
...
`vigil sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.vigil/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `vigil sessions list`, the cause is
`state.db` not receiving them — run `vigil sessions repair` and watch for a
...
`*.jsonl` files in `~/.vigil/sessions/`. They are no longer written or
read by Vigil. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Vigil process in the same `VIGIL_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.vigil/config.yaml`:
...
vigil sessions delete <session_id>
