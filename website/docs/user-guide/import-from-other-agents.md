...
description: "One-command import of a Claude Code (~/.claude) or OpenAI Codex CLI (~/.codex) setup into Vigil — instructions, allowlists, MCP servers, skills, and memories."
---

# Import from Other Agents

`vigil import-agent` imports your existing **Claude Code** or **OpenAI Codex CLI** setup into Vigil with one command. It follows the same preview-first pattern as [`vigil claw migrate`](../guides/migrate-from-openclaw.md): you always see a per-item plan before anything is written, and `--dry-run` never touches disk.
...
vigil import-agent claude-code --dry-run          # preview only
vigil import-agent codex --source /path/to/.codex # custom location
vigil import-agent claude-code --overwrite --yes  # replace conflicts, skip prompts
```
...
| Claude Code | Vigil |
|---|---|
| `CLAUDE.md` (global instructions) | Memory entries in `~/.vigil/memories/MEMORY.md` |
...
| `skills/<name>/` (dirs with `SKILL.md`) | `~/.vigil/skills/claude-code-imports/<name>/` |
...
| Codex CLI | Vigil |
|---|---|
| `AGENTS.md` (global instructions) | Memory entries in `~/.vigil/memories/MEMORY.md` |
| `config.toml` → `[mcp_servers.*]` | `mcp_servers` in `config.yaml` |
| `memories/*.md` | Memory entries in `~/.vigil/memories/MEMORY.md` |
| `skills/<name>/` (dirs with `SKILL.md`) | `~/.vigil/skills/codex-imports/<name>/` |

## What is never imported

**API keys and credentials.** Credential files (`~/.claude/.credentials.json`, `~/.codex/auth.json`) are never read, and MCP server environment variables or headers with secret-looking names (`*_TOKEN`, `*_API_KEY`, `Authorization`, ...) are stripped and listed in the report so you can re-add them deliberately. Run `vigil setup` to configure providers, or add secrets to `~/.vigil/.env`.
...
- **Conflicts are skipped by default.** An MCP server or skill that already exists in Vigil is reported as a conflict; pass `--overwrite` to replace it.
...
- Coming from OpenClaw instead? Use [`vigil claw migrate`](../guides/migrate-from-openclaw.md).
