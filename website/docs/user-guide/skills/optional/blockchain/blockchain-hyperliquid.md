...
| | |
|---|---|
| Source | Optional — install with `vigil skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Vigil |
| License | MIT |
...
The script reads `${VIGIL_HOME:-~/.vigil}/.env` for two optional defaults:
...
Helper script: `~/.vigil/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${VIGIL_HOME:-~/.vigil}/.env`.
