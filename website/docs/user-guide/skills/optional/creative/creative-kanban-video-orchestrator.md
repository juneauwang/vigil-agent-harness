...
| | |
|---|---|
| Source | Optional — install with `vigil skills install official/creative/kanban-video-orchestrator` |
...
short to a music video to an ASCII loop — in a Vigil Kanban pipeline that
...
3. **Generates** a setup script that creates Vigil profiles, project workspace, and the initial kanban task
...
For mapping role → which Vigil skills + toolsets it loads, see
...
3. Creates each Vigil profile via `vigil profile create --clone`
...
7. Fires the initial `vigil kanban create` task assigned to the director

Use `scripts/bootstrap_pipeline.py` to generate setup.sh from a brief +
team-design JSON. See **[references/kanban-setup.md](https://github.com/juneauwang/vigil-agent-harness)**
...
```bash
vigil kanban watch --tenant <project-tenant>     # live events
vigil kanban list  --tenant <project-tenant>     # board snapshot
vigil dashboard                                   # visual board UI
```
...
playbook, see **[references/monitoring.md](https://github.com/juneauwang/vigil-agent-harness)**.
...
task graphs. See **[references/examples.md](https://github.com/juneauwang/vigil-agent-harness)**.
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `vigil kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${VIGIL_HOME:-~/.vigil}/.env` or the user's secret store.
