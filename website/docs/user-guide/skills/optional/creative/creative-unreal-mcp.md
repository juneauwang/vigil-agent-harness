...
| | |
|---|---|
| Source | Optional — install with `vigil skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Vigil MCP catalog. The
...
Two halves, in this order: the editor side must be up before Vigil connects.
...
### One-time, Vigil side
    ...
    vigil mcp install unreal-engine
...
`~/.vigil/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Vigil — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Vigil connects
...
2. Start the Vigil session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Vigil
they appear as:
...
then happens at `vigil mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Vigil runs batched calls
...
- **Start order matters.** Editor + server up first, then the Vigil
...
- **Timeouts on long operations.** Vigil' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.vigil/config.yaml` for
