...
| | |
|---|---|
| Source | Optional — install with `vigil skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${VIGIL_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${VIGIL_SKILL_DIR}/scripts/drive.sh` for private Drive storage
...
| `--client {name}`      | Agent name for attribution (e.g. `vigil`)    |
