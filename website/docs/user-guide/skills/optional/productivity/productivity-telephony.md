...
| | |
|---|---|
| Source | Optional — install with `vigil skills install official/productivity/telephony` |
...
This optional skill gives Vigil practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `${VIGIL_HOME:-~/.vigil}/.env`
...
It does **not** turn Vigil into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Vigil memory
...
### 1) "I want Vigil to own a real phone number"
Use **Twilio**.
...
- pairs well with Vigil `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${VIGIL_HOME:-~/.vigil}/.env`
...
```bash
SCRIPT="$(find ~/.vigil/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
vigil skills search telephony
vigil skills install official/productivity/telephony
```
...
Then save credentials into Vigil:
...
3. Buy it and save it into `${VIGIL_HOME:-~/.vigil}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Vigil calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Vigil's existing `text_to_speech` support.

Use this when:
- you want the call to use Vigil's configured TTS voice rather than Twilio `<Say>`
...
Recommended Vigil TTS -> Twilio Play workflow:

1. Generate the audio with Vigil `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Vigil to create the message audio with `text_to_speech`
...
Important note:
- Vigil TTS is great for prerecorded outbound messages
...
- Vigil STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Vigil memory.
...
- Do not store arbitrary third-party phone numbers in Vigil memory.
...
3. persist that number to `${VIGIL_HOME:-~/.vigil}/.env`
