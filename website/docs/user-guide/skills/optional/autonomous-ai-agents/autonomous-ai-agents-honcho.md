---
title: "Honcho — Configure and troubleshoot Honcho memory for Vigil"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Vigil"
---
...
Configure and troubleshoot Honcho memory for Vigil.
...
# Honcho Memory for Vigil

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Vigil profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/hermes#running-honcho-locally-with-vigil
...
3. **AI peer card** -- the identity card for this Vigil profile's AI peer
...
Honcho models conversations as interactions between **peers**. Vigil creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Vigil instance. Each profile gets its own AI peer so agents develop independent views.
...
Each Vigil profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
Guidelines for Vigil when Honcho memory is active.
...
Config file: `$VIGIL_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
