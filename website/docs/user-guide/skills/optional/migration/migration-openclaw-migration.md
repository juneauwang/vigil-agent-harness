---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Vigil"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Vigil"
---
...
Import an OpenClaw setup (memories, skills) into Vigil.
...
# OpenClaw -> Vigil Migration

Use this skill when a user wants to move their OpenClaw setup into Vigil with minimal manual cleanup.
...
- merge OpenClaw command approval patterns into Vigil `command_allowlist`
- migrate Hermes-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Vigil working-directory configuration
- copy OpenClaw skills into `~/.vigil/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Vigil workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.vigil/tts/`
- archive non-secret docs that do not have a direct Vigil destination
...
1. Prefer the installed path under `~/.vigil/skills/migration/openclaw-migration/`.
...
Vigil CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Vigil. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Vigil workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Vigil skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Vigil unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Vigil targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Vigil destination. Unsupported auth blobs must still be reported as skipped.
...
- Vigil persona state imported
- Vigil memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.vigil/skills/openclaw-imports/`
