...
| | |
|---|---|
| Source | Optional — install with `vigil skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Vigil or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Vigil. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.vigil/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.vigil/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Vigil integration, either:

- configure the server in `~/.vigil/config.yaml` using the `native-mcp` skill, or
...
### Vigil cannot see the deployed server

The server-building part may be correct while the Vigil config is not. Load the `native-mcp` skill and configure the server in `~/.vigil/config.yaml`, then restart Vigil.
