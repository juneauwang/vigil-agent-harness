...
| | |
|---|---|
| Source | Optional — install with `vigil skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Vigil |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Vigil terminal calls (desktop app flow only)
...
### Service Account (recommended for Vigil)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${VIGIL_HOME:-~/.vigil}/.env` (the skill will prompt for this on first load).
...
## Vigil Execution Pattern (desktop app flow)

Vigil terminal commands are non-interactive by default and can lose auth context between calls.
