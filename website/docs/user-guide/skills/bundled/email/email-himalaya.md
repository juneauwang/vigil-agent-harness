...
This skill is separate from the Vigil Email gateway adapter. The gateway
adapter lets people email the agent and uses Vigil' built-in IMAP/SMTP
...
To reply non-interactively from Vigil, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Vigil)** — pipe the message via stdin:
