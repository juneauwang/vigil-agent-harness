...
Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/.vigil/.env`. One bootstrap secret (a machine-account access token) replaces N per-provider keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
2. Create or pick a **Project** (e.g. "Vigil keys").
...
4. **Machine accounts → New machine account → My Vigil machine** → **Projects** tab → grant Read access to your project.
...
1. Download and verify `bws v2.0.0` into `~/.vigil/bin/bws`.
2. Prompt you for the access token (input is hidden). Stored in `~/.vigil/.env` as `BWS_ACCESS_TOKEN`.
...
| `vigil secrets bitwarden status` | Show config + binary version + token presence/validation |
| `vigil secrets bitwarden token` | Rotate the access token: validate the new token against Bitwarden, then store it in `.env` |
...
Bitwarden Secrets Manager: → Run `vigil secrets bitwarden token` to paste a fresh access token ...
```

Fix it without re-running the whole wizard:

```bash
vigil secrets bitwarden token                     # masked prompt
vigil secrets bitwarden token --access-token 0.…  # non-interactive
```
...
| `encrypted_cache.enabled` | `false` | Store the last successful fetch in an AES-GCM encrypted cache at `~/.vigil/cache/bws_cache.enc.json`. |
...
Bitwarden never blocks Vigil startup. If anything goes wrong, you'll see a one-line warning in stderr and Vigil continues with whatever credentials `.env` already had:

| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `vigil secrets bitwarden setup` |
| `Bitwarden rejected the machine-account access token … invalid_client` | Token revoked, expired, machine account deleted — or the token belongs to another region (e.g. EU token hitting the US identity endpoint) | Run `vigil secrets bitwarden token` to paste a fresh token; for region mismatches re-run setup and pick EU/self-hosted (or set `secrets.bitwarden.server_url`) |
| `bws exited 1: invalid access token` | Token revoked or wrong | Run `vigil secrets bitwarden token` with a new token |
...
- Vigil will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — Vigil does not auto-upgrade `bws` to "latest" because upstream release shapes can change.

## When NOT to use this

- **Single-machine personal setups** where `~/.vigil/.env` is fine. You're trading one credential for another and adding a network dependency at startup.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple Vigil installations.
