...
Use the Teams meeting pipeline when you want Vigil to ingest Microsoft Graph meeting events, fetch transcripts first, fall back to recordings plus STT when needed, and deliver a structured summary to downstream sinks.
...
Add Graph app-only credentials to `~/.vigil/.env`:
...
Use this when you want Vigil to post the summary through Microsoft Graph into a Teams chat or channel.
