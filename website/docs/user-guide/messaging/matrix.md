...
sidebar_position: 9
title: "Matrix"
description: "Set up Vigil as a Matrix bot"
---

# Matrix Setup

Vigil integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `mautrix` Python SDK, processes messages through the Vigil pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

Vigil works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Vigil behaves once it's connected.

## How Vigil Behaves

| Context | Behavior |
|---------|----------|
| **DMs** | Vigil responds to every message. No `@mention` needed. Each DM has its own session. Set `MATRIX_DM_MENTION_THREADS=true` to start a thread when the bot is `@mentioned` in a DM. |
| **Rooms** | By default, Vigil requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Vigil supports Matrix threads (MSC3440). If you reply in a thread, Vigil keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Vigil auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. Set `MATRIX_DM_AUTO_THREAD=true` (default false) to also auto-create threads for DM messages — this is distinct from `MATRIX_DM_MENTION_THREADS`, which only starts a thread when the bot is `@mentioned` in a DM. |
| **Commands** | Vigil accepts normal `/commands` when your Matrix client sends them. If your client reserves `/` for local commands, use `!commands` instead; Vigil normalizes known `!command` aliases to `/command`. |
...
| **Shared rooms with multiple users** | By default, Vigil isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
Vigil sends structured Matrix user mentions for explicit Matrix IDs such as `@alice:example.org`. Room-wide `@room` notifications are disabled by default; set `MATRIX_ALLOW_ROOM_MENTIONS=true` only in rooms where the bot is allowed to notify everyone.
:::
...
Vigil now includes the current Matrix room name, room ID, topic, message ID,
...
You can also run Vigil as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

Vigil needs an access token to authenticate with the homeserver. You have two options:
...
Vigil uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
Bridge and appservice deployments need extra loop protection. Vigil always
...
Optional behavior settings in `~/.vigil/config.yaml`:
...
Vigil supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
cd ~/.vigil/hermes-agent && uv pip install -e ".[matrix]"
```
...
When E2EE is enabled, Vigil:

- Stores encryption keys in `~/.vigil/platforms/matrix/store/` (legacy installs: `~/.vigil/matrix/store/`)
...
In Matrix conversations, Vigil exposes Matrix-specific tools to the agent:
...
Set `MATRIX_APPROVAL_REQUIRE_SENDER=false` if you intentionally want any authorized Matrix user in the room to operate an approval/model picker prompt. The default is requester-bound when Vigil knows who requested the action.

### Media Limits

Vigil uploads and downloads Matrix images, files, audio, and video through Matrix media APIs. Multiple generated images are sent as one ordered logical batch, preserving captions and thread context across the batch.
...
Inbound media must use Matrix `mxc://` content URIs. Vigil rejects arbitrary
...
On each startup, if `MATRIX_RECOVERY_KEY` is set, Vigil imports cross-signing keys from the homeserver's secure secret storage and signs the current device. This is idempotent and safe to leave enabled permanently.

If Vigil bootstraps a new Matrix recovery key, it never logs the raw key. Set
...
If you delete `~/.vigil/platforms/matrix/store/crypto.db`, the bot loses its encryption identity. Simply restarting with the same device ID will **not** fully recover — the homeserver still holds one-time keys signed with the old identity key, and peers cannot establish new Olm sessions.

Vigil detects this condition on startup and refuses to enable E2EE, logging: `device XXXX has stale one-time keys on the server signed with a previous identity key`.
...
Vigil supports the same gateway commands in Matrix that it supports on other
...
Vigil only normalizes `!command` when the command is known to the gateway, a
...
**Cause**: The host's system clock is set ahead of real time. The Matrix adapter applies a 5-second startup-grace filter (`event_ts < startup_ts - 5`) to ignore events replayed from initial sync. When the wall clock is ahead, every incoming event looks "older than startup" and is dropped before reaching the message handler — the bot appears connected but never replies. See [#12614](https://github.com/juneauwang/vigil-agent-harness).
...
If you previously used Vigil with `MATRIX_ENCRYPTION=true` and are upgrading to
...
1. **Generate a new access token** to get a fresh device ID. The simplest way:
   ...
   Copy the new `access_token` and update `MATRIX_ACCESS_TOKEN` in `~/.vigil/.env`.

2. **Delete old encryption state**:

   ```bash
   rm -f ~/.vigil/platforms/matrix/store/crypto.db
   rm -f ~/.vigil/platforms/matrix/store/crypto_store.*
   ```

3. **Set your recovery key** (if you use cross-signing — most Element users do). Add to `~/.vigil/.env`:
...
a working E2EE setup with a previous version of Vigil and are upgrading.
...
RUN cd ~/.vigil/hermes-agent && uv pip install -e ".[matrix]"
...
| Setting | Description |
|---------|-------------|
| `GATEWAY_PROXY_URL` | URL of the remote Vigil API server (e.g., `http://192.168.1.100:8642`) |
...
:::tip
Session continuity is maintained via the `X-Vigil-Session-Id` header. The host's API server tracks sessions by this ID, so conversations persist across messages just like they would with a local agent.
:::
...
**Fix**: Vigil uses an explicit sync loop that calls `client.handle_sync()` on
both the initial sync and every incremental sync response. This matches the
diagnosis in upstream issue #7914 and closed PR #37807, but keeps Vigil's own
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the Vigil logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.
...
**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.vigil/.env` and restart the gateway. Use the full `@user:server` format.
...
- **Media support**: Vigil can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
