...
[A2A](https://a2a-protocol.org) is the open Agent2Agent protocol (v1.0, stewarded by the Linux Foundation) for communication between independent AI agents. The Vigil A2A plugin works in **both directions**: your agent can call other A2A agents as tools, and other agents can send tasks to your Vigil over HTTP.

It interoperates with any A2A-compliant peer — another Vigil, LangChain, CrewAI, Google ADK agents, or anything built on the official `a2a-sdk`.

## When to use A2A

- **Vigil ↔ Vigil across machines** — let your desktop agent hand tasks to a Vigil on a server, or vice versa, each with its own memory, tools, and credentials.
...
- **Being a callable service** — expose your Vigil so other frameworks' agents can send it tasks.
...
The outbound client tools ship as the `a2a` toolset, **off by default** — enable it with `vigil tools`.
...
With the platform enabled, Vigil serves:
...
- **Audit log** — every exchange appends to `~/.vigil/a2a_audit.jsonl`.
