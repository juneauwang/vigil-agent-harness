...
sidebar_position: 4
title: "Slack"
description: "Set up Vigil as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Vigil to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Vigil instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Vigil uses the modern
...
The fastest path is to paste a manifest Vigil generates for you. It
...
1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   vigil slack manifest --agent-view --write
   ```
   ...
   ```bash
   vigil slack manifest --agent-view \
...
4. Enter an app name (e.g., "Vigil") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Vigil can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Vigil configuration issue.
:::
...
Vigil uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; Vigil now
...
Every Vigil command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
New Slack apps use Slack's **Agent** messaging experience. Existing Vigil
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so Vigil can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. Vigil only supplies that context as a label; it does not read the viewed
channel's history.
...
3. Paste the new contents of `~/.vigil/slack-manifest.json`
...
Slack never delivers them to Vigil.

As a workaround, Vigil recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Vigil treats it identically to the slash form and replies in the same
thread.
...
The bang form also works behind a mention (`@Vigil !stop`) and with
...
Vigil falls back to a text prompt, the prompt instructs you to reply
...
fails, Vigil retries via a second ephemeral API path — a slash reply is never
posted publicly to the channel as a fallback. (Commands typed as regular
messages — `!cmd` in threads, `@Vigil /cmd` — reply as normal visible
messages instead.)
...
Understanding how Vigil behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Vigil what time is it?`). In channels, Vigil replies in a thread attached to that message. |
| **Threads** | If you @mention Vigil inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.vigil/config.yaml`.
...
platforms:
  slack:
    ...
    extra:
      ...
      # that message itself @mentions Vigil, and "all" accepts every
      # other bot. Vigil always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
| `platforms.slack.extra.allow_bots` | `"none"` | Controls messages from other Slack bots: `"none"` ignores them, `"mentions"` accepts a bot message only when **that message itself** @mentions Vigil, and `"all"` accepts all of them. Use `"mentions"` for the safest bot-to-bot collaboration mode. See [Accepting messages from other bots](#accepting-messages-from-other-bots-allow_bots). |
...
name in the thread. By default Vigil sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"Vigil is thinking…"`) —
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Vigil in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Vigil will respond.
...
By default Vigil ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several Vigil instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- Vigil always ignores its own messages, in every mode, to prevent self-echo loops.

`mentions` is the recommended mode for bot-to-bot collaboration: each agent must explicitly summon the other per turn. Avoid `all` unless every peer bot's own reply policy is loop-safe — two bots that answer everything will answer each other forever. Detection covers labeled bot messages (`bot_id`, `subtype: bot_message`), app-originated events, and unlabeled bot *users* (probed via `users.info`), so peer Vigil agents are filtered consistently across workspaces.
...
manifest — regenerate with `vigil slack manifest`):
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Vigil will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Vigil`).
...
Delivery works even when the cron process isn't co-located with the gateway — Vigil falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
...
Vigil can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Vigil also loads tokens from an **OAuth token file** at:

```
~/.vigil/slack_tokens.json
```
...
- When a message arrives, Vigil uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Vigil` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Vigil` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Vigil now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Vigil`)
...
- Tokens should be stored in `~/.vigil/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Vigil config directory
