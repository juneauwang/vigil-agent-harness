...
description: "Set up Vigil as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Vigil to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Vigil strips automatically before processing.
...
```bash
VIGIL_UID=$(id -u) VIGIL_GID=$(id -g) docker compose up -d gateway
```
...
Alternatively, configure via `~/.vigil/config.yaml`:
...
- Store credentials in `~/.vigil/.env` with permissions `600` (`chmod 600 ~/.vigil/.env`)
