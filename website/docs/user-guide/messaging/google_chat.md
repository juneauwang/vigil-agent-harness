...
sidebar_position: 12
title: "Google Chat"
description: "Set up Vigil as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Vigil to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Vigil
...
download the file. Save it somewhere only Vigil can read (e.g.,
`~/.vigil/google-chat-sa.json`, `chmod 600`).
...
- **App name**: whatever you want users to see ("Vigil" is reasonable).
...
`ADDED_TO_SPACE` event that Vigil uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Vigil

Add the Google Chat section to `~/.vigil/.env`:
...
Send "hola" in the test DM. The bot posts a "Vigil is thinking…" marker, then
...
`~/.vigil/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Vigil is thinking…").
...
Thread support: when a user replies inside a thread, Vigil detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Vigil session.
...
3. Download the JSON. Move it onto the host that runs Vigil.
4. Register the client with Vigil (run under the profile you want it scoped to):
...
That writes the client secret into the active profile's Vigil home (e.g.
`~/.vigil/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.vigil/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.vigil/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Vigil isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: Vigil will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.vigil/google_chat_user_tokens/<sanitized_email>.json` (filesystem
