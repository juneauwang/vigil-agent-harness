...
Connect Vigil to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Vigil — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Vigil runs it in a
...
2. **Finds or creates** the `Vigil` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.vigil/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.vigil/.env`
...
(device token, dashboard project id) lives in `~/.vigil/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Vigil
...
**Open access** (dev only, in `~/.vigil/.env`):
...
By default Vigil responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Vigil` and
`@Vigil agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Vigil will reply.
...
assigned iMessage line Vigil uses. When a Photon token and dashboard project
...
- **Outbound attachments are supported.** Vigil sends images, voice
  ...
  media.
- **Native polls are supported.** Vigil sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** Vigil sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `vigil send`, the dashboard) reuse the sidecar the
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Vigil is set
...
| `PHOTON_MENTION_PATTERNS` | Vigil wake words  | JSON list / comma / newline regex patterns for group mentions |
