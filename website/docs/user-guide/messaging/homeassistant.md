...
description: Control your smart home with Vigil via Home Assistant integration.
...
Vigil integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Vigil"
5. Copy the token
...
Vigil registers four tools for smart home control:
...
Configure which events the agent sees in `~/.vigil/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Vigil".
...
The adapter reads credentials from `~/.vigil/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Vigil profile
...
running Vigil — `curl -H "Authorization: Bearer <token>" <url>/api/` should
