...
Vigil runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Vigil
...
1. Vigil captures a baseline of current diagnostics for the file.
...
…). Vigil auto-detects the binary on PATH or in
`<VIGIL_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Vigil at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<VIGIL_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`vigil lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Vigil installs both packages together when you
run `vigil lsp install typescript` or auto-install fires on first
use.

## CLI

```
vigil lsp status          # service state + per-server install status
vigil lsp list            # registry, optionally --installed-only
vigil lsp install <id>    # eagerly install one server
vigil lsp install-all     # try every server with a known recipe
vigil lsp restart         # tear down running clients
vigil lsp which <id>      # print resolved binary path
```

`vigil lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <VIGIL_HOME>/lsp/bin
...
When `install_strategy: auto`, Vigil installs binaries into
`<VIGIL_HOME>/lsp/bin/`. NPM packages land in
`<VIGIL_HOME>/lsp/node_modules/` with bin symlinks one level up.
...
**`vigil lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<VIGIL_HOME>/lsp/bin/`. Run
`vigil lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `vigil lsp status`**
...
When `vigil lsp status` shows a `Backend warnings` section, install
...
`~/.vigil/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.vigil/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `vigil lsp restart` to clear the set;
