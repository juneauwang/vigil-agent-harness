...
# Optional: only if a browser must call Vigil directly
...
enumerate every authenticated provider/model combination Vigil can route to,
...
Normal opens are intentionally conservative for custom providers: Vigil probes
...
Use this endpoint when integrating dashboards, browser UIs, or control planes so they can discover whether the running Vigil version supports runs, streaming, cancellation, and session continuity without depending on private Python internals.

## Per-request model selection

Authenticated clients can override Vigil' default model selection per request
by sending:

- `model` — the target model id for this turn
- `provider` — the Vigil provider slug to resolve credentials/runtime for this turn
...
alias, Vigil rejects the request with `400` instead of silently remixing route
...
Runs accept a simple `input` string and optional `session_id`, `instructions`, `conversation_history`, or `previous_response_id`. When `session_id` is provided, Vigil surfaces it in the run status so external UIs can correlate runs with their own conversation IDs.
...
Interrupt a running agent turn. The endpoint returns immediately with `{"status": "stopping"}` while Vigil asks the active agent to stop at the next safe interruption point.
...
External UIs can manage Vigil sessions over REST without standing up the dashboard. All endpoints are gated by `API_SERVER_KEY` and live under `/api/sessions/*`.
...
## Long-term memory scoping (`X-Vigil-Session-Key`)

Multi-user frontends like Open WebUI need a stable per-channel identifier for long-term memory (Honcho, etc.) that is **independent** of the transcript-scoped `X-Vigil-Session-Id` (which rotates on `/new`). Pass `X-Vigil-Session-Key` on `/v1/chat/completions`, `/v1/responses`, or `/v1/runs` and Vigil threads it through to `AIAgent(gateway_session_key=...)`, where the Honcho memory provider uses it to derive a stable scope.

```http
POST /v1/chat/completions HTTP/1.1
Authorization: Bearer ***
X-Vigil-Session-Id: transcript-alpha
X-Vigil-Session-Key: agent:main:webui:dm:user-42
```

Rules: max 256 chars, control characters (`\r`, `\n`, `\x00`) are rejected, and the value is echoed back on responses (JSON + SSE). `/v1/capabilities` advertises support via `"session_key_header": "X-Vigil-Session-Key"`. Without the key, Honcho's `per-session` strategy produces a different scope per `session_id` — exactly the behavior Vigil had before.
...
Configure the key via `API_SERVER_KEY` env var. If you need a browser to call Vigil directly, also set `API_SERVER_CORS_ORIGINS` to an explicit allowlist.
...
- Requests to `/p/<profile>/v1/...` must present that profile's own
  `API_SERVER_KEY` (from `~/.vigil/profiles/<profile>/.env`). The default
...
The same settings can live in `~/.vigil/config.yaml` under a nested `gateway.api_server:` section:
...
To give multiple users their own isolated Vigil instance (separate config, memory, skills), use [profiles](/user-guide/profiles):
...
In Open WebUI, add each as a separate connection. The model dropdown shows `alice` and `bob` as distinct models, each backed by a fully isolated Vigil instance. See the [Open WebUI guide](/user-guide/messaging/open-webui#multi-user-setup-with-profiles) for details.
