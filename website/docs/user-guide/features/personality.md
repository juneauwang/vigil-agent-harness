...
description: "Customize Vigil's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Vigil's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `VIGIL_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Vigil is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Vigil now seeds a default `SOUL.md` automatically in:

```text
~/.vigil/SOUL.md
```

More precisely, it uses the current instance's `VIGIL_HOME`, so if you run Vigil with a custom home directory, it will use:
...
- Vigil creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Vigil loads `SOUL.md` only from `VIGIL_HOME`
- Vigil does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Vigil falls back to a built-in default identity
...
If Vigil loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `VIGIL_HOME`, the personality belongs to the Vigil instance itself.

That also makes it easier to teach users:
- "Edit `~/.vigil/SOUL.md` to change Vigil' default personality."
...
- how Vigil should handle uncertainty, disagreement, or ambiguity
...
## What Vigil injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Vigil falls back to a built-in default identity ("You are Vigil, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Vigil ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Vigil, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Vigil its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.vigil/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.vigil/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-vigil)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Vigil speaks
- `display.skin` and `/skin` affect how Vigil looks in the terminal
