...
description: "Build themes and plugins for the Vigil web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---
...
This page covers the **web dashboard** (`vigil dashboard`) plugin system — `window.__VIGIL_PLUGIN_SDK__`, a `manifest.json`, and a pre-built JS bundle. The **native desktop app** (`vigil desktop`) has its own, unrelated SDK — `@hermes/plugin-sdk`, a single ESM file, no build step — documented at [Desktop Plugin SDK](/developer-guide/desktop-plugin-sdk). Only the backend `plugin_api.py` namespace (`/api/plugins/<name>`) is shared between them.
:::
...
Themes are YAML files stored in `~/.vigil/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.vigil/dashboard-themes
```

```yaml
# ~/.vigil/dashboard-themes/neon.yaml
name: neon
label: Neon
...
|-------|---------|------------|--------|
| **Vigil Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **Vigil Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except Vigil Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
```yaml
# ~/.vigil/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other Vigil plugins in `~/.vigil/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.

Plugins don't bundle React or UI components. They use the **Plugin SDK** exposed on `window.__VIGIL_PLUGIN_SDK__`. This keeps plugin bundles tiny (typically a few KB) and avoids version conflicts.
...
```bash
mkdir -p ~/.vigil/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.vigil/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.vigil/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";

  const SDK = window.__VIGIL_PLUGIN_SDK__;
  const { React } = SDK;
  ...
  window.__VIGIL_PLUGINS__.register("my-plugin", MyPage);
})();
```
...
Everything a plugin needs is on `window.__VIGIL_PLUGIN_SDK__`. Plugins should never import React directly.
...
#### Calling built-in Vigil endpoints
...
```javascript
window.__VIGIL_PLUGINS__.registerSlot("my-plugin", "sidebar", MySidebar);
window.__VIGIL_PLUGINS__.registerSlot("my-plugin", "header-left", MyCrest);
```
...
| `header-left` | Before the Vigil brand in the top bar. |
...
window.__VIGIL_PLUGINS__.registerSlot("my-plugin", "sessions:top", PinnedSessionsBanner);
```
...
```json
// ~/.vigil/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.vigil/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__VIGIL_PLUGIN_SDK__;
  const { React } = SDK;
  ...
  window.__VIGIL_PLUGINS__.register("session-notes", function () { return null; });

  // The real work.
  window.__VIGIL_PLUGINS__.registerSlot("session-notes", "sessions:top", Banner);
})();
```
...
A reference plugin (`example-dashboard` in [`hermes-example-plugins`](https://github.com/juneauwang/vigil-agent-harness)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
```python
# ~/.vigil/plugins/my-plugin/dashboard/plugin_api.py
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.vigil/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.hermes/plugins/<name>/dashboard/` | `project` — only when `VIGIL_ENABLE_PROJECT_PLUGINS` is set |
...
1. Dashboard loads. `main.tsx` exposes the SDK on `window.__VIGIL_PLUGIN_SDK__` and the registry on `window.__VIGIL_PLUGINS__`.
...
4. The plugin's IIFE runs and calls `window.__VIGIL_PLUGINS__.register(name, Component)` — and optionally `.registerSlot(name, slot, Component)` for each slot.
...
The [`strike-freedom-cockpit`](https://github.com/juneauwang/vigil-agent-harness) plugin (companion repo `hermes-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **Vigil Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
...
|--------|------|----------|
| `window.__VIGIL_PLUGIN_SDK__` | object | `registry.ts` — React, hooks, UI components, API client, utils. |
| `window.__VIGIL_PLUGINS__.register(name, Component)` | function | Register a plugin's main component. |
| `window.__VIGIL_PLUGINS__.registerSlot(name, slot, Component)` | function | Register into a named shell slot. |
...
Check that the file is in `~/.vigil/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.vigil/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.vigil/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
4. Open browser dev tools → Console — look for errors during the IIFE or `window.__VIGIL_PLUGINS__ is undefined` (indicates the SDK didn't initialize, usually a React render crash earlier).
5. Verify your bundle calls `window.__VIGIL_PLUGINS__.register(...)` with the **same name** as `manifest.json:name`.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.vigil/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
