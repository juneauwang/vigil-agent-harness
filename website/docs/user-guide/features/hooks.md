...
Vigil has four hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.vigil/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.vigil/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
| **[Outbound webhooks](#outbound-webhooks)** | `hooks.outbound:` list in `~/.vigil/config.yaml` | CLI + Gateway | Push signed lifecycle events to external HTTP endpoints — CI, dashboards, other agents |
...
Each hook is a directory under `~/.vigil/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".vigil" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.vigil/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.vigil/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".vigil" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.vigil/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.vigil/hooks/session-webhook/handler.py
import httpx
...
A popular pattern from the community: drop a Markdown checklist at `~/.vigil/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."

This tutorial shows how to build it yourself as a user-defined hook. Vigil does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.

#### What we're building

1. A file at `~/.vigil/BOOT.md` with natural-language startup instructions.
...
Create `~/.vigil/BOOT.md`. Write it as if you were giving instructions to a human assistant:

```markdown
# Startup Checklist

1. Run `vigil cron list` and check if any scheduled jobs failed overnight.
...
**`~/.vigil/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.vigil/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.vigil/hooks/boot-md/handler.py`**

```python
"""Run ~/.vigil/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".vigil" / "BOOT.md"
...
```bash
vigil logs --follow --level INFO | grep boot-md
```
...
Delete `~/.vigil/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
An earlier version of Vigil shipped this as a built-in hook and silently spawned an agent with bare defaults on every gateway boot. That surprised users with custom endpoints and made the feature invisible to users who didn't know it was running. Keeping it as a documented pattern — built by you, in your hooks directory — means you see exactly what it does and opt in by writing the files.

### How It Works

1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.vigil/hooks/`
...
**Where context is injected:** Always the **user message**, never the system prompt. This preserves the prompt cache — the system prompt stays identical across turns, so cached tokens are reused. The system prompt is Vigil's territory (model guidance, tool enforcement, personality, skills). Plugins contribute context alongside the user's input.
...
Declare shell-script hooks in your `~/.vigil/config.yaml` and Vigil will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.vigil/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.vigil/agent-hooks/` (by convention) | `~/.vigil/plugins/<name>/` | `~/.vigil/hooks/<name>/` |
...
Each time the event fires, Vigil spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.vigil/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.vigil/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.vigil/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.vigil/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.vigil/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.vigil/agent-hooks/inject-cwd-context.sh
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate Vigil event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
hooks:
  subagent_stop:
    - command: "~/.vigil/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.vigil/agent-hooks/log-orchestration.sh
log=~/.vigil/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Vigil sees it, then persists the decision to `~/.vigil/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `vigil --accept-hooks chat`)
2. `VIGIL_ACCEPT_HOOKS=1` environment variable
3. `hooks_auto_accept: true` in `~/.vigil/config.yaml`
...
**Script edits are silently trusted.** The allowlist keys on the exact command string, not the script's hash, so editing the script on disk does not invalidate consent. `vigil hooks doctor` flags mtime drift so you can spot edits and decide whether to re-approve.

#### Manual allowlisting

Manual allowlisting is useful for non-TTY or service-account deployments where an operator cannot answer the first-use prompt interactively. The allowlist file is `~/.vigil/shell-hooks-allowlist.json`, and the expected format is an `approvals` array. Each approval records the hook `event` and the exact `command` string:
...
The command string must match the configured hook command exactly. A path-keyed object with a `sha256` field is not the expected format and will not approve the hook. Verify manual entries with `vigil hooks list`.

### The `vigil hooks` CLI

| Command | What it does |
|---------|--------------|
| `vigil hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `vigil hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `vigil hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `vigil hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.vigil/agent-hooks/` so the path is easy to audit.
- Re-run `vigil hooks doctor` after you pull a shared config to spot newly-added hooks before they register.
...
Outbound webhooks are the push-side mirror of the [inbound webhook platform](/user-guide/messaging/webhooks): inbound webhooks wake Vigil when the world changes; outbound webhooks tell the world when Vigil does something. Configure a list of HTTP endpoints and the lifecycle events they care about, and Vigil POSTs a signed JSON payload to each endpoint whenever a matching event fires — no polling on the receiving end.
...
- Wake *another* Vigil instance: point the URL at that instance's inbound webhook

### Configuration

Add a `hooks.outbound:` list to `~/.vigil/config.yaml`:
...
hooks:
  outbound:
    - name: ci-notify                       # optional label for logs
      ...
      secret_env: VIGIL_OUTBOUND_WEBHOOK_SECRET   # env var holding the HMAC secret
...
Secrets: prefer `secret_env` (the name of an environment variable, typically set in `~/.vigil/.env`) over an inline `secret:` literal, so the config file stays free of credentials. Entries without a secret are delivered unsigned (flagged as `UNSIGNED` by `vigil hooks list`).
...
| `X-Vigil-Event` | The hook event name |
| `X-Vigil-Delivery` | Unique id per delivery — same value as `delivery_id` in the body |
| `X-Vigil-Signature-256` | `sha256=<hex>` — HMAC-SHA256 of the raw body, GitHub-style; only present when a secret is configured |
...
- **Dedupe** on `delivery_id` (or the matching `X-Vigil-Delivery` header) — remember recently seen ids and skip duplicates. Vigil retries failed deliveries once, so the same id can legitimately arrive twice.
...
- **No consent prompt.** Outbound targets execute no code on your machine — they receive data at a URL you configured. `VIGIL_SAFE_MODE=1` still skips registration, same as plugins and shell hooks. Note that payloads include tool inputs and event metadata, so only point targets at endpoints you trust, and prefer `https://`.

`vigil hooks list` shows configured outbound targets alongside shell hooks, including whether each target is signed.
