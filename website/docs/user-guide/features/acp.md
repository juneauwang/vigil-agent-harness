...
description: "Use Vigil inside ACP-compatible editors and collaboration platforms"
---

# ACP Host Integration

Vigil can run as an ACP server, letting ACP-compatible hosts talk to
Vigil over stdio. Editors can render:
...
Vigil. ACP is a good fit when you want Vigil to keep its existing identity,
...
Install Vigil normally, then add the ACP extra from the install checkout:
...
Vigil logs to stderr so stdout remains reserved for ACP JSON-RPC traffic.
...
This is the standalone command. The terminal-auth flow (`vigil acp --setup`) also offers the browser bootstrap as a follow-up question after model selection, so most users never need to run `--setup-browser` directly.

What it does:

- Installs Node.js 26 into `~/.vigil/node/` if missing
...
```text
Buzz relay <-- WebSocket --> buzz-acp <-- ACP over stdio --> Vigil
```

This is a transport integration, not a second Vigil installation. The
subprocess launched by `buzz-acp` uses the same Vigil configuration,
credentials, memory, skills, and state as `vigil` on that host.

(This is distinct from [Buzz Desktop's managed runtime](#buzz-desktop), which
spawns Vigil locally as a preset harness. The relay bridge is for joining Buzz
*channels* as an agent identity, typically on a server.)

Prerequisites:

- Complete the ACP installation and `vigil acp --check` above.
- Build `buzz-acp` and the `buzz` CLI from the
  [Buzz repository](https://github.com/block/buzz)
  (`cargo build --release -p buzz-acp`).
- Mint a dedicated Nostr keypair for Vigil (`buzz-admin generate-key`) and
...
export BUZZ_ACP_AGENT_COMMAND="vigil"
...
the same operating-system user that owns the intended Vigil home. Setup,
...
The bridge discovers every Buzz channel where the Vigil identity is a member
and automatically subscribes when it is added to another channel. Buzz channel
membership therefore remains the access boundary; Vigil does not need a
separate channel list in its own configuration.

To expose Vigil ACP activity in the owner's Buzz Desktop, add:
...
where Vigil is expected to work.
...
2. Select **Vigil** from the built-in agent list.
3. Connect and start chatting.

If you want to define Vigil manually, add it through VS Code settings under `acp.agents`:
...
Configure Vigil as a custom agent server in Zed settings:
...
3. Start a new Vigil external-agent thread.
...
Use an ACP-compatible plugin and point it at `vigil acp` or `hermes-acp`.

### Buzz Desktop

[Buzz](https://github.com/block/buzz) ships Vigil as a preset runtime.
With Vigil installed the normal way, Buzz discovers it automatically —
open **Settings → Runtimes** and Vigil appears under your runtimes.

If discovery fails (older installs), make sure the ACP launcher resolves on a
login-shell PATH:

```bash
command -v hermes-acp || command -v vigil
```

Recent installs write both `vigil` and `hermes-acp` launchers into
`~/.local/bin`; running `vigil update` adds the `hermes-acp` launcher to
older installs. As a manual fallback, configure Buzz's agent command as
`vigil` with args `["acp"]`.

#### Model picker

Buzz Desktop (v0.5.1+) renders Vigil' full model menu in the agent's runtime
settings. The list comes from Vigil itself over ACP: it shows every model
from providers you have authenticated in Vigil (the same inventory behind
`vigil model` and the `/model` command), so a model missing from the menu
means its provider has no credentials configured on the Vigil side.
...
change your Hermes-wide default — use `vigil model` for that.

#### Keep Buzz agents owner-only

Buzz creates every agent with **Who can talk to this agent** set to `Owner only`.
Leave it there when the runtime is Vigil.

Two behaviors combine on this path. The `hermes-acp` toolset includes `terminal`
and `execute_code`, and Buzz's ACP bridge answers Vigil' permission requests
itself with `allow_once` rather than surfacing them. A Vigil agent in Buzz
...
- `approvals.mode: manual` does make Vigil raise the permission request, but
...
ACP mode uses the same Vigil configuration as the CLI:
...
Provider resolution uses Vigil' normal runtime resolver, so ACP inherits the currently configured provider and credentials. Vigil also advertises a terminal auth method (`--setup`) for first-run ACP clients; this opens Vigil' interactive model/provider setup.

## Host integration

These variables are set by an **ACP host process** (an editor or another agent
harness) on the Vigil subprocess it spawns. They are not user configuration —
do not set them by hand in `.env` or `config.yaml`.

| Variable | Value | Effect |
|----------|-------|--------|
| `VIGIL_ACP_SKIP_CONFIGURED_MCP` | `1` | Skip starting the **globally configured** MCP servers from `config.yaml` before the ACP JSON-RPC loop begins. |

Vigil normally starts every MCP server configured in `config.yaml` before it
...
The underlying `AIAgent` still uses Vigil' normal persistence/logging paths, but ACP `list/load/resume/fork` are scoped to the currently running ACP server process.

## Working directory behavior

ACP sessions bind the editor's cwd to the Vigil task ID so file and terminal tools run relative to the editor workspace, not the server process cwd.
...
| `allow_always` | Allow always | All future sessions | Yes (written to the Vigil permanent allowlist) |
| `deny` | Deny | This one tool call | No |

`allow_session` is the right default for an editor workflow where you trust an agent for the duration of a task but don't want to grant a long-lived allowlist entry. The safety trade-off is straightforward: the broader the scope, the less the editor will interrupt you, and the more damage a misbehaving agent (or prompt injection) can do before you notice. Start with `allow_once` for unfamiliar commands; promote to `allow_session` once you've seen the agent run the same pattern correctly a few times; reserve `allow_always` for truly idempotent commands you trust forever (e.g. `git status`).

The ACP bridge maps these options onto Vigil' internal approval semantics — `allow_always` writes a permanent allowlist entry the same way the CLI does, while `allow_session` only affects the in-process approval cache for the current ACP session.
...
- For manual/local development, verify the host command points to `vigil acp`.
- Vigil is installed and on your PATH.
- The ACP extra is installed (`cd ~/.vigil/hermes-agent && uv pip install -e '.[acp]'`).
...
or by editing `~/.vigil/.env`. The terminal auth flow (`vigil acp --setup`) can also trigger the interactive provider/model setup.
