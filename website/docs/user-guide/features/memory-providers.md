...
Vigil ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.
...
When a memory provider is active, Vigil automatically:
...
See the [config reference](https://github.com/juneauwang/vigil-agent-harness) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
`vigil memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.vigil/.env`, and for named profiles use
`~/.vigil/profiles/<profile>/.env`.
...
`OPENVIKING_AGENT` is Vigil' peer ID in OpenViking for peer-scoped memories.
...
```bash
vigil memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
vigil memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.vigil/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.vigil/.env
```
...
**Switching modes:** Re-run `vigil memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
| `retain_context` | `conversation between Vigil and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/juneauwang/vigil-agent-harness) for the full configuration reference.
...
| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$VIGIL_HOME/memory_store.db` | SQLite database path |
...
- Knowledge tree stored at `$VIGIL_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=vigil), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
Before running `vigil memory setup`, set `base_url` in
`$VIGIL_HOME/supermemory.json`:
...
Then run `vigil memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$VIGIL_HOME/supermemory.json`
...
- **Local storage providers** (Holographic, ByteRover) use `$VIGIL_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$VIGIL_HOME/` so each profile has its own credentials
