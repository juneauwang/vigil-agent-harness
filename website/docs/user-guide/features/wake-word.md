...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Vigil' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Vigil")

The wake word turns Vigil into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Vigil listens in the background for a spoken
trigger phrase. Say it, and Vigil starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Vigil backend (for example a
...
Vigil supports **client capture** for that case:
...
|--------|------|---------|-------|
| **openWakeWord** (default) | Free | None | Local ONNX models. Ships a bundled **"hey vigil"** model (default); also supports `hey_jarvis`, `alexa`, `hey_mycroft`, … and custom models |
...
By default the phrase is **"hey vigil"** — a model for it ships with Vigil, so
...
```bash
cd ~/.vigil/hermes-agent && uv pip install -e ".[wake]"
```

## Quick start

```bash
# In an interactive `vigil` session:
...
desktop ear button — also writes `wake_word.enabled` to `~/.vigil/config.yaml`,
...
wake_word:
  ...
  phrase: "hey vigil"        # cosmetic label only — detection is keyed by the model/keyword below
...
- **`confirmation_frames`** (default `3`, openWakeWord only) — how many
  *consecutive* over-threshold frames are required before the wake fires. A real
  "hey vigil" holds a high score across several frames; an ambient blip spikes
...
- **`sensitivity`** (default `0.6`) — the detection threshold, `0.0`–`1.0`.
  ...
  **all** engines — for openWakeWord it's the raw per-frame score threshold, for
  ...
  if real "hey vigil" utterances are missed.
...
default) to let Vigil choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Vigil installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Vigil surfaces, and `surface` picks which one
...
| `cli` | Only the classic `vigil` CLI. |
| `tui` | Only `vigil --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when Vigil surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Vigil does not silently fail over to another open surface.
...
"Hey Vigil" works out of the box — the bundled openWakeWord model
...
command (`vigil -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.vigil/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
syllables with an uncommon word ("vigil" qualifies) beat common words like
"hello" or "stop".
:::

### Option C — Porcupine (custom keyword in seconds)

Create a "Hey Vigil" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey vigil"
  porcupine:
    keyword: ~/.vigil/wakewords/hey_hermes.ppn
```

Set your access key in `~/.vigil/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `vigil tools` (Voice section) sets them up.
...
but the phrase never fires. Vigil detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Vigil
backend (it may appear as your terminal, `python`, or Vigil), then toggle the
...
```bash
vigil config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
vigil config set wake_word.input_device null
```
