...
Beyond installing pre-made pets from the gallery, Vigil can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `vigil pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `vigil tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `VIGIL_PET_IMAGE_PROVIDER` env var (e.g. `VIGIL_PET_IMAGE_PROVIDER=openrouter`).
