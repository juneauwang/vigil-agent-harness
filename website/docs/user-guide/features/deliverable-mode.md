...
When Vigil runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.vigil/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.vigil/config.yaml` (switchable per session
via `/personality`).
...
If you use Vigil' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.vigil/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Vigil's deliverable
