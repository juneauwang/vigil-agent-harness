...
| `vigil moa` | Configure named Mixture of Agents presets selectable from the model picker. |
...
| `vigil egress` | Outbound credential-injection firewall for remote terminal sandboxes (iron-proxy). Disabled by default. See [Egress proxy](../user-guide/egress/iron-proxy.md). |
...
| `vigil whatsapp-cloud` | Configure the official Meta WhatsApp Business Cloud API adapter (Business account + public webhook required). Distinct from `vigil whatsapp` (Baileys personal-account bridge). |
...
| `vigil project` | Manage named, multi-folder workspaces (projects). Anchors desktop session grouping and, when bound to a kanban board, gives tasks a deterministic worktree + branch convention. State is per-profile. |
...
| `vigil approvals` | Approval-prompt tools — mine approval history into allowlist proposals. |
...
| `vigil skin` | List, switch, and tweak display skins. |
...
| `vigil journey` (aliases `learning`, `memory-graph`) | Timeline of learned skills + memories over time. |
...
| `vigil plugins` | Manage Vigil plugins (install, enable, disable, remove). |
| `vigil portal` | Nous Portal status, subscription link, and Tool Gateway routing. See [Tool Gateway](../user-guide/features/tool-gateway.md). |
| `vigil tools` | Configure enabled tools per platform. |
| `vigil computer-use` | Install or check the Computer Use (cua-driver) backend (macOS/Windows/Linux). |
| `vigil pets` | Browse, install, and select [petdex](../user-guide/features/pets.md) animated pets shown across the CLI, TUI, and desktop app. Subcommands: `list`, `install`, `select`, `show`, `off`, `scale`, `remove`, `doctor`. |
...
| `vigil import-agent` | Import a Claude Code (`~/.claude`) or Codex CLI (`~/.codex`) setup. |
| `vigil dashboard` | Launch the web dashboard for managing config, API keys, and sessions. |
| `vigil serve` | Start the Vigil backend server (headless; powers the desktop app and remote backends). |
| `vigil desktop` (alias `gui`) | Build and launch the native Electron desktop app. |
...
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, shell hooks, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from Vigil itself. |
...
`vigil -z "…" --usage-file /path/report.json` writes a machine-readable usage report after the run: `estimated_cost_usd`, `input_tokens` / `output_tokens` / `cache_read_tokens` / `cache_write_tokens` / `reasoning_tokens` / `total_tokens`, `api_calls`, `model`, `provider`, `session_id`, `service_tier`, and `completed` / `failed` flags. The report is written **even when the run fails**, so batch pipelines can always account for spend. It has no effect outside `-z`/`--oneshot`, and a broken usage write never masks the run's own outcome.

```bash
vigil -z "summarize this repo" --usage-file /tmp/usage.json
...
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [Vigil Relay](/user-guide/messaging/relay). |
...
`vigil gateway enroll` accepts `--token`, `--connector-url`, `--gateway-id`, and `--wake-url`. It exchanges the enrollment token with the connector and writes the resulting `GATEWAY_RELAY_ID`, `GATEWAY_RELAY_SECRET`, `GATEWAY_RELAY_DELIVERY_KEY`, optional `GATEWAY_RELAY_URL`, and (when `--wake-url` is given) `GATEWAY_RELAY_WAKE_URL` values to the active profile's `.env`.

:::tip WSL users
Use `vigil gateway run` instead of `vigil gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s vigil 'vigil gateway run'`. See [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-hermes-gateway-start-fails) for details.
:::
...
**Easiest path:** `vigil setup --portal` — OAuth into Nous Portal and opt into the [Tool Gateway](../user-guide/features/tool-gateway.md) in one shot.
...
vigil slack manifest --long-description-file AGENTS.md --write
...
vigil egress install --force          # re-download even if already installed

vigil egress setup                    # interactive wizard: CA, mappings, config
vigil egress setup --tunnel-port N    # override the tunnel listener port (default 9090)
vigil egress setup --from-bitwarden   # use Bitwarden Secrets Manager as credential source
vigil egress setup --no-bitwarden     # explicitly switch back to env-based credentials
vigil egress setup --rotate-tokens    # mint fresh proxy tokens (default preserves existing)

vigil egress start                    # spawn the managed proxy daemon
vigil egress stop                     # SIGTERM (then SIGKILL after 5s grace)
vigil egress restart                  # stop (if running) then start — needed for secret changes
vigil egress reload                   # hot-reload the ruleset in-place (no restart, no dropped
                                       #   connections) via the loopback management API

vigil egress status                   # binary + config + pid + listening + mappings
vigil egress status --show-tokens     # print proxy tokens in full (default: redacted)

vigil egress disable                  # flip proxy.enabled = false (does not stop a running proxy)
vigil egress config                   # print the path to proxy.yaml for inspection
```
...
vigil egress setup && vigil egress start
vigil config set terminal.backend docker   # if not already

# Switching credential source after the fact
vigil egress setup --from-bitwarden       # env → bitwarden
vigil egress setup --no-bitwarden         # bitwarden → env
# (just `setup` without either flag preserves the existing mode)

# Rotating all tokens (e.g. after a suspected token leak)
vigil egress setup --rotate-tokens    # setup offers to restart the running daemon for you
# (running sandboxes still hold old tokens; restart them too)

# Adding a new upstream
# Edit ~/.vigil/config.yaml proxy.extra_allowed_hosts: [api.example.com]
vigil egress setup
vigil egress restart                  # one-command apply (stop + start)
```

### Diagnostic shortcuts

```bash
vigil egress status                     # current state in one view
cat ~/.vigil/proxy/proxy.yaml           # the rendered iron-proxy config
tail -20 ~/.vigil/proxy/iron-proxy.log  # daemon-level diagnostics
tail -f ~/.vigil/proxy/iron-proxy.log | jq  # daemon + per-request log (line-delimited JSON; v0.39 combines both streams)
```
...
```bash
vigil project <create|list|show|add-folder|remove-folder|rename|set-primary|use|archive|restore|bind-board>
```
...
| `--script` | Filter/transform script under `~/.vigil/scripts/`. The webhook payload is passed as JSON on stdin; JSON stdout replaces the payload, and empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. See [Script Filters and Transforms](../user-guide/messaging/webhooks.md#script-filters-and-transforms). |
...
The report includes system info (OS, Python version, Vigil version), recent agent, gateway, GUI/dashboard, and desktop logs (512 KB limit per file), and redacted API key status. By default, uploads are redacted so secrets are not included.
...
vigil debug share --nous       # Upload a private diagnostics bundle for Nous support
...
Create a zip archive of your Vigil configuration, skills, sessions, and data. The backup excludes the hermes-agent codebase itself.
...
```bash
vigil backup                           # Full backup to ~/hermes-backup-*.zip
vigil backup -o /tmp/hermes.zip        # Full backup to specific path
...
### Examples
```bash
vigil import ~/hermes-backup-20260423.zip           # Prompts before overwriting existing config
vigil import ~/hermes-backup-20260423.zip --force   # Overwrite without prompting
```
...
| `get <key> [--json]` | Print a single config value by dotted key (e.g. `vigil config get model.default`). `--json` emits machine-readable output. |
...
vigil skills install https://sharethis.chat/SKILL.md                     # Direct URL (+ referenced support files)
...
```bash
vigil moa list
vigil moa configure [name]
vigil moa delete <name>
```

`vigil moa configure` reuses Vigil' provider → model picker for each reference model and the aggregator. A preset is an execution-mode configuration, not a primary model or provider.
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Vigil](../guides/use-mcp-with-hermes.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server).
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Vigil Plugin](../developer-guide/plugins/index.md).
...
```bash
vigil pets <list|install|select|show|off|scale|remove|doctor>
```

[Petdex](https://github.com/crafter-station/petdex) is a public gallery of animated sprite pets for coding agents. Install one and Vigil shows it reacting to agent activity across the CLI, TUI, and desktop app.
...
```bash
vigil import-agent [claude-code|codex] [options]
```

Import a **Claude Code** (`~/.claude`) or **OpenAI Codex CLI** (`~/.codex`) setup into Vigil. Maps `CLAUDE.md`/`AGENTS.md` instructions to memory entries, `Bash(...)` permission allow/deny rules to `command_allowlist`/`approvals.deny`, MCP servers to `mcp_servers` in `config.yaml`, and skill directories into `~/.vigil/skills/`. Always previews before applying; API keys and credentials are never imported.
...
Start the Vigil **backend server** — the JSON-RPC/WebSocket gateway the [desktop app](/user-guide/desktop) and remote clients connect to. It is the same server `vigil dashboard` runs, but **headless**: it never opens a browser UI. The desktop app launches its own `vigil serve` backend; use this command directly when you want a headless backend on a remote host. Accepts the same `--host` / `--port` / `--insecure` / `--skip-build` / `--stop` / `--status` options as `vigil dashboard` below (a non-loopback bind engages the same auth gate). Requires the `[web]` extra; the embedded Chat socket additionally needs `[pty]` on a POSIX host.
...
Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. (For a headless backend with no browser UI — e.g. what the desktop app spawns — use [`vigil serve`](#hermes-serve) above.) Requires `cd ~/.vigil/hermes-agent && uv pip install -e ".[web]"` (FastAPI + Uvicorn). The embedded browser Chat tab is always available and additionally needs the `pty` extra (`cd ~/.vigil/hermes-agent && uv pip install -e ".[web,pty]"`) plus a POSIX PTY environment such as Linux, macOS, or WSL2. See [Web Dashboard](/user-guide/features/web-dashboard) for full documentation.
...
Register this install as a self-hosted dashboard with your Nous Portal account. Creates an OAuth client, writes `VIGIL_DASHBOARD_OAUTH_CLIENT_ID` into `~/.vigil/.env`, and prints how to engage the login gate. Requires being logged in (`vigil setup`).
...
| `--portal-url` | Override the Nous Portal base URL for registration (default: the portal you logged into). Also settable via `VIGIL_DASHBOARD_PORTAL_URL`. |
...
| `--backup` | Force a **full** pre-update backup for this run: the quick state snapshot plus a complete zip of `VIGIL_HOME` (config, auth, sessions, skills, pairing data). The default mode is `quick` — a lightweight state snapshot only. Set the permanent mode via `updates.pre_update_backup: quick | full | off` in `config.yaml`. |
...
- **Legacy `hermes.service` warning.** If Vigil detects a pre-rename `hermes.service` systemd unit (instead of the current `hermes-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| `vigil uninstall [--full] [--gui] [--dry-run] [--yes]` | Remove Vigil, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--dry-run` prints what would be removed without changing anything; `--yes` skips prompts. |
