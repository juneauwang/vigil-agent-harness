...
description: "Catalog of bundled skills that ship with Vigil"
---

# Bundled Skills Catalog

Vigil ships with a large built-in skill library copied into `~/.vigil/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.

Vigil also syncs bundled skills on `vigil update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.vigil/skills/` tree, it is still shipped with Vigil; restore it with `vigil skills reset <name> --restore`.
...
| [`hermes-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-hermes-agent) | Use, configure, theme, extend, and orchestrate Vigil. | `autonomous-ai-agents/hermes-agent` |
...
| [`inspecting-hermes-desktop-dom`](/docs/user-guide/skills/bundled/software-development/software-development-inspecting-hermes-desktop-dom) | Read the live Vigil desktop DOM/CSS over CDP. | `software-development/inspecting-hermes-desktop-dom` |
