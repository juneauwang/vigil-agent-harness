...
This page covers all commands related to [Vigil profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `vigil profile` without a subcommand shows help.
...
```bash
vigil profile use <name>
```

Sets `<name>` as the active profile. All subsequent `vigil` commands (without `-p`) will use this profile.
...
vigil profile use default
```

## `vigil profile create`

```bash
vigil profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `vigil profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `vigil update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.vigil`), use `vigil skills opt-out` / `vigil skills opt-in`. |
...
vigil profile create work2 --clone-from work

# Clone everything from a specific profile
vigil profile create work2-backup --clone-from work --clone-all
```

## `vigil profile describe`

```bash
vigil profile describe [<name>] [options]
```
...
vigil profile describe researcher

# Set it explicitly
vigil profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
vigil profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
vigil profile describe --all --auto
```

## `vigil profile delete`

```bash
vigil profile delete <name> [options]
```
...
```bash
vigil profile delete mybot
vigil profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.vigil`) cannot be deleted — use `vigil uninstall` to remove everything.
:::

## `vigil profile show`

```bash
vigil profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Vigil home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ vigil profile show work
Profile: work
Path:    ~/.vigil/profiles/work
...
```bash
vigil profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Vigil installation.
...
```bash
vigil profile alias work
# Creates/updates ~/.local/bin/work

vigil profile alias work --name mywork
# Creates ~/.local/bin/mywork

vigil profile alias work --remove
# Removes the wrapper script
```

## `vigil profile rename`

```bash
vigil profile rename <old-name> <new-name>
```
...
```bash
vigil profile rename mybot assistant
# ~/.vigil/profiles/mybot → ~/.vigil/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `vigil profile export`

```bash
vigil profile export <name> [options]
```
...
```bash
vigil profile export work
# Creates work.tar.gz in the current directory

vigil profile export work -o ./work-2026-03-29.tar.gz
```

## `vigil profile import`

```bash
vigil profile import <archive> [options]
```
...
```bash
vigil profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

vigil profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`vigil profile export` / `import` are still the right commands for
...
### `vigil profile install`

```bash
vigil profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `vigil -p telemetry`). |
...
vigil profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
vigil profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
vigil profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
vigil profile install ./telemetry/
```

### `vigil profile update`

```bash
vigil profile update <name> [--force-config] [--yes]
```
...
```bash
vigil profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Vigil version, author, env var requirements, the source URL/path, and
...
`vigil profile list` also shows the distribution name and version in a
`Distribution` column, and `vigil profile show <name>` / `delete <name>`
...
vigil profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
vigil profile install https://github.com/your-org/internal-assistant.git
```
...
Vigil version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Vigil can clone from.
3. Tell recipients to run `vigil profile install <your-repo-url>`.
...
## `vigil -p` / `vigil --profile`

```bash
vigil -p <name> <command> [options]
vigil --profile <name> <command> [options]
```

Global flag to run any Vigil command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
vigil -p work chat -q "Check the server status"
vigil --profile dev gateway start
vigil -p personal skills list
vigil -p work config edit
```

## `vigil completion`

```bash
vigil completion <shell>
```
...
- `vigil profile <TAB>` — subcommands (list, use, create, etc.)
- `vigil profile use <TAB>` — profile names
- `vigil -p <TAB>` — profile names
