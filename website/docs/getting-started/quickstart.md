...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Vigil — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Vigil setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Vigil Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.
...
- Setting up Vigil for a team, bot, or always-on workflow
...
## 1. Install Vigil
### With the Vigil Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Vigil Desktop installer](https://github.com/juneauwang/vigil-agent-harness) from our website and run it.

### Without Vigil Desktop:
For a command-line only install without Vigil Desktop, run:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `vigil model` |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hermes_cli/models.py` may change between releases) | `vigil model` → MiniMax (OAuth) |
...
Vigil separates secrets from normal config:

- **Secrets and tokens** → `~/.vigil/.env`
- **Non-secret settings** → `~/.vigil/config.yaml`
...
For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `vigil egress setup && vigil egress start`; `vigil setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Vigil install directory (the curl installer placed it at
# ~/.vigil/hermes-agent on Linux/macOS or %LOCALAPPDATA%\hermes\hermes-agent on Windows):
...
/k8s                                       # load it and let Vigil ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.vigil/hermes-agent && uv pip install -e ".[acp]"` first.)
...
- **[Moving to another machine](/reference/faq#exporting-hermes-to-another-machine)** — `vigil backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
