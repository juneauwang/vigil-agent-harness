...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Vigil already has applies. The plugin never sees a
...
[`plugin-llm-async-example`](https://github.com/juneauwang/vigil-agent-harness)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Vigil provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Vigil provider
        # (must be one Vigil already knows about — same names as
        # `vigil model` and config.yaml model.provider).
        ...
        # plugin sends — Vigil does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.vigil/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Vigil' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Vigil subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/juneauwang/vigil-agent-harness)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/juneauwang/vigil-agent-harness)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/juneauwang/vigil-agent-harness) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/juneauwang/vigil-agent-harness) — async with `asyncio.gather()`
