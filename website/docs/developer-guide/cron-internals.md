...
description: "How Vigil stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `hermes_cli/cron.py` | CLI `vigil cron` subcommands |
...
Jobs are stored in `~/.vigil/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `vigil cron` commands are run or during active CLI sessions.
...
```python
# ~/.vigil/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.vigil/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `vigil cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `vigil cron` CLI provides direct job management:

```bash
vigil cron list                    # Show all jobs
vigil cron create                  # Interactive job creation (alias: add)
vigil cron edit <job_id>           # Edit job configuration
vigil cron pause <job_id>          # Pause a running job
vigil cron resume <job_id>         # Resume a paused job
vigil cron run <job_id>            # Trigger immediate execution
vigil cron remove <job_id>         # Delete a job
```
