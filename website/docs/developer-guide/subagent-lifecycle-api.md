...
Plugins can launch and supervise fresh Vigil child sessions without importing
...
active agent turn fails closed with `No active Vigil parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Vigil can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Vigil's existing unsafe-tool block remains enforced.
