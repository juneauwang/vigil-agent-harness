...
description: "How Vigil builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Vigil deliberately separates:
...
```
# Layer 1: Agent Identity (from ~/.vigil/SOUL.md)
You are Vigil, an AI assistant created by Nous Research.
...
The platform hint (Layer 10 above) is the per-surface guidance Vigil
...
`SOUL.md` lives at `~/.vigil/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
```
You are Vigil, an intelligent AI assistant created by Nous Research.
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    # SOUL.md from VIGIL_HOME (independent of project context)
    if not skip_soul:
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, Vigil concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Vigil already loads, rather than editing Python templates in place.
...
- if you want to change how Vigil assembles prompts for everyone, change Python and treat it as a code contribution
