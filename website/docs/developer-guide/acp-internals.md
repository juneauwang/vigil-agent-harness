...
The ACP adapter wraps Vigil' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
vigil acp / hermes-acp / python -m acp_adapter
...
- `allow_once` -> Vigil `once`
- `allow_always` -> Vigil `always`
- reject options -> Vigil `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Vigil tools to ACP tool kinds and builds editor-facing content.
...
So ACP advertises and uses the currently configured Vigil provider/credentials. It also always advertises a terminal setup auth method (`hermes-setup`, args `--setup`) so first-run ACP clients can open Vigil' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.vigil/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `hermes_cli/main.py` — `vigil acp` CLI subcommand
