...
This guide covers adding a new messaging platform to the Vigil gateway. A platform adapter connects Vigil to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.vigil/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core Vigil code. Your plugin is a directory with two files:

```
~/.vigil/plugins/my-platform/
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `vigil config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-hermes-config) below).
...
| `vigil config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `vigil status` | Shows plugin platforms with `(plugin)` tag |
| `vigil gateway setup` | Plugin platforms appear in setup menu |
| `vigil tools` / `vigil skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.vigil/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `vigil gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `vigil cron run` separate from `vigil gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `vigil config`
...
version: 1.0.0
description: >
  My Platform gateway adapter for Vigil.
author: Your Name
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #vigil)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
:::note
This checklist is for adding a platform directly to the Vigil core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`toolsets.py`** — Add `"hermes-newplat"` toolset definition with `_VIGIL_CORE_TOOLS`
