...
description: "How the iron-proxy egress firewall integrates with Vigil — module layout, lifecycle, security invariants, and extension points"
---

# Egress proxy internals

This page covers the architecture of the egress credential-injection firewall (`vigil egress` / iron-proxy) from a contributor / plugin author's perspective. End-user setup + usage docs live at [Egress proxy](../user-guide/egress/iron-proxy.md).
...
hermes_cli/proxy_cli.py               Wizard + slash command handlers.
                                       `vigil egress {install,setup,start,stop,
...
hermes_cli/main.py:_dispatch_egress   Top-level subparser dispatcher.
                                       ...
                                       `vigil proxy` subparser, which uses
                                       dest='proxy_command').
...
tools/environments/docker.py
  ...
  DockerEnvironment.__init__          Docker-side merge logic: collision
                                       ...
                                       _VIGIL_EGRESS_NODE_OPTIONS_APPEND
...
tests/test_iron_proxy_e2e.py          Live E2E (gated on VIGIL_RUN_E2E=1).
...
vigil egress install
  -> agent.proxy_sources.iron_proxy.install_iron_proxy(force=...)
       ...
       Stage into ~/.vigil/bin/.iron-proxy_XXXX, chmod 755, os.replace
         to ~/.vigil/bin/iron-proxy (atomic).
       _VERSION_CACHE.pop(target) so a forced reinstall re-probes
         --version on next call.

vigil egress setup [--from-bitwarden | --no-bitwarden] [--rotate-tokens]
  -> proxy_cli.cmd_setup
...
| Path | Mode | Test |
|---|---|---|
| `~/.vigil/proxy/` (dir) | `0o700` | `test_proxy_state_dir_is_0o700` |
...
`merge_mappings(existing, discovered, rotate=False)` MUST return prior tokens for providers that overlap.  Re-running `vigil egress setup` cannot silently 401 running sandboxes.  `--rotate-tokens` is the explicit opt-in.
...
`cmd_setup` MUST NOT downgrade `credential_source: bitwarden` to `env` on re-run without an explicit `--no-bitwarden` flag.  Running `vigil egress setup` (no flag) preserves whatever was previously configured.
...
If the provider uses SigV4 / SDK-minted OAuth / request signatures, a static header swap cannot cover it.  Add the env var to `_NON_BEARER_PROVIDERS` so the wizard and `vigil egress status` warn about it:
...
4. Sets the seven env vars (HTTPS_PROXY, NO_PROXY, REQUESTS_CA_BUNDLE, SSL_CERT_FILE, CURL_CA_BUNDLE, NODE_EXTRA_CA_CERTS, VIGIL_EGRESS_PROXY) and the per-mapping `VIGIL_PROXY_TOKEN_<NAME>` vars.
...
VIGIL_RUN_E2E=1 scripts/run_tests.sh tests/test_iron_proxy_e2e.py

# Live PTY smoke against `vigil egress`
VIGIL_HOME=/tmp/hermes-egress-test python3 -m hermes_cli.main egress --help
VIGIL_HOME=/tmp/hermes-egress-test python3 -m hermes_cli.main egress setup --help
```
...
- User-facing setup + troubleshooting: [Egress proxy](https://github.com/juneauwang/vigil-agent-harness)
- Docker backend internals: [Docker](https://github.com/juneauwang/vigil-agent-harness)
- Bitwarden Secrets Manager integration: [`vigil secrets bitwarden`](https://github.com/juneauwang/vigil-agent-harness)
- CLI command reference: [`vigil egress`](https://github.com/juneauwang/vigil-agent-harness)
- Sandbox-injected environment variables: [Egress proxy (sandbox-injected)](https://github.com/juneauwang/vigil-agent-harness)
