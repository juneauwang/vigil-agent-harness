# nix/hermes-agent.nix — Overridable Vigil Agent package
#
...
let
  ...
  # lib.nix) — the wrapper exposes them via VIGIL_OPTIONAL_SKILLS, the
  ...
  # to a dev checkout — the loader reads them from VIGIL_BUNDLED_PLUGINS.
  ...
  # at by VIGIL_BUNDLED_LOCALES so the wrapped binary always resolves human
  ...
  # exposed via VIGIL_OPTIONAL_MCPS.
...
stdenv.mkDerivation (finalAttrs: {
    ...
    ${lib.concatMapStringsSep "\n"
      (name: ''
        makeWrapper ${hermesVenv}/bin/${name} $out/bin/${name} \
          ...
          --set VIGIL_BUNDLED_SKILLS $out/share/hermes-agent/skills \
          --set VIGIL_OPTIONAL_SKILLS $out/share/hermes-agent/optional-skills \
          --set VIGIL_BUNDLED_PLUGINS $out/share/hermes-agent/plugins \
          --set VIGIL_BUNDLED_LOCALES $out/share/hermes-agent/locales \
          --set VIGIL_OPTIONAL_MCPS $out/share/hermes-agent/optional-mcps \
          --set VIGIL_WEB_DIST $out/share/hermes-agent/web_dist \
          --set VIGIL_TUI_DIR $out/ui-tui \
          --set VIGIL_PYTHON ${hermesVenv}/bin/python3 \
          --set VIGIL_NODE ${lib.getExe hermesNpmLib.nodejs}${
  ...
  passthru =
    ...
    in
    {
      ...
      # ("existing vigil on PATH") then picks up the fully wrapped
      # `vigil` binary — venv with all deps, bundled skills/plugins,
      ...
      devShellHook = ''
        export VIGIL_PYTHON=${devPython}/bin/python3
      '';
