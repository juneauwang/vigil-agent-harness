    ...
    in {
      ...
      checks = {
        ...
        bundled-skills = pkgs.runCommand "hermes-bundled-skills" { } ''
          ...
          grep -q "VIGIL_BUNDLED_SKILLS" ${hermes-agent}/bin/hermes || \
            (echo "FAIL: VIGIL_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: VIGIL_BUNDLED_SKILLS set in wrapper"
          ...
          grep -q "VIGIL_OPTIONAL_SKILLS" ${hermes-agent}/bin/hermes || \
            (echo "FAIL: VIGIL_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, VIGIL_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "hermes-bundled-plugins" { } ''
          ...
          grep -q "VIGIL_BUNDLED_PLUGINS" ${hermes-agent}/bin/hermes || \
            (echo "FAIL: VIGIL_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: VIGIL_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "hermes-bundled-locales" { } ''
          ...
          grep -q "VIGIL_BUNDLED_LOCALES" ${hermes-agent}/bin/hermes || \
            (echo "FAIL: VIGIL_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: VIGIL_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + VIGIL_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && VIGIL_BUNDLED_LOCALES=${hermes-agent}/share/hermes-agent/locales \
            ${hermesVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with VIGIL_BUNDLED_LOCALES set"; exit 1)
        ...
        # VIGIL_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "hermes-bundled-mcps" { } ''
          ...
          grep -q "VIGIL_OPTIONAL_MCPS" ${hermes-agent}/bin/hermes || \
            (echo "FAIL: VIGIL_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: VIGIL_OPTIONAL_MCPS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "hermes-bundled-tui" { } ''
          ...
          grep -q "VIGIL_TUI_DIR" ${hermes-agent}/bin/hermes || \
            (echo "FAIL: VIGIL_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: VIGIL_TUI_DIR set in wrapper"
        ...
        # Verify VIGIL_NODE is set in wrapper and points to Node 26+
        # (Vigil pins its toolchain to Node 26 everywhere)
        hermes-node = pkgs.runCommand "hermes-node-version" { } ''
          set -e
          echo "=== Checking VIGIL_NODE in wrapper ==="
          grep -q "VIGIL_NODE" ${hermes-agent}/bin/hermes || \
            (echo "FAIL: VIGIL_NODE not set in wrapper"; exit 1)
          echo "PASS: VIGIL_NODE present in wrapper"

          VIGIL_NODE=$(sed -n "s/^export VIGIL_NODE='\(.*\)'/\1/p" ${hermes-agent}/bin/hermes)
          test -x "$VIGIL_NODE" || (echo "FAIL: VIGIL_NODE=$VIGIL_NODE not executable"; exit 1)
          echo "PASS: VIGIL_NODE executable at $VIGIL_NODE"

          NODE_MAJOR=$("$VIGIL_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Hermes requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All VIGIL_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify VIGIL_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "hermes-managed-guard" { } ''
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(VIGIL_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking VIGIL_MANAGED guards ==="
