...
#
# Container mode: vigil runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $VIGIL_HOME/.env
# and read by vigil at startup — no container recreation needed for env changes.
#
# Tool resolution: the vigil wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    # terminal.cwd replaces the deprecated MESSAGING_CWD env var — vigil
    ...
    # VIGIL_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "hermes-documents" { } (
      ...
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'VIGIL_DOC_EOF'\n${value}\nVIGIL_DOC_EOF"
        ) cfg.documents
      )
    );
    ...
    # vigil user + sudo on first boot (writable layer persists), then
    ...
    containerEntrypoint = pkgs.writeShellScript "hermes-container-entrypoint" ''
      ...
      VIGIL_UID="''${VIGIL_UID:?VIGIL_UID must be set}"
      VIGIL_GID="''${VIGIL_GID:?VIGIL_GID must be set}"

      # ── Group: ensure a group with GID=$VIGIL_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$VIGIL_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="hermes"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$VIGIL_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$VIGIL_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$VIGIL_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$VIGIL_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        ...
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$VIGIL_UID" -g "$VIGIL_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$VIGIL_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$VIGIL_UID:$VIGIL_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure VIGIL_HOME is owned by the target user.
      ...
      if [ -n "''${VIGIL_HOME:-}" ] && [ -d "$VIGIL_HOME" ]; then
        find "$VIGIL_HOME" \! -user "$VIGIL_UID" -exec chown "$VIGIL_UID:$VIGIL_GID" {} +
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$VIGIL_UID" --regid="$VIGIL_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $VIGIL_HOME/.env.
  ...
  in {
    options.services.hermes-agent = with lib; {
      ...
      stateDir = mkOption {
        ...
        description = "State directory. Contains .hermes/ subdir (VIGIL_HOME).";
      };
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $VIGIL_HOME/.env at activation time.
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $VIGIL_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $VIGIL_HOME/mcp-tokens/.
              '';
            };
      ...
      addToSystemPackages = mkOption {
        ...
        description = ''
          ...
          VIGIL_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.vigil symlink to the service
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the vigil CLI to system PATH and export VIGIL_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.vigil/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.VIGIL_HOME = "${cfg.stateDir}/.hermes";
      })
      ...
      # Wire extraPackages into the vigil user's per-user profile so the
      ...
      # ── Activation: link config + auth + documents ────────────────────
      {
        system.activationScripts."hermes-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.hermes/.container-mode <<'VIGIL_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    hermes_bin=${containerDataDir}/current-package/bin/hermes
    VIGIL_CONTAINER_MODE_EOF
          ...
          # Create ~/.vigil -> stateDir/.vigil for each hostUser so the
          ...
          # Vigil reads $VIGIL_HOME/.env at startup via load_hermes_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.hermes/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'VIGIL_NIX_ENV_EOF'
    ${envFileContent}
    VIGIL_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.hermes-agent = {
          ...
          environment = {
            HOME = cfg.stateDir;
            VIGIL_HOME = "${cfg.stateDir}/.hermes";
            VIGIL_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $VIGIL_HOME/.env by the activation script. load_hermes_dotenv()
            ...
            # so interactive users in the vigil group can read/write them.
            UMask = "0007";
        ...
        systemd.services.hermes-agent = {
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              VIGIL_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              VIGIL_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env VIGIL_UID="$VIGIL_UID" \
                --env VIGIL_GID="$VIGIL_GID" \
                --env VIGIL_HOME=${containerDataDir}/.hermes \
                --env VIGIL_MANAGED=true \
