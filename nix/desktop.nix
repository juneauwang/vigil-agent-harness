# nix/desktop.nix — Vigil Desktop (Electron) app build + wrapper
#
# `hermesAgent` is the fully-built `.#default` package — it ships the
# `vigil` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `VIGIL_DESKTOP_HERMES` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Vigil CLI").
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    # VIGIL_DESKTOP_HERMES to the absolute path of the nix-built `vigil`
    # binary so the desktop's resolver step 4 ("existing Vigil CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/hermes-desktop \
      --add-flags "$out/share/hermes-desktop" \
      --set VIGIL_DESKTOP_HERMES "${lib.getExe hermesAgent}" \
