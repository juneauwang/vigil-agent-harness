...
| **监控** | 健康探测 + PromQL 查询 + 告警快照 + 告警人工处置 + 告警→runbook 处置建议（Web 监控页 / 告警页） | 拓扑实体健康探测（并发 + 缓存，不依赖 Prometheus 也能用），PromQL 兼容查询 + Alertmanager 告警汇总，连接配置可在 UI 里改了直接测；告警快照可逐条**确认 / 清除**（状态落本地库，不写回快照——采集层再报到也不会复活）；活跃告警自动匹配处置 SOP——建议给人，人批准后走受控执行链；自动派发（无人值守）的处置记录可在告警页回看（哪个 runbook、哪几步、结果与耗时逐条留痕） |
...
- **对话页**（`/chat`）：浏览器里和 Vigil 对话，流式回复、工具调用折叠展示、
  ...
  组词中不误发）；支持直接上传图片（截图丢给模型看图，走原生多模态，不经过 OCR
  预描述）；token 用量面板实时显示当前会话/累计费用
...
- **告警页**（`/incidents`）：告警快照列表（按 `alertname|instance` 去重保留最新），
  ...
  再次上报而复活，只有恢复后复发（`startsAt` 变化）才重新提醒；页内还可回看
  **自动派发的执行记录**（无人值守触发了哪个 runbook、执行了哪几步、结果与耗时）
