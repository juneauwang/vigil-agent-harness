"""ACP auth helpers — detect and advertise Vigil authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active Vigil runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Vigil can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Vigil.
    ...
    not have Vigil provider credentials configured yet, so Vigil always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Vigil using the currently configured "
