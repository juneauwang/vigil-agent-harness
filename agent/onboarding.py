...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Vigil starts and finds ``~/.openclaw/``.

    Points users at ``vigil claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``vigil claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Vigil, run "
        "`vigil claw migrate`.\n"
