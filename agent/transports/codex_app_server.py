...
projection into Vigil' display, approval bridging, transcript projection into
...
"codex_app_server"`. Vigil' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Vigil secret — gateway
        ...
        if spawn_env.get("VIGIL_KANBAN_TASK"):
            kanban_db = spawn_env.get("VIGIL_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "VIGIL_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("VIGIL_HOME", os.path.expanduser("~/.vigil")),
                        "kanban",
                    ),
                )
            )
