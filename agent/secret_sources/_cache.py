...
cache problem must never block Vigil startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Vigil home used for cache paths.

    ``home_path`` is whatever ``load_hermes_dotenv()`` already resolved;
    falling back to ``$VIGIL_HOME`` / ``~/.vigil`` keeps direct callers
...
class DiskCache(Generic[K]):
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if VIGIL_HOME is ever made traversable.
            try:
