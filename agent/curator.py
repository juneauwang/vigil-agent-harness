...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.vigil/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `vigil update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``vigil curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``vigil update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``vigil curator run`` (with or without
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Vigil' background skill CURATOR. This is an "
    ...
    "into ~/.vigil/skills/.archive/) is the maximum destructive action. "
    ...
    "~/.vigil/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.vigil/logs/curator/``)
    ...
    authored skill data in ``~/.vigil/skills/``.
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every vigil install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: vigil curator status
        keep an umbrella stable: vigil curator pin document-tools
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.vigil/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.vigil/skills/.archive/` for "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.vigil/skills/.archive/`. "
    ...
    lines.append("- All archives live under `~/.vigil/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``vigil curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `vigil curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `vigil curator status` can point at it.
        try:
            after_report = skill_usage.curated_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``vigil model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `vigil model` → auxiliary
