...
that instructs the live agent to:
  ...
  1. Gather the sources the user named, using the tools it already has
     ...
     material).
  2. Author a single ``SKILL.md`` via ``skill_manage`` that follows the Vigil
...
Follow the Vigil skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Vigil`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Vigil.
...
4. "## How to Run" — the canonical invocation, framed through Vigil tools.
...
- Reference Vigil tools by name in backticks: `terminal`, `read_file`,
