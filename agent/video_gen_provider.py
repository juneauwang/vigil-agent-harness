...
as ``kind: backend``) or ``~/.vigil/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$VIGIL_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$VIGIL_HOME/cache/videos/``.
