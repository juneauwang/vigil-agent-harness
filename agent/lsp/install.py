...
``<VIGIL_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``vigil lsp
  status``.
...
# ``<VIGIL_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `vigil lsp status` reports the host's presence.
...
def _install_npm(
    ...
    """
    # Managed npm first: $VIGIL_HOME/node is not on an arbitrary process's
    # PATH, so a bare which() misses the Node that Vigil installed and
    ...
    if npm is None:
        ...
        return None
    staging = hermes_lsp_bin_dir().parent  # <VIGIL_HOME>/lsp/
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``vigil lsp status`` CLI to give users a quick
