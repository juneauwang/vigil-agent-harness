...
# un-throttled log fires on *every* selection. On Windows several Vigil
...
# readiness handshake ("Timed out connecting to Vigil backend after
...
def _write_through_provider_state_to_global_root(
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".vigil" / "auth.json"
            try:
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``vigil model`` / ``vigil auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Vigil process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``vigil`` invocation that defaults to the active provider
        (e.g. setup wizard, ``vigil auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Vigil
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``VIGIL_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "VIGIL_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "VIGIL_XAI_REFRESH_TIMEOUT_SECONDS"
        )
    ...
    def _refresh_entry_impl(
        ...
        try:
            if self.provider == "anthropic":
                ...
                # refresh_token — single-use tokens consumed by another Vigil
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Vigil process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Vigil profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `vigil model` / `vigil auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `vigil model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``vigil auth add``.  Singleton-
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `vigil auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Vigil PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `vigil setup` ("Claude
        ...
        # that masquerade.  Prefer ~/.vigil/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `vigil setup` writes.
        _env_file = load_env()
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `vigil auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (vigil auth remove copilot gh_cli) must not pay either on
        ...
        # Vigil auth store or env vars, so resolve them here.
        ...
        # MiniMax OAuth tokens live in ~/.vigil/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `vigil auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `vigil auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Vigil auth store.  Without this gate the removal is instantly
        ...
        # Vigil owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``vigil model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``vigil auth list`` reflects the logged-in state and so the pool
...
# Prefer ~/.vigil/.env over os.environ — the user's config file is the
# authoritative source for Vigil credentials. Stale env vars from parent
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `vigil auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.vigil/.env.
    ...
    if provider == "openrouter":
        # Prefer ~/.vigil/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `vigil auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Vigil
