...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from hermes_cli.providers import VIGIL_OVERLAYS, get_provider

        overlay = VIGIL_OVERLAYS.get(provider)
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$VIGIL_HOME`` (which is profile-scoped) alongside the other
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$VIGIL_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .hermes.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / VIGIL_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.vigil/SOUL.md as the primary
    ...
    # report cumulative micros spent.  Surfaced behind VIGIL_DEV_CREDITS.
    ...
    # both live under ~/.vigil/logs/.  Idempotent, so gateway mode
    ...
    # worker (kanban_show tool is present iff VIGIL_KANBAN_TASK is set).
    ...
    # Session logs go into ~/.vigil/sessions/ alongside gateway sessions
    hermes_home = get_hermes_home()
    agent.logs_dir = hermes_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.vigil/sessions/session_{sid}.json)
    ...
    # `vigil --resume <id>` can restore the bypass (CLI resume paths read
    ...
    # management API (the historical Vigil behavior) or left to LM Studio's
    ...
    # Lets an enterprise admin append to or replace Vigil' built-in
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Vigil Agent.  Choose a model with at least "
    ...
    # Nous Vigil 3/4 are chat models, not tool-call-tuned. The interactive
