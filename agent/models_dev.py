...
Data resolution order:
  ...
  2. Disk cache (~/.vigil/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Vigil ↔ models.dev
# ---------------------------------------------------------------------------

# Vigil provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Vigil (built lazily)
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``vigil config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Vigil provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Vigil agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Vigil provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Vigil ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Vigil or models.dev provider ID.  Tries exact match then
