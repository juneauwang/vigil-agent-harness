...
#
# When a user invokes a /skill (or /bundle), Vigil expands the turn into a
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``VIGIL_PLATFORM`` env var and
    ``VIGIL_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("VIGIL_PLATFORM")
        ...
        for scan_dir in dirs_to_scan:
            for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
                ...
                try:
                    ...
                    # with a core Vigil slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core Vigil command; skipping "
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``vigil -s <skill>`` or
    a deployment's ``VIGIL_TUI_SKILLS`` env var could force-load a skill an
