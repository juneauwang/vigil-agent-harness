...
def build_billing_state(*, timeout: float = 15.0) -> BillingState:
    ...
    Dev override: ``VIGIL_DEV_BILLING_FIXTURE`` short-circuits to a fixture so the
    card-on-file / admin / scope states are testable offline (mirrors
    ``VIGIL_DEV_CREDITS_FIXTURE`` for the usage model).
    """
...
def _dev_fixture_billing_state() -> Optional[BillingState]:
    """Map ``VIGIL_DEV_BILLING_FIXTURE`` to a :class:`BillingState` for offline UX.
    ...
    Mirrors ``VIGIL_DEV_CREDITS_FIXTURE``; the usage *bar* still comes from
    ``VIGIL_DEV_CREDITS_FIXTURE`` (set both to pair a bar with a billing state).
    """
    name = (os.getenv("VIGIL_DEV_BILLING_FIXTURE") or "").strip().lower()
    if not name:
        return None
    ...
    return BillingState(logged_in=False, error=f"unknown VIGIL_DEV_BILLING_FIXTURE: {name}")
