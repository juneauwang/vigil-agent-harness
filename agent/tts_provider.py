...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Vigil with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.vigil/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Vigil
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``vigil tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``vigil setup`` for
