...
* First-use consent is gated by the allowlist under
  ``~/.vigil/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``VIGIL_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
...
def register_from_config(
    ...
    *,
    ...
    setting.  ``VIGIL_ACCEPT_HOOKS=1`` and ``hooks_auto_accept: true`` are
    ...
    # Safe mode (--safe-mode / VIGIL_SAFE_MODE=1): shell hooks are user
    ...
    if env_var_enabled("VIGIL_SAFE_MODE"):
        logger.info("VIGIL_SAFE_MODE=1 — shell-hook registration skipped")
        return []
        ...
        if not already_allowlisted:
            if not _prompt_and_record(
                ...
                logger.warning(
                    ...
                    "Use --accept-hooks / VIGIL_ACCEPT_HOOKS=1 / "
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``vigil hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Vigil wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Vigil
    ...
    if event == "pre_verify":
        # "continue" (Vigil) / "block" (Claude-Code Stop: block the stop) both
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / VIGIL_ACCEPT_HOOKS).",
            p, exc,
        )
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Vigil is about to register a shell hook that will run a\n"
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``VIGIL_ACCEPT_HOOKS`` env var
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = os.environ.get("VIGIL_ACCEPT_HOOKS", "").strip().lower()
...
# ---------------------------------------------------------------------------
# Introspection (used by `vigil hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``vigil hooks test`` and ``vigil hooks doctor``.
    ...
    produce — otherwise scripts tested via ``vigil hooks test`` could
