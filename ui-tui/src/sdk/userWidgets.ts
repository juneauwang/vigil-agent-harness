...
/**
 * User widget apps — Vigil authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$VIGIL_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.vigil/plugins/`: files under VIGIL_HOME execute
...
const widgetsDir = () => join(process.env.VIGIL_HOME?.trim() || join(homedir(), '.hermes'), 'tui-widgets')
 ...
 *  every change, so a widget Vigil writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Vigil) ever writes must hot-load too — a 10s poll here read
