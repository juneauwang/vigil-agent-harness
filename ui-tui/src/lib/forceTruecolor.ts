...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.VIGIL_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Vigil on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via VIGIL_TUI_TRUECOLOR=1.
