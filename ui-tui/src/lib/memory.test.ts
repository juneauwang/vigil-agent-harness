...
const ENV_KEYS = ['VIGIL_AUTO_HEAPDUMP', 'VIGIL_HEAPDUMP_DIR', 'VIGIL_HEAPDUMP_MAX_BYTES'] as const
...
describe('performHeapDump auto opt-in gate (#21767)', () => {
  ...
  it('writes diagnostics only for auto-high without VIGIL_AUTO_HEAPDUMP', async () => {
  ...
  it('writes diagnostics only for auto-critical without VIGIL_AUTO_HEAPDUMP', async () => {
  ...
  it('writes both diagnostics and snapshot for auto-high when VIGIL_AUTO_HEAPDUMP=1', async () => {
    process.env.VIGIL_AUTO_HEAPDUMP = '1'
  ...
  it('accepts truthy spellings (true|yes|on, case-insensitive) as opt-in', async () => {
    for (const value of ['true', 'YES', 'On']) {
      process.env.VIGIL_AUTO_HEAPDUMP = value
      const result = await performHeapDump('auto-high')
  ...
  it('writes both for manual triggers regardless of VIGIL_AUTO_HEAPDUMP', async () => {
...
describe('heapdump retention guard (#21767)', () => {
  ...
  afterEach(() => {
    if (savedDir === undefined) {
      delete process.env.VIGIL_HEAPDUMP_DIR
    } else {
      process.env.VIGIL_HEAPDUMP_DIR = savedDir
    }

    if (savedMax === undefined) {
      delete process.env.VIGIL_HEAPDUMP_MAX_BYTES
    } else {
      process.env.VIGIL_HEAPDUMP_MAX_BYTES = savedMax
    }
  ...
  it('evicts oldest files when total bytes exceed the cap, retaining the newest', async () => {
    ...
    process.env.VIGIL_HEAPDUMP_MAX_BYTES = String(2 * 1024)
