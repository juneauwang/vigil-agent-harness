...
// Profile-aware: the Python launcher exports VIGIL_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.vigil matches get_hermes_home()'s default.
const bootFilePath = () => join(process.env.VIGIL_HOME ?? join(homedir(), '.hermes'), 'tui-theme-boot.json')

// Never touch the user's real ~/.vigil from test runs (the TS suite has no
// VIGIL_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into VIGIL_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.VIGIL_TUI_THEME || env.VIGIL_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.VIGIL_TUI_BACKGROUND
  ) {
    env.VIGIL_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.VIGIL_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.VIGIL_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into VIGIL_TUI_THEME.
