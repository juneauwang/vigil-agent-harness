...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $VIGIL_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see VIGIL_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['VIGIL_TUI_BACKGROUND', process.env.VIGIL_TUI_BACKGROUND ?? '(unset)'],
            ['VIGIL_TUI_THEME', process.env.VIGIL_TUI_THEME ?? '(unset)'],
