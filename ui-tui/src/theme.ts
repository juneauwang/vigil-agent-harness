...
const BRAND: ThemeBrand = {
  ...
  icon: '◉',
  prompt: '❯',
  welcome: '欢迎使用 Vigil——记住整个平台，安全地动生产。输入消息或 /help 查看命令。',
  goodbye: 'Goodbye! ◉',
  tool: '┊',
  helpHeader: '(◉) Commands'
}
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic Vigil slate surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode Vigil look was never hand-authored: for years the
 ...
 *  answer when known (cached in VIGIL_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.VIGIL_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit VIGIL_TUI_THEME / COLORFGBG signals above still win,
...
// `VIGIL_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `VIGIL_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `VIGIL_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `VIGIL_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Vigil palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.VIGIL_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.VIGIL_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.VIGIL_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into VIGIL_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into VIGIL_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Vigil slate
  ...
  // the derived value already carries the identity seeds (e.g. Vigil slate).
  const activeRow =
