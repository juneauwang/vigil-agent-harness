#!/usr/bin/env python3
"""
SQLite State Store for Vigil Agent.
...
def _workspace_key_clause(key: str) -> Tuple[str, List[str]]:
    ...
    Used by ``vigil -c``/``--resume`` to continue the most recent session in
...
def _default_db_path() -> Path:
    ...
    freezes the developer's real ``~/.vigil`` even when a test fixture later
    redirects ``VIGIL_HOME`` — importing this module during collection was
    ...
    2. Otherwise resolve ``get_hermes_home()`` fresh so a runtime
       ``VIGIL_HOME`` redirect takes effect regardless of import order.
    """
...
def _wal_reset_repair_hint() -> str:
    ...
    ``vigil update`` can actually do for this install (#75153).
    """
    try:
    ...
    return (
        ...
        "(or backports 3.50.7 / 3.44.6) and restart Vigil"
    )
...
def preflight_db_writability(
    ...
    *,
    ...
    - **Repairs** permissions with ``chmod u+rw`` when the file lives inside
      the Vigil home tree (``get_hermes_home()``) — the safe repair scope:
      Vigil owns those files, and the OS makes ``chmod`` fail on files the
    ...
    def _ensure_writable(p: Path, *, is_dir: bool = False) -> None:
        ...
        raise sqlite3.OperationalError(
            ...
            f"user. Vigil needs read-write access to open the database. "
...
# (``~/.vigil/lib/libfts5_cjk.so``, built by ``native/fts5_cjk/build.sh``).
...
# next ``vigil sessions optimize-storage`` on a capable host).
#
...
def fts5_cjk_so_path() -> Path:
    ...
    env = os.getenv("VIGIL_FTS5_CJK_SO")
    if env:
...
def _cjk_fts_config_enabled() -> bool:
    ...
    return os.getenv("VIGIL_CJK_FTS", "1").strip().lower() not in (
        "0", "false", "off", "no",
    )
...
class SessionDB(SessionSearchMixin, SessionSchemaMixin, SessionPortabilityMixin):
    ...
    # With multiple vigil processes (gateway + CLI sessions + worktree agents)
    ...
    # legitimately held for multi-second stretches by sibling Vigil
    ...
    # `vigil update` leaves mixed-version processes sharing the DB until
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # NOTE: the v23 FTS optimization is OPT-IN (`vigil db optimize`),
        ...
        while True:
            try:
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    raise sqlite3.OperationalError(
                        f"database is locked (another Vigil process held the "
    ...
    #
    # `optimize_fts_storage()` (the `vigil sessions optimize-storage`
    ...
    # ── Opt-in v23 FTS storage optimization (`vigil sessions optimize-storage`) ──
    #
    ...
    def set_session_yolo(self, session_id: str, enabled: bool) -> None:
        ...
        ``--yolo`` launch — survives ``vigil --resume`` into a fresh
    ...
    def search_sessions(
        ...
        ``vigil -c``/``--resume`` so the "last" session is the last one in
    ...
    def delete_session_if_empty(
        ...
        and ``vigil sessions list`` output. (Pattern ported from
    ...
    def retag_kanban_worker_sessions(self, workspaces_root: str) -> int:
        ...
        Workers used to spawn without ``VIGIL_SESSION_SOURCE``, so their runs
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Vigil, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Vigil session.

        A Vigil session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Vigil session is already bound to any Telegram DM topic.
