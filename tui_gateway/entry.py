...
# ``vigil --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    # session profile's VIGIL_HOME first), so a launch profile with no
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``vigil --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``vigil dashboard`` spawn theirs via
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's VIGIL_HOME override — the shared owner in
...
def main():
    ...
    # Live-apply skins Vigil activates mid-conversation.
