...
def _(rid, params: dict) -> dict:
    try:
        ...
        api_key = get_secret("VIGIL_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("VIGIL_BASE_URL", "") or cfg.get("base_url", "")
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``vigil journey`` CLI.
    """
    try:
