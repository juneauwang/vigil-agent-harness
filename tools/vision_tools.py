...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("VIGIL_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: VIGIL_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = os.getenv("VIGIL_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $VIGIL_HOME/cache/vision/ and cleaned up
