...
it is automatically downloaded from GitHub releases to $VIGIL_HOME/bin/tirith.
...
def _get_hermes_home() -> str:
    """Return the Vigil home directory, respecting VIGIL_HOME env var."""
...
def _hermes_bin_dir() -> str:
    """Return $VIGIL_HOME/bin, creating it if needed."""
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $VIGIL_HOME/bin/tirith.
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $VIGIL_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $VIGIL_HOME/bin/tirith
...
def tirith_install_status() -> str:
    ...
    Values:
      ...
      "installed"         — binary resolved on PATH or in $VIGIL_HOME/bin
