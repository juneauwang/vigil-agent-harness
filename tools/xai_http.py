...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``vigil tools`` repaint, tool-registration scans,
    ...
    2. ``~/.vigil/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``vigil auth add xai-oauth``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.vigil/.env`` first, then ``os.environ``.
...
def hermes_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Vigil Agent,
...
def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Vigil config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Vigil home, then mark it seen."""
...
def resolve_xai_http_credentials(
    *,
    ...
    stored in ``~/.vigil/.env`` (the standard Vigil location) are honored —
    ...
    try:
        ...
        override_base_url = str(
            get_env_value("VIGIL_XAI_BASE_URL")
