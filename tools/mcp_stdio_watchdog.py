...
<url>``) is spawned as a direct child of the Vigil process. Vigil's own
...
Vigil process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Vigil startup's opt-in
