...
def _get_active_profile_name() -> str:
    """Return the active Vigil profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Vigil
    process leaves its container behind permanently because there's no
    subsequent Vigil process scheduled to reuse that exact (task, profile)
    pair.
    """
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``VIGIL_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("VIGIL_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using VIGIL_DOCKER_BINARY override: %s", override)
        return override
...
def _egress_proxy_args_for_docker() -> tuple[list[str], dict[str, str], list[str]]:
    ...
    * ``env_overrides`` — env vars to set on container creation: ``HTTPS_PROXY``,
      ...
      paths, and one ``VIGIL_PROXY_TOKEN_<NAME>`` per minted mapping
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    if not status.configured:
        msg = (
            ...
            "Run `vigil egress setup` to mint tokens and write proxy.yaml."
        )
        if enforce:
    ...
    if not (status.pid and status.listening):
        msg = (
            ...
            "Start it with `vigil egress start`."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        msg = (
            ...
            "Re-run `vigil egress setup` to regenerate it."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if not mappings:
        msg = (
            ...
            "corrupt.  Re-run `vigil egress setup` to mint provider "
    ...
    # through the proxy.  Keep the VIGIL_PROXY_TOKEN_* aliases for
    # diagnostics.
    for m in mappings:
        env_overrides[m.real_env_name] = m.proxy_token
        env_overrides[f"VIGIL_PROXY_TOKEN_{m.real_env_name}"] = m.proxy_token
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.vigil/sandboxes/). Non-persistent
        ...
        # ``_VIGIL_EGRESS_NODE_OPTIONS_APPEND`` and merge here.
        _egress_node_append = merged_env.pop(
            "_VIGIL_EGRESS_NODE_OPTIONS_APPEND", None,
        )
        ...
        # container shared across sessions").  If a prior Vigil process
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        forward_keys = explicit_forward_keys | (_implicit_forward - _VIGIL_PROVIDER_ENV_BLOCKLIST)
    ...
    def _find_reusable_container(
        ...
        try:
            ...
            if egress_label != "off":
                ...
                # reused after the operator runs "vigil egress disable",
        ...
        # container) but can happen if a previous Vigil process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Vigil exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Vigil process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Vigil startup. That covers the
        ...
        # outlives Vigil processes, processes inside it stay alive, and
