...
def _action_result_from(
    ...
    *,
    ...
    cua-driver's mcp-tool-notes and juneauwang/vigil-agent-harness#67052.
    """
...
# fetches the latest release, so a `VIGIL_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `VIGIL_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD_ENV = "VIGIL_CUA_DRIVER_CMD"
...
def _cua_no_overlay() -> bool:
    """True when Vigil should pass ``--no-overlay`` to cua-driver.
...
def _cua_telemetry_disabled() -> bool:
    """True when Vigil should disable cua-driver telemetry for this user.
...
def _wsl_windows_path_to_posix(path: str) -> str:
    """Translate a Windows absolute manifest command when Vigil runs in WSL.

    Windows cua-driver manifests can report ``C:\\Users\\...\\cua-driver.exe``
    even though the Vigil process uses POSIX subprocess spawning inside WSL.
...
class _EmbeddedCuaDaemon:
    ...
    machine-wide daemon would therefore let one Vigil session's YOLO choice
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of juneauwang/vigil-agent-harness#47072: instead of hardcoding
    ...
    # A Windows-installed cua-driver can hand a WSL-hosted Vigil an absolute
...
def _candidate_cua_driver_commands(override: Optional[str] = None) -> List[str]:
    ...
    ``VIGIL_CUA_DRIVER_CMD`` is authoritative; only when neither is set do we
    ...
    Vigil Desktop/TUI session can otherwise filter out the `computer_use`
    tool even though `vigil computer-use doctor` succeeds from a login shell.
    """
...
def resolve_cua_driver_cmd(override: Optional[str] = None) -> Optional[str]:
    ...
    A supplied override (or ``VIGIL_CUA_DRIVER_CMD``) is never silently
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of juneauwang/vigil-agent-harness#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(
        ...
        self._started = False
        # Surface 4 of juneauwang/vigil-agent-harness#47072: per-tool
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    juneauwang/vigil-agent-harness#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self, permission_mode: str = "standard") -> None:
        ...
        # Surface 6 of juneauwang/vigil-agent-harness#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per Vigil run, with
        ...
        #     concurrent Vigil runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Vigil run = one backend = one session — and pass it as
    ...
    def start(self) -> None:
        ...
        # `computer-use` / `mcp` extras), not part of Vigil' minimal core.
    ...
    def capture(
        ...
        # Surface 3 of juneauwang/vigil-agent-harness#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of juneauwang/vigil-agent-harness#47072: prefer the
    ...
    def _apply_delivery(
        ...
        to proceed. See juneauwang/vigil-agent-harness#67052 phase B.
        """
    ...
    def click(
        ...
        *,
        ...
        # juneauwang/vigil-agent-harness#47072). cua-driver-rs gained an explicit
    ...
    def focus_app(self, app: str, raise_window: bool = False) -> ActionResult:
        ...
        separately approved by the Vigil adapter, and uses cua-driver's
