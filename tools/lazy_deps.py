"""
Lazy dependency installer for opt-in Vigil Agent backends.

Many Vigil features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
* **Durable-target mode (immutable images).** When the deployment seals the
  ...
  installed package cannot brick Vigil, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (VIGIL_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    # HF Agent Trace Viewer upload (vigil trace upload / /upload-trace).
    #
    ...
    # presence, the `vigil update` lazy-refresh pass re-asserts THIS pin on
...
_LAZY_TARGET_ENV = "VIGIL_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``VIGIL_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("VIGIL_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``vigil update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Vigil under WSL to use Matrix on Windows."
        )
    return None
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    try:
        ...
        # which() misses the uv Vigil installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Vigil
    ...
    # deployment sets VIGIL_MANAGED=true *and* VIGIL_LAZY_INSTALL_TARGET
    ...
    if _lazy_install_target() is None:
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Vigil cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Vigil instead)."
            )
...
def install_specs(specs: list[str] | tuple[str, ...], *, timeout: int = 300) -> InstallSpecsResult:
    ...
    * **Durable-target on immutable images** — when the deployment seals the
      agent venv (``VIGIL_DISABLE_LAZY_INSTALLS=1``) and sets
      ``VIGIL_LAZY_INSTALL_TARGET``, installs are redirected to the writable
