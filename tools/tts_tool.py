...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.vigil/config.yaml``. Vigil
...
Configuration is loaded from ~/.vigil/config.yaml under the 'tts:' key.
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``vigil auth add <provider_id>``).
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.vigil/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.vigil/config.yaml.
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``vigil tools``); otherwise the historical Edge backend remains active.
    """
...
# custom voice-cloning scripts, etc.) into Vigil without any Python code
...
#           output_format: wav
#
# Vigil writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__VIGIL_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Vigil secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of Vigil secrets (salvage of #56332) while still
...
# file, so retired models disappear from vigil the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `vigil setup` to configure, "
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when Vigil actually needs a non-default format/override.
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        ...
        # Include Vigil client context following Gemini's partner
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Vigil caches Piper voice models.
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.vigil/config.yaml (tts: section).
    ...
    platform = get_session_env("VIGIL_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `vigil setup` to install Mistral support."
                }, ensure_ascii=False)
