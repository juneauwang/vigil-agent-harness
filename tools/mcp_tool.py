...
Configuration is read from ~/.vigil/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.vigil/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Vigil' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Vigil explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Vigil and
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = str(get_process_hermes_home())
            candidates = [
                ...
                # image (which the Vigil Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Vigil MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Vigil gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Vigil document cache; returns a marker with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Vigil' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (VIGIL_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit VIGIL_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Vigil' logging (agent.log via hermes_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "it is not installed. Run `vigil setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this Vigil process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Vigil' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Vigil is fully
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Vigil processes (e.g.
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local VIGIL_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local VIGIL_HOME override onto the MCP loop.
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Vigil config file.
    ...
    try:
        ...
        if _env_enabled("VIGIL_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Vigil' MEDIA: tag convention and out to messaging
            ...
            # Vigil' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Vigil's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Vigil uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Vigil registry schema format.
