...
def _make_hermes_provider_class() -> Optional[type]:
    ...
    class HermesMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``vigil mcp reauth``.
...
_VIGIL_PROVIDER_CLS: Optional[type] = _make_hermes_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _VIGIL_PROVIDER_CLS is None:
            logger.warning(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `vigil mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``vigil mcp remove <name>`` and (indirectly) by
        ``vigil mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
