...
409), and is driven by:
  ...
  * the ``vigil sync status|pull|push|now`` CLI.
...
is toggled via ``vigil sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.vigil/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``vigil sync enable`` — it is
...
# sync.base_url (or the VIGIL_SYNC_BASE_URL bridge env) overrides it to point
...
def resolve_sync_base_url() -> Optional[str]:
    ...
    Order: VIGIL_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("VIGIL_SYNC_BASE_URL")
...
# ---------------------------------------------------------------------------
# Sync feature configuration — env-first, so a Vigil Cloud instance can be set
...
# precedence as base_url: the VIGIL_SYNC_* env var wins, else config.yaml
# ``sync.*``, else a built-in default.
#
#   VIGIL_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   VIGIL_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   VIGIL_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    ``VIGIL_SYNC_ENABLED`` -> ``sync.enabled`` -> False. This is the master
    switch a Vigil Cloud deployment sets to opt its instances into sync by
    ...
    """
    return _sync_config_bool("VIGIL_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``VIGIL_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``vigil sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "VIGIL_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``VIGIL_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``vigil sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    Vigil Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("VIGIL_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.vigil/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.vigil/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Vigil Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``VIGIL_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.vigil/skills/ containing
...
def stable_device_id() -> str:
    ...
    Use ``set_device_name()`` / ``vigil sync device --name`` to set an explicit
    label."""
    ...
    # Vigil Cloud (and any templated deployment) can seed the label
    # declaratively via VIGIL_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `vigil sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("VIGIL_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.vigil/skills/.sync_device_id, overwriting
...
# ~/.vigil/skills/.sync_state as JSON.
#
...
# path under ~/.vigil/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.vigil/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Vigil speaks "
            f"{WIRE_VERSION} — update Vigil to sync with it"
        )
...
def pull_skills(
    ...
    *,
    ...
    ~/.vigil/skills/. Only paths the user has opted into (``sync: true``) are
...
def maybe_push_skills(*, message: str = "vigil skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not sync_feature_enabled():
            return None  # feature off for this instance (VIGIL_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``vigil sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
#
# Org skills live under a DISTINCT local namespace, ~/.vigil/skills/_org/
...
#
# `vigil sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.vigil/skills/_org/<org_id>/``.
