...
Lets the LLM write a Python script that calls Vigil tools via RPC,
...
# available on every platform Vigil itself runs on.
...
# match a safe prefix, the operational VIGIL_ allowlist, or (on Windows) an
# OS-essential name.  Delegate-task child context is also an exact-name
# operational marker: without it, a sandbox script that spawns/imports Vigil
# code can lose the DB-layer Kanban mutation guard while still inheriting
# VIGIL_HOME.
#
# NB: the broad "VIGIL_" prefix was deliberately removed (#27303) — it leaked
# VIGIL_*-named config that lacks a secret substring (e.g. VIGIL_BASE_URL,
# VIGIL_KANBAN_DB, VIGIL_*_WEBHOOK).  The child only needs the few
# location/profile vars in _VIGIL_CHILD_ALLOWED below; VIGIL_RPC_SOCKET /
# VIGIL_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational VIGIL_* vars the child legitimately needs by exact name — these
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational VIGIL_* vars (_VIGIL_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret VIGIL_* vars dropped by the tightened allowlist (#27303). The
    # broad "VIGIL_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like VIGIL_KANBAN_DB / VIGIL_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if k.startswith("VIGIL_"):
            ...
            # allowlist — a deliberately-dropped VIGIL_* var.
            _dropped_hermes.append(k)
    if _dropped_hermes:
        logger.debug(
            "execute_code: dropped %d non-allowlisted VIGIL_* var(s) from the "
...
"""Auto-generated Vigil tools RPC stubs."""
...
def _connect():
    ...
    VIGIL_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["VIGIL_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("VIGIL_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated Vigil tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("VIGIL_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hermes_rpc")
_seq = 0
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            f"VIGIL_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"VIGIL_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("VIGIL_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # selector from VIGIL_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    try:
        ...
        #   connect.  VIGIL_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["VIGIL_RPC_SOCKET"] = rpc_endpoint
        child_env["VIGIL_RPC_TOKEN"] = rpc_token
        ...
        # VIGIL_TIMEZONE is an internal Vigil setting and must not leak
        # into child processes.
        _tz_name = os.getenv("VIGIL_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("VIGIL_TIMEZONE", None)
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.vigil/.env')).
...
def _load_config() -> dict:
    ...
    ``vigil --tui`` where it is never used.  Read the lightweight raw config
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``vigil tools`` (e.g. web is turned off),
    ...
    if mode == "strict":
        cwd_note = (
            ...
            "(os.path.expanduser('~/.vigil/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that calls Vigil tools programmatically. "
