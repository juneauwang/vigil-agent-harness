...
Tracks per-skill usage metadata in a sidecar JSON file (~/.vigil/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.vigil/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.vigil/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def list_agent_created_skill_names() -> List[str]:
    ...
    for skill_md in base.rglob("SKILL.md"):
        # Skip Vigil metadata, VCS, virtualenv/dependency, and cache dirs
...
def _is_curator_managed_record(record: Any) -> bool:
    ...
    * management = "may autonomous curation mutate/archive this" — a policy
      decision the user can change at any time via ``vigil curator adopt``.
...
def list_unmanaged_skill_names() -> List[str]:
    ...
    * It was created by a FOREGROUND ``skill_manage(action="create")`` call,
    ...
    every automatic transition. ``vigil curator status`` surfaces this count
    so the blind spot is legible instead of silent, and ``vigil curator
...
def adopt_skill(skill_name: str) -> Tuple[bool, str]:
    ...
    if is_bundled(skill_name):
        ...
        # claim Vigil' own shipped skill was agent-authored and change nothing
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move a curator-eligible skill directory to ~/.vigil/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.vigil/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.vigil/skills/<skill>/SKILL.md) and category-nested
    (~/.vigil/skills/<category>/<skill>/SKILL.md) layouts. Uses the gated
