...
def _media_caption_split(text, media_files, *, max_caption_len):
    ...
    across every standalone sender. ``vigil send`` (and the send_message
    ...
    ``vigil send --to whatsapp "MEDIA:/x.png This Caption"`` into two parts.
    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.vigil/config.yaml or environment variables.")
        else:
            return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.vigil/config.yaml or environment variables.")
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("success") and mirror_text:
            try:
                ...
                source_label = get_session_env("VIGIL_SESSION_PLATFORM", "cli")
...
def _get_cron_auto_delivery_target():
    ...
    from gateway.session_context import get_session_env
    platform = get_session_env("VIGIL_CRON_AUTO_DELIVER_PLATFORM", "").strip().lower()
    chat_id = get_session_env("VIGIL_CRON_AUTO_DELIVER_CHAT_ID", "").strip()
    if not platform or not chat_id:
        return None
    thread_id = get_session_env("VIGIL_CRON_AUTO_DELIVER_THREAD_ID", "").strip() or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
...
async def _send_matrix_via_adapter(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    is running (standalone cron, ``vigil send`` CLI).
    """
    media_files = media_files or []
...
def _check_send_message():
    ...
    Also passes for kanban workers — the dispatcher sets ``VIGIL_KANBAN_TASK``
    on every spawned worker, but those workers run with the assignee profile's
    ``VIGIL_HOME`` which has no ``gateway.pid``, so the gateway-running check
...
#   - the ``vigil send`` CLI command (hermes_cli/send_cmd.py)
