...
_VIGIL_CORE_TOOLS = [
    ...
    # pane, and react to a message with an emoji (all gated on VIGIL_DESKTOP
    ...
    # spawned as a kanban worker (VIGIL_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (VIGIL_KANBAN_TASK env "
    ...
    # Coding posture (base Vigil — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    # ==========================================================================
    # Full Vigil toolsets (CLI + messaging platforms)
    #
        ...
        if include_registry and name.startswith("hermes-"):
            ...
            try:
                ...
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_VIGIL_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
