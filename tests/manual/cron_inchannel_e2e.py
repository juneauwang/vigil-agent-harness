...
Uses a throwaway VIGIL_HOME so it never touches ~/.vigil. No real names.
"""
...
def _fresh_home():
    """Point VIGIL_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_hermes_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["VIGIL_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER VIGIL_HOME is set.
...
def main():
    ...
    print(f"VIGIL_HOME (throwaway): {HOME}")
