...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape VIGIL_HOME."""
    ...
    def test_nested_subdir_inside_hermes_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of VIGIL_HOME must be allowed."""
        hermes_home = tmp_path / ".vigil"
        hermes_home.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside VIGIL_HOME pointing outside must be rejected."""
        hermes_home = tmp_path / ".vigil"
        hermes_home.mkdir()
        monkeypatch.setenv("VIGIL_HOME", str(hermes_home))
        ...
        # The resolved path escapes VIGIL_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape VIGIL_HOME."""
        hermes_home = tmp_path / ".vigil"
        hermes_home.mkdir()
        monkeypatch.setenv("VIGIL_HOME", str(hermes_home))
    ...
    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside VIGIL_HOME via config must still mount."""
        hermes_home = tmp_path / ".vigil"
        hermes_home.mkdir()
        monkeypatch.setenv("VIGIL_HOME", str(hermes_home))
...
class TestCacheDirectoryMounts:
    ...
    def test_images_upload_dir_is_mounted(self, tmp_path, monkeypatch):
        ...
        Desktop / clipboard / PDF uploads land in ``VIGIL_HOME/images``, not
...
class TestMasterCredentialStoresAreNeverMountable:
    """Containment is not enough — VIGIL_HOME *is* where the keys live.
    ...
    ``../../.ssh/id_rsa`` from escaping VIGIL_HOME, but every master
