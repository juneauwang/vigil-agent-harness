...
class TestSmartApproval:
    ...
    def test_smart_approval_does_not_allowlist_the_pattern_for_session(self, monkeypatch):
        ...
        monkeypatch.setenv("VIGIL_SESSION_KEY", session_key)
        monkeypatch.setenv("VIGIL_EXEC_ASK", "1")
        monkeypatch.delenv("VIGIL_CRON_SESSION", raising=False)
        monkeypatch.setattr(
            approval_module,
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"VIGIL_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_to_sensitive_target(self):
        for command in (
            ...
            "echo x | tee /dev/sda",
            "echo x | tee ~/.vigil/.env",
            "echo x | tee $VIGIL_HOME/.env",
            'echo x | tee "$VIGIL_HOME/.env"',
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
...
class TestHermesConfigWriteProtection:
    ...
    ~/.vigil/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_write_idioms_against_config(self):
        for command in (
            "echo 'approvals:' > ~/.vigil/config.yaml",
            "echo '  mode: off' >> ~/.vigil/config.yaml",
            "echo x | tee ~/.vigil/config.yaml",
            "echo x | tee $VIGIL_HOME/config.yaml",
            "cp /tmp/evil.yaml ~/.vigil/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(command)
            assert dangerous is True, command
            assert key is not None, command
    ...
    def test_reads_and_unrelated_writes_are_safe(self):
        # Reading config is not a write; a non-Vigil absolute config.yaml is
        # handled by the project patterns, not the Hermes-home rule.
        for cmd in (
            "cat ~/.vigil/config.yaml",
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.vigil/config.yaml/.env must require approval — the
...
class TestWindowsAbsolutePathFolding:
    ...
    ~/.vigil/ in dangerous-command detection.
    ...
    SSH, and Vigil config/env files returned "safe" without an approval prompt.
    ...
    HOME/VIGIL_HOME so the fold is verified on the POSIX CI runner too."""
...
class TestSmartDeniedPrompt:
    ...
    def test_smart_deny_uses_locale_specific_once_deny_choices(self, monkeypatch, capsys):
        monkeypatch.setenv("VIGIL_LANGUAGE", "tr")
...
class TestGatewayProtection:
    ...
    def test_gateway_run_backgrounded_detected(self):
        cmd = "kill 1605 && cd ~/.vigil/hermes-agent && source venv/bin/activate && python -m hermes_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_forms_still_flagged_dangerous(self):
        for cmd in (
            ...
            # In-place edit of the Vigil security config via IFS.
            "sed${IFS}-i ~/.vigil/config.yaml",
        ):
            dangerous, key, desc = detect_dangerous_command(cmd)
...
class TestPgrepKillExpansion:
    ...
    def test_kill_pidof_expansion_detected(self):
        """`kill $(pidof vigil)` is the BSD/Linux equivalent of the
        ...
        assert detect_dangerous_command("kill -9 `pidof vigil`")[0] is True
...
class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Vigil service
    label achieves the same effect as `vigil gateway stop|restart` and
    ...
    def test_unrelated_labels_not_flagged(self):
        """Read-only inspection, and lifecycle ops on non-Vigil labels, are
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("VIGIL_GATEWAY_SESSION", "VIGIL_CRON_SESSION",
                      "VIGIL_YOLO_MODE",
                      "VIGIL_SESSION_KEY", "VIGIL_INTERACTIVE")
        }
        os.environ.pop("VIGIL_YOLO_MODE", None)
        os.environ.pop("VIGIL_INTERACTIVE", None)
        # VIGIL_CRON_SESSION takes priority over VIGIL_GATEWAY_SESSION in
        ...
        os.environ.pop("VIGIL_CRON_SESSION", None)
        os.environ["VIGIL_GATEWAY_SESSION"] = "1"
        os.environ["VIGIL_SESSION_KEY"] = self.SESSION_KEY
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("hermes_cli.config.load_config_readonly", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"VIGIL_INTERACTIVE": "1"}, clear=False):
