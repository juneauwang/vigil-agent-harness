...
def test_dockerfile_does_not_chown_install_trees_to_hermes() -> None:
    text = _dockerfile_text()
    forbidden_patterns = (
        r"chown\s+-R\s+vigil:hermes\s+/opt/hermes/\.venv",
        r"chown\s+-R\s+vigil:hermes\s+/opt/hermes/ui-tui",
        r"chown\s+-R\s+vigil:hermes\s+/opt/hermes/gateway",
        r"chown\s+-R\s+vigil:hermes\s+/opt/hermes/node_modules",
    )
...
def test_dockerfile_bakes_code_scoped_install_method_stamp() -> None:
    ...
    (/opt/vigil/.install_method) first; baking it at build time keeps the
    published image self-identifying as 'docker' WITHOUT writing into the
    shared $VIGIL_HOME data volume (which a host install may also use).
    The stamp is created by root in the shim-wiring RUN block; the vigil
...
def test_dockerfile_bakes_photon_sidecar_deps() -> None:
    ...
    assert not re.search(
        r"chown\s+-R\s+vigil:hermes\s+/opt/hermes/plugins", text
    )
