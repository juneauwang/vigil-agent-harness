...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad VIGIL_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: VIGIL_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_hermes_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "VIGIL_HOME": "/h", "VIGIL_PROFILE": "p",
        "VIGIL_CONFIG": "/c.yaml", "VIGIL_ENV": "/e",
        "VIGIL_DELEGATED_CHILD_CONTEXT": "1",
        # other VIGIL_* → dropped (broad prefix removed)
        "VIGIL_BASE_URL": "https://x", "VIGIL_INTERACTIVE": "1",
        "VIGIL_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "VIGIL_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "VIGIL_BASE_URL", "VIGIL_INTERACTIVE", "VIGIL_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped VIGIL_* vars."""
    import logging
