...
``~/.vigil/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.vigil/.env`` would be told
...
class TestEndToEndRegressionGuard:
    ...
    ``~/.vigil/.env`` carrying the key while ``os.environ`` does not.
