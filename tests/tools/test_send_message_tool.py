...
class TestCheckSendMessage:
    ...
    1. ``VIGIL_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       — parent gateway is by definition running, but the worker's
       ``VIGIL_HOME`` may be a profile dir without a ``gateway.pid``).
    2. ``VIGIL_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
