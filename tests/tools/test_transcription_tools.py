...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("VIGIL_LOCAL_STT_COMMAND", raising=False)
    monkeypatch.delenv("VIGIL_LOCAL_STT_LANGUAGE", raising=False)
...
class TestTranscribeLocalCommand:
    def test_command_provider_uses_sanitized_child_env(self, monkeypatch):
        """Salvage of #56332: command STT must not inherit Vigil secrets."""
    ...
    def test_command_fallback_with_template(self, monkeypatch, sample_ogg, tmp_path):
        ...
        monkeypatch.setenv("VIGIL_LOCAL_STT_LANGUAGE", "en")
...
class TestTranscribeXAI:
    ...
    def test_sends_language_and_format(self, monkeypatch, sample_ogg, mock_xai_http_module):
        ...
        monkeypatch.setenv("VIGIL_LOCAL_STT_LANGUAGE", "fr")
...
class TestRunCommandSttIdleTimeout:
    ...
    def test_stderr_progress_extends_beyond_timeout(self, tmp_path):
        ...
        result = _run_command_stt(
            ...
            # Idle timeout still well under the ~1.6s total runtime, but with
            # enough headroom that a loaded parallel CI runner's interpreter
            # startup (which can exceed 0.1s) can't trip it before the first
            # progress tick lands.
            timeout=1.0,
        )
