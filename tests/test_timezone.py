...
class TestHermesTimeNow:
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["VIGIL_TIMEZONE"] = "UTC"
        result = hermes_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["VIGIL_TIMEZONE"] = "America/New_York"
        result = hermes_time.now()
        assert result.tzinfo is not None
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["VIGIL_TIMEZONE"] = "Europe/London"
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When VIGIL_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) VIGIL_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("VIGIL_TIMEZONE=" + os.environ.get("VIGIL_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "VIGIL_TIMEZONE=NOT_SET" in result["output"], (
            "VIGIL_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When VIGIL_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("VIGIL_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        absolute time when system-local tz != Vigil tz.  The fix interprets
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Vigil tz
        ...
        # Use a Vigil timezone far behind UTC so that the numeric wall time
        ...
        os.environ["VIGIL_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_hermes_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Vigil timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["VIGIL_TIMEZONE"] = "US/Eastern"
        _reset_hermes_time_cache()
