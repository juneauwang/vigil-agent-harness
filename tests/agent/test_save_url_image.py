...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate VIGIL_HOME under tmp_path."""
    monkeypatch.setenv("VIGIL_HOME", str(tmp_path / ".vigil"))
    (tmp_path / ".vigil").mkdir()

    # Force the constants/image cache helpers to re-read VIGIL_HOME.
    import sys
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_hermes_home_cache(self, http_server):
        ...
        # The cache directory must be under VIGIL_HOME — gateway cleanup
