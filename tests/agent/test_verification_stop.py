...
def clear_verify_env(monkeypatch):
    ...
    set VIGIL_SESSION_SOURCE and the gateway sets VIGIL_SESSION_PLATFORM.
    """
    for var in (
        "VIGIL_VERIFY_ON_STOP",
        "VIGIL_PLATFORM",
...
def test_verify_on_stop_env_can_enable(clear_verify_env):
    ...
    clear_verify_env.setenv("VIGIL_VERIFY_ON_STOP", "1")
    clear_verify_env.setenv("VIGIL_SESSION_PLATFORM", "telegram")
...
def test_verify_on_stop_auto_on_for_interactive_surfaces(clear_verify_env, source):
    ...
    clear_verify_env.setenv("VIGIL_SESSION_SOURCE", source)
...
def test_verify_on_stop_default_path_through_load_config(tmp_path, clear_verify_env):
    ...
    # cannot exercise.
    clear_verify_env.setenv("VIGIL_HOME", str(tmp_path / ".vigil"))
    ...
    clear_verify_env.setenv("VIGIL_SESSION_SOURCE", "cli")
