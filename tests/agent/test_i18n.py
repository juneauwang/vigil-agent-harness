...
def test_default_when_nothing_set(monkeypatch):
    ...
    monkeypatch.delenv("VIGIL_LANGUAGE", raising=False)
...
def test_locales_dir_env_override_ignored_when_missing(tmp_path, monkeypatch):
    """A bogus VIGIL_BUNDLED_LOCALES falls through to source/wheel resolution
    instead of returning a path that doesn't exist."""
    monkeypatch.setenv("VIGIL_BUNDLED_LOCALES", str(tmp_path / "does-not-exist"))
