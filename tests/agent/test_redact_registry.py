...
打码（低熵裸密码形态：``echo 'example-pw-4711'``）、file_read 非复用 sentinel、
...
class TestRegisterEligibility:
    def test_low_entropy_value_registers(self):
        """example-pw-4711 类低熵密码（无键名形态、不足 16 字符）可登记。"""
        assert redact.register_credential_value("example-pw-4711") is True
...
class TestRegisteredValueMasking:
    ...
    def test_all_quote_variants_masked(self):
        ...
        for text in (
            ...
            "echo example-pw-4711",
            "pw=example-pw-4711",
            "the password is example-pw-4711, keep it safe",
        ):
            out = redact.redact_sensitive_text(text, force=True)
            assert "example-pw-4711" not in out, text

    def test_file_read_uses_nonreusable_sentinel(self):
        redact.register_credential_value("example-pw-4711")
        out = redact.redact_sensitive_text(
            '{"values": ["example-pw-4711"]}', file_read=True
        )
        assert "example-pw-4711" not in out
    ...
    def test_boundary_protection_does_not_corrupt_longer_token(self):
        redact.register_credential_value("example-pw-4711")
        out = redact.redact_sensitive_text(
            "prefix example-pw-4711X suffix example-pw-4711 end", force=True
        )
        assert "example-pw-4711X" in out  # 长 token 中的前缀不被腐蚀
        assert "example-pw-4711 end" not in out
    ...
    def test_credential_values_json_self_protection(self):
        ...
        assert "example-pw-4711" in raw  # 磁盘明文（600 权限）
...
class TestRegistryPersistence:
    def test_persisted_with_0600(self):
        ...
        assert "example-pw-4711" in data["values"]
