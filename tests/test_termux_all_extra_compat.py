...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"vigil-agent-harness[termux]"' in text
    assert '"vigil-agent-harness[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"vigil-agent-harness[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
