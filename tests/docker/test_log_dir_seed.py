"""Runtime smoke test for Docker $VIGIL_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the vigil user after container boot.
...
registered later runs its log service as the dropped vigil user and
...
def test_logs_gateways_seeded_and_hermes_owned(
    ...
    """logs/ and logs/gateways/ must exist and be owned by vigil after boot."""
    ...
    assert "logs=vigil" in r.stdout, (
        f"logs/ not owned by vigil: {r.stdout}"
    )
    assert "gateways=vigil" in r.stdout, (
        f"logs/gateways/ not owned by vigil: {r.stdout}"
    )
