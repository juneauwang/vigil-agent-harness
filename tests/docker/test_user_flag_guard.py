...
def test_arbitrary_user_uid_rejected(
    ...
    assert "VIGIL_UID" in combined or "PUID" in combined, (
...
def test_user_pinned_to_hermes_uid_works(
    ...
    """docker run --user 10000:10000 (the vigil UID) must be allowed.
    ...
    assert r.returncode == 0, (
        f"--user 10000:10000 (vigil UID) was rejected: {r.stderr[-500:]}"
    )
    assert "OK" in r.stdout