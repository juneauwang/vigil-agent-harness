...
Build the real image and verify: a config.yaml present in $VIGIL_HOME
is migrated by docker_config_migrate.py on boot, running as the vigil
user.
"""
...
def test_config_migration_runs_on_boot(
    ...
    """A config.yaml in $VIGIL_HOME must be migrated on boot by
    docker_config_migrate.py, running as the vigil user."""
    # Start container
    ...
    assert "EXISTS" in r.stdout, (
        f"config.yaml not found in $VIGIL_HOME: {r.stdout}"
    )
    ...
    # Verify config.yaml is owned by vigil (migration ran as vigil)
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() == "hermes", (
        f"config.yaml not owned by vigil (migration may have run as root): "
        f"{r.stdout.strip()}"
    )
