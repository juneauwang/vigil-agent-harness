...
Before the s6 migration, ``docker run juneauwang/vigil-agent-harness gateway
...
crash and a supervised dashboard alongside (when ``VIGIL_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``VIGIL_GATEWAY_NO_SUPERVISE=1`` opts out.
...
def _wait_for_gateway_or_exit(
    ...
    *,
    ...
    ``pgrep -f 'vigil.*gateway'`` (the gateway is running) or
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → vigil gateway run) can take
    ...
    # (main-wrapper.sh → python → vigil gateway run) can take well over
    ...
    # top), NOT the static main-vigil service's sleep — a bare grep
    # for `sleep infinity` would false-positive on the main-vigil
...
def test_supervised_gateway_does_not_recurse(
    ...
    # (main-wrapper.sh → python → vigil gateway run) can take well
    ...
    # started either — so we'd see the CMD's `vigil gateway run`
    ...
    # `vigil gateway run`. If the recursion guard fails, s6 would
    # respawn fresh `gateway run` processes on every cycle, leaving
    # multiple Python-process descendants under the gateway-default
    # supervise tree.
    r = docker_exec_sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*vigil.*gateway run' | wc -l")
    assert r.returncode == 0
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``VIGIL_DASHBOARD=1`` is set, ``docker run <image> gateway
