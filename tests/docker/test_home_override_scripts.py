...
def test_dashboard_service_resets_home(
    ...
    container with VIGIL_DASHBOARD=1 and verify the dashboard process
    ...
    the process env.
    """
    start_container(built_image, container_name, "VIGIL_DASHBOARD=1", "VIGIL_DASHBOARD_HOST=127.0.0.1")

    # Check if the dashboard process is running and inspect its HOME.
    r = docker_exec_sh(
        container_name,
        # Find the dashboard process (vigil dashboard) and read its HOME
...
def test_dashboard_does_not_auto_insecure_from_host(
    ...
    VIGIL_DASHBOARD_HOST. The auth gate is the authority now.
    ...
    """
    start_container(built_image, container_name, "VIGIL_DASHBOARD=1", "VIGIL_DASHBOARD_HOST=0.0.0.0")
...
def test_stage2_repairs_profiles_and_cron_ownership(
    ...
    The stage2 hook chowns these dirs to vigil:vigil on every boot.
    ...
    # Verify files are now owned by vigil.
    r = docker_exec_sh(
        container_name,
        'stat -c "%U" /opt/data/profiles/testprof/marker '
        '/opt/data/cron/root_owned.json',
