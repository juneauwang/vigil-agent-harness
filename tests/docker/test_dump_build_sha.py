"""Regression test: ``vigil dump`` reports a real git SHA inside the container.

Background: ``.dockerignore`` excludes ``.git``, so ``git rev-parse HEAD``
fails inside the published image and ``vigil dump`` used to report
``version: ... [(unknown)]``.  The Dockerfile now writes the build-time
``$VIGIL_GIT_SHA`` build-arg to ``/opt/vigil/.hermes_build_sha`` and
...
is absent and ``vigil dump`` correctly falls back to ``(unknown)``.

This test handles both cases:

* If ``/opt/vigil/.hermes_build_sha`` exists in the image, assert that
  ``vigil dump`` surfaces its content as the version SHA (not
  ``(unknown)``).
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's vigil user (UID 10000) can bootstrap
    ...
    assert r.returncode == 0, (
        f"vigil dump exited {r.returncode}: "
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/vigil/.hermes_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    ...
    # CI path: build-arg was set, baked file exists.  ``vigil dump``
