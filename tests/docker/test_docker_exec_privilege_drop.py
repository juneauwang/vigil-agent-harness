...
The shim (docker/hermes-exec-shim.sh, installed at /opt/vigil/bin/vigil)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> vigil login` would write /opt/data/auth.json as
...
failed: Vigil is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> vigil …`` (defaulting to root) gets dropped to the
   vigil user before the real binary runs.
2. ``docker exec --user vigil <c> vigil …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $VIGIL_HOME from a ``docker exec`` session land
   as vigil:vigil — the actual user-visible invariant.
4. The VIGIL_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    finished seeding + ``chown vigil:vigil`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to vigil:vigil,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$VIGIL_HOME/logs/container-boot.log``: it is written by
...
def test_shim_drops_root_to_hermes_uid(sleep_container: str) -> None:
    ...
    We invoke `vigil` with a Python-style `-c` shim equivalent — there's no
    pure-vigil "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `vigil` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `vigil`
    do anything that writes to disk, then check the file's owner.

    Use `vigil config set` which writes config.yaml under VIGIL_HOME.
    ...
    # The written file must be owned by vigil, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "vigil:vigil", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected vigil:vigil. "
        "The shim did not drop privileges before invoking vigil."
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/vigil/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid vigil vigil <args>` which resolves `vigil`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> vigil login`` (root) wrote
    ...
    "Vigil is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `vigil config set`-style
    ...
    Specifically: pretend the operator ran `vigil login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the vigil UID. We use `vigil auth list` since that touches the
    ...
    # `vigil auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `vigil config set` first to
    # provoke a write into VIGIL_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    # VIGIL_HOME that docker exec just wrote.
    r = subprocess.run(
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the vigil user "
