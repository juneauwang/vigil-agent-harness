"""
Tests for mcp_serve — Vigil MCP server.
...
def _isolate_hermes_home(tmp_path, monkeypatch):
    """Redirect VIGIL_HOME to a temp directory."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_detects_new_message_after_db_write(self, tmp_path, monkeypatch):
        ...
        os.utime(db_path, (time.time() + 1.0, time.time() + 1.0))
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <VIGIL_HOME>/state.db for its mtime gate; the autouse
        # fixture points VIGIL_HOME at tmp_path.
        db_path = tmp_path / "state.db"
    ...
    def test_startup_baseline_suppresses_historical_replay(self, tmp_path, monkeypatch):
        ...
        # WSL/ext4 timestamps are coarse (ms granularity), so utime(None)
        # can land on the SAME tick as the baseline stat and the poll gate
        # stays closed. Bump to a distinctly future mtime instead.
        os.utime(db_path, (time.time() + 1.0, time.time() + 1.0))
        bridge._poll_once(DB())
