"""Live Fireworks smoke test — exercises the Vigil runtime, not a raw SDK client.

Opt-in only:
    VIGIL_LIVE_TESTS=1 FIREWORKS_API_KEY=fw_... \\
        pytest tests/run_agent/test_fireworks_live.py -q

Unlike a bare OpenAI() client pointed at the endpoint, this drives Vigil'
...
def _resolve_runtime_client(provider="fireworks"):
    """Build the Fireworks client the way the Vigil runtime does."""
    from agent.auxiliary_client import resolve_provider_client

    client, model = resolve_provider_client(provider)
    assert client is not None, "Vigil failed to build a Fireworks client"
    return client, model
