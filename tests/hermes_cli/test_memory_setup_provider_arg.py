"""Tests for `vigil memory setup [provider]` routing.
...
``vigil memory setup honcho``) without the interactive picker — which matters
because the per-provider ``vigil <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "vigil memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        # The hermetic conftest sets VIGIL_DISABLE_LAZY_INSTALLS=1 so no test
        ...
        with _patch.dict(os.environ, {"VIGIL_DISABLE_LAZY_INSTALLS": "0"}), \
