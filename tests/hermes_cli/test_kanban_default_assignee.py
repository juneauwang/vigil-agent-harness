...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh VIGIL_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_default_assignee_test_")
    monkeypatch.setenv("VIGIL_HOME", test_home)
    # Force-reimport so the fresh VIGIL_HOME is picked up.
    ...
    # gives each test its own monkeypatched VIGIL_HOME so no cross-test
    # contamination.
