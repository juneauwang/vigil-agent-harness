...
Every aux picker (``vigil model`` → Configure auxiliary models, the
``vigil tools`` vision picker, and any future one) must route through
...
def configured_home(tmp_path, monkeypatch):
    """A VIGIL_HOME with one ``providers:`` entry and one legacy
