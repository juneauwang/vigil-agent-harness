...
class TestCredentialPoolEndpoints:
    ...
    def test_env_seeded_delete_stays_deleted(self):
        ...
        load_pool() re-seeds from ~/.vigil/.env on every call, so removing
        just the pool row silently reverts on the next dashboard refresh.
        The endpoint must mirror `vigil auth remove`: clean up the backing
    ...
    def test_post_readd_lifts_suppression(self):
        ...
        Mirrors `vigil auth add`, which clears every suppression for the
...
class TestUpdateCheckEndpoint:
    ...
    ``POST /api/vigil/update`` runs ``vigil update``.
    """
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `vigil tools` configurator."""
...
def test_spawn_hermes_action_scrubs_gateway_loop_guard_env(monkeypatch, tmp_path):
    ...
    _VIGIL_GATEWAY=1. Spawned actions (e.g. `gateway restart`) must NOT inherit
    ...
    assert "_VIGIL_GATEWAY" not in captured["env"]
    assert captured["env"]["VIGIL_NONINTERACTIVE"] == "1"
