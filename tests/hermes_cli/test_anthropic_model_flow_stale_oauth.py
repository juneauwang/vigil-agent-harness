"""Tests for Bug #12905 fix — stale OAuth token detection in vigil model flow.

Bug 3: `vigil model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.vigil/.env but no valid
