...
def hermes_home(tmp_path, monkeypatch):
    """Isolated VIGIL_HOME so SessionDB.state_meta writes don't clobber the real one."""
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves VIGIL_HOME.
...
class TestGoalManager:
    ...
    def test_continuation_prompt_shape(self, hermes_home):
        ...
        mgr.set("port goal command to vigil")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to vigil" in prompt
