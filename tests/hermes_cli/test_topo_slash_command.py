...
def test_topo_with_host_calls_engine_and_uses_topology_credential(topo_home, monkeypatch):
    ...
    cli_obj = _make_cli()
    assert cli_obj.process_command("/topo 203.0.113.44 --env prod --yes") is True
    assert calls["host"] == "203.0.113.44"
...
def test_topo_no_host_interactive_collection(topo_home, monkeypatch):
    ...
    cli_obj = _make_cli()
    answers = iter(["203.0.113.44", "prod", "ops", "/keys/id_ed25519", "y"])
    ...
    index = topo_home / "services" / "203.0.113.44.yaml"
...
def test_topo_confirm_default_no_write(topo_home, monkeypatch):
    ...
    assert cli_obj.process_command("/topo 203.0.113.44 --env prod") is True
    out = _printed(cli_obj)
    assert "将写入" in out
...
def test_topo_write_passes_merge_and_force(topo_home, monkeypatch):
    ...
    def fake_write(home_, discovery, force=False, merge=True):
        ...
        return {"written": ["hosts/203.0.113.44.yaml"], "appended": 1, "kept": 0}
    ...
    write_calls.clear()
    assert cli_obj.process_command("/topo 203.0.113.44 --env prod --force --yes") is True
...
def test_topo_unknown_flag_reports_error(topo_home, monkeypatch):
    cli_obj = _make_cli()
    assert cli_obj.process_command("/topo 203.0.113.44 --env prod --nope") is True
    out = _printed(cli_obj)
...
def test_topo_host_without_env_requires_flag(topo_home, monkeypatch):
    cli_obj = _make_cli()
    assert cli_obj.process_command("/topo 203.0.113.44") is True
    out = _printed(cli_obj)
    assert "--env" in out
