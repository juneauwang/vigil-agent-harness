"""Tests for hermes_cli.dump._get_git_commit — git SHA resolution for ``vigil dump``.

``vigil dump`` prints the running commit so support bug reports identify the
...
written by the Dockerfile's ``VIGIL_GIT_SHA`` build-arg.
...
def test_get_git_commit_output_format_identical_between_sources(tmp_path):
    ...
    so support tooling that parses ``vigil dump`` doesn't have to special-case
