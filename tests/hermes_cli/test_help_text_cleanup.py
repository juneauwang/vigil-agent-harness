...
任务 1：用户可见 help 文案 ``~/.vigil`` → ``~/.vigil``（数据根语义不变，
        只改展示层文案；代码逻辑里的 .vigil 兼容引用保留）。
...
LEGACY_HOME_DISPLAY = "~" + "/." + "hermes"
VIGIL_HOME_DISPLAY = "~/.vigil"

# 每个白名单文件：旧数据根展示串必须消失，新 ~/.vigil 展示串必须就位。
_TASK1_STRINGS: list[tuple[str, str]] = [
    ("hermes_cli/main.py", "Inspect / prune / clear ~/.vigil/checkpoints/"),
    ("hermes_cli/main.py", "instead of storing them in ~/.vigil/.env."),
    ("hermes_cli/_parser.py", "Ignore ~/.vigil/config.yaml and fall back to built-in defaults"),
    ("hermes_cli/subcommands/acp.py", "Install agent-browser + Playwright Chromium into ~/.vigil/node/"),
    ("hermes_cli/subcommands/approvals.py", "default: ~/.vigil/state.db"),
    ("hermes_cli/subcommands/claw.py", "zip snapshot of ~/.vigil/"),
    ("hermes_cli/subcommands/claw.py", "written to ~/.vigil/backups/"),
    ("hermes_cli/subcommands/webhook.py", "script under ~/.vigil/scripts/"),
    ("hermes_cli/subcommands/hooks.py", "declared in ~/.vigil/config.yaml"),
    ("hermes_cli/subcommands/hooks.py", "consent allowlist at ~/.vigil/shell-hooks-allowlist.json"),
    ("hermes_cli/subcommands/security.py", "~/.vigil/plugins/"),
    ("hermes_cli/subcommands/skills.py", "sync manifest (~/.vigil/skills/.bundled_manifest)"),
    ("hermes_cli/subcommands/cron.py", "Path to a script under ~/.vigil/scripts/"),
    ("hermes_cli/subcommands/dashboard.py", "VIGIL_DASHBOARD_OAUTH_CLIENT_ID into ~/.vigil/.env"),
    ("hermes_cli/subcommands/gateway.py", "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.vigil/.env."),
    ("hermes_cli/subcommands/gateway.py", "GATEWAY_RELAY_WAKE_URL in ~/.vigil/.env"),
    ("hermes_cli/curator.py", "snapshot of ~/.vigil/skills/"),
    ("hermes_cli/curator.py", "Restore ~/.vigil/skills/ from a curator snapshot"),
]
...
# ---------------------------------------------------------------------------
# 任务 1：~/.vigil → ~/.vigil 用户可见文案清理
# ---------------------------------------------------------------------------

def test_help_strings_no_longer_reference_hermes_home():
    """每个白名单文件的用户可见 help 文案旧串已消失、新串已就位。"""
    for rel, expected in _TASK1_STRINGS:
        text = (REPO_ROOT / rel).read_text(encoding="utf-8")
        assert LEGACY_HOME_DISPLAY not in text, f"{rel} 仍含旧串: {LEGACY_HOME_DISPLAY!r}"
        assert expected in text, f"{rel} 缺少新串: {expected!r}"


def test_help_output_has_no_hermes_home():
    """行为探针：``vigil -h`` / ``vigil --help-all`` 输出都不含旧数据根展示串。"""
    for flag in ("-h", "--help-all"):
        proc = _run_cli(flag)
        assert proc.returncode == 0, proc.stderr
        assert LEGACY_HOME_DISPLAY not in proc.stdout, flag
...
def test_help_all_grouped_and_expanded():
    ...
    out = proc.stdout
    assert "Vigil 命令：" in out
    m = re.search(r"继承命令（来自上游，(\d+) 个）：", out)
    assert m, "缺少继承命令分组标题"
