"""Tests for ``vigil dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``vigil update``
...
class TestDashboardStatus:
    ...
    def test_status_with_processes(self, capsys):
        processes = [
            (12345, "vigil dashboard --port 9119"),
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        # for the post-`vigil update` path).
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``vigil dashboard --stop`` must not end up ALSO starting
    a new server."""
...
class TestArgparseWiring:
    ...
    ``vigil dashboard --stop`` / ``--status`` actually parse."""
