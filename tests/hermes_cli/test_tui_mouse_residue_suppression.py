...
The Python launcher (`vigil --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Vigil session
...
class TestEarlyMouseDisable:
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.setenv("VIGIL_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_oserror_is_swallowed(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["hermes", "--tui"])
        monkeypatch.delenv("VIGIL_TUI", raising=False)
        monkeypatch.delenv("VIGIL_TUI_NO_EARLY_DISABLE", raising=False)
