...
These tests run against a fake $VIGIL_HOME under tmp_path; no real
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fallback). On a host that is *also* running vigil containers, those
...
def _make_profile(
    ...
    *,
    ...
    if config:
        ...
        # `vigil profile create`. See container_boot._render_run_script.
...
def _seed_default_root(
    ...
    *,
    ...
    VIGIL_HOME root (the implicit default profile)."""
    if state is not None:
