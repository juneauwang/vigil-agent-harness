"""Tests for IRC gateway configuration via `vigil setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Vigil install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Vigil install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#vigil")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `vigil setup gateway` experience with IRC present."""
