...
PATH, so those hosts report Vigil as not installed. `setup_path()` therefore
writes a `hermes-acp` launcher next to the `vigil` one.
...
def _run_block(tmp_path: Path, use_venv: str) -> Path:
    ...
    script = (
        "set -e\n"
        f"VIGIL_BIN={hermes_bin}\n"
        f"VIGIL_ENTRYPOINT={entrypoint}\n"
...
def _extract_hermes_agent_shim_block() -> str:
    match = VIGIL_AGENT_BLOCK.search(INSTALL_SH.read_text(encoding="utf-8"))
    assert match, (
