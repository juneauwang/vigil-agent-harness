...
def test_media_delivery_denies_encrypted_bitwarden_cache(tmp_path, monkeypatch):
    ...
    hermes_home.mkdir()
    monkeypatch.setattr(base, "_VIGIL_HOME", hermes_home)
    monkeypatch.setattr(base, "_VIGIL_ROOT", hermes_home)
...
class TestExtensionlessMediaDelivery:
    ...
    def _patch_allow_root(self, monkeypatch, root):
        ...
        monkeypatch.delenv("VIGIL_MEDIA_DELIVERY_STRICT", raising=False)
...
class TestMediaDeliveryPathValidation:
    def _patch_roots(self, monkeypatch, *roots):
        ...
        monkeypatch.setenv("VIGIL_MEDIA_DELIVERY_STRICT", "1")
        ...
        monkeypatch.setenv("VIGIL_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_allows_stale_kanban_attachment_but_not_neighboring_workspace(
        ...
        monkeypatch.setenv("VIGIL_KANBAN_HOME", str(tmp_path / "hermes"))
        monkeypatch.setenv("VIGIL_MEDIA_TRUST_RECENT_FILES", "0")
    ...
    def test_recency_trust_denies_system_paths_even_when_fresh(self, tmp_path, monkeypatch):
        ...
        """
        self._patch_roots(monkeypatch)
        monkeypatch.delenv("VIGIL_MEDIA_ALLOW_DIRS", raising=False)
        monkeypatch.setenv("VIGIL_MEDIA_TRUST_RECENT_FILES", "1")
...
class TestMediaDeliveryDefaultMode:
    ...
    def test_denylist_blocks_mcp_oauth_tokens(self, tmp_path, monkeypatch, rel):
        """Live MCP OAuth tokens/client creds under ~/.vigil/mcp-tokens/ must
    ...
    def test_denylist_blocks_google_token_default_mode(self, tmp_path, monkeypatch):
        """Integration credentials at the VIGIL_HOME root (google_token.json)
    ...
    def test_strict_mode_envvar_restores_legacy_behavior(self, tmp_path, monkeypatch):
        """Setting VIGIL_MEDIA_DELIVERY_STRICT=1 reactivates the older
    ...
    def test_profile_scoped_cache_delivers_under_symlinked_root(self, tmp_path, monkeypatch):
        """Reopened #31733: a profile gateway whose VIGIL_HOME is symlinked
        under a denied prefix (e.g. /opt/data -> /root/.vigil) emits
        ...
        monkeypatch.setattr(
            "gateway.platforms.base._VIGIL_ROOT", hermes_root
        )
...
class TestGetHumanDelay:
    ...
    def test_natural_mode_ignores_malformed_custom_env_vars(self):
        env = {
            "VIGIL_HUMAN_DELAY_MODE": "natural",
            "VIGIL_HUMAN_DELAY_MIN_MS": "oops",
            "VIGIL_HUMAN_DELAY_MAX_MS": "still-bad",
        }
...
class TestMediaDeliveryDiagnosability:
    ...
    def test_rejected_path_appears_in_log(self, tmp_path, caplog):
        ...
        with patch.dict(os.environ, {"VIGIL_MEDIA_DELIVERY_STRICT": "1",
                                     "VIGIL_MEDIA_TRUST_RECENT_FILES": "0"}), \
    ...
    def test_crafted_null_path_does_not_abort_batch(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("VIGIL_MEDIA_DELIVERY_STRICT", "0")
...
class _CapturingAdapter(BasePlatformAdapter):
    ...
    chat text. That argument is a host filesystem path inside the Vigil
...
class TestMediaFallbackDoesNotLeakHostPath:
    ...
    a chat message like ``🎬 Video: /home/.../vigil/cache/video/abc.mp4``
