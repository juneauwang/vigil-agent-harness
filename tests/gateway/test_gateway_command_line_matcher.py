...
Regression guard for the Windows ``vigil gateway restart`` silent-outage bug:
...
ACCEPT = [
    ...
    "hermes-gateway.exe",
    "vigil gateway",          # bare `vigil gateway` defaults to run
    "vigil gateway run",
    # profile selector AFTER the `gateway` token (argv is profile-position
    # agnostic — _apply_profile_override strips --profile/-p anywhere)
    "vigil gateway --profile work run",
