...
substrings like 'foo@vigil_bot.example'.
...
def _make_adapter():
    ...
    adapter._bot = SimpleNamespace(id=999, username="vigil_bot")
    return adapter


def _mention_entity(text, mention="@vigil_bot"):
...
class TestSubstringFalsePositivesAreRejected:
    ...
    def test_email_like_substring(self):
        """bug #12545 exact repro: 'foo@vigil_bot.example'."""
        adapter = _make_adapter()
        msg = _message(text="email me at foo@vigil_bot.example")
    ...
    def test_substring_inside_url_without_entity(self):
        ...
        adapter = _make_adapter()
        msg = _message(text="see https://example.com/@vigil_bot for details")
        assert adapter._message_mentions_bot(msg) is False


    def test_email_substring_in_caption(self):
        adapter = _make_adapter()
        msg = _message(caption="foo@vigil_bot.example")
        assert adapter._message_mentions_bot(msg) is False
...
class TestEntityEdgeCases:
    ...
    def test_malformed_entity_with_negative_offset(self):
        adapter = _make_adapter()
        msg = _message(text="@vigil_bot hi",
...
class TestCaseInsensitivity:
    ...
    def test_uppercase_mention(self):
        adapter = _make_adapter()
        text = "hi @VIGIL_BOT"
        msg = _message(text=text, entities=[_mention_entity(text, mention="@VIGIL_BOT")])
        assert adapter._message_mentions_bot(msg) is True
