...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Vigil"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^vigil"]}
        )
        assert adapter._message_matches_mention_patterns("vigil please help") is True
        assert adapter._message_matches_mention_patterns("please vigil help") is False
