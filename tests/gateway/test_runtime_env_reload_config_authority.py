...
VIGIL_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env VIGIL_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_max_turns(tmp_path: Path, monkeypatch) -> None:
    ...
    assert os.environ["VIGIL_MAX_ITERATIONS"] == "9000"
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``vigil config get terminal.backend`` still says local.
    """
    hermes_home = tmp_path / ".vigil"
    hermes_home.mkdir()
    (hermes_home / "config.yaml").write_text(
