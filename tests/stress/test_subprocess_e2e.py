...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - worker logs are captured to VIGIL_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "VIGIL_KANBAN_TASK": task.id,
            "VIGIL_KANBAN_WORKSPACE": workspace,
...
def main():
    ...
    # Point the `vigil` CLI child processes will run at the worktree
