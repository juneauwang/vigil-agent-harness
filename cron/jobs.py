...
Jobs are stored in ~/.vigil/cron/jobs.json
Output is saved to ~/.vigil/cron/output/{job_id}/{timestamp}.md
"""
...
# store under its own VIGIL_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same VIGIL_HOME — so a job authored in profile
# `coder` lives in `~/.vigil/profiles/coder/cron/jobs.json` and executes with
...
# into one shared `jobs.json` and run them under whatever VIGIL_HOME the
...
# threshold in `vigil cron status` (hermes_cli/cron.py), so the two never
# drift apart.
...
def _current_cron_store() -> _CronStorePaths:
    ...
    3. the ACTIVE profile home, resolved fresh via get_hermes_home()
       (context-local override, then the VIGIL_HOME env var) — so a test
       or embedder that re-points VIGIL_HOME after this module was
...
# the cron inactivity timeout is disabled (VIGIL_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. VIGIL_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``VIGIL_CRON_TIMEOUT`` (the cron inactivity timeout the
    ...
    """
    raw = os.getenv("VIGIL_CRON_TIMEOUT", "").strip()
...
def _preserve_file_ownership(path: Path, before: Optional[os.stat_result]) -> None:
    ...
    command (``docker exec vigil vigil cron create ...`` — ``docker exec``
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Vigil configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Vigil timezone.
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Vigil/system local time
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    in another process (``vigil cron status``) never sees a torn/truncated
...
def get_ticker_heartbeat_age() -> Optional[float]:
    ...
    ``vigil cron status`` must report per-profile liveness (#69377).
    """
    store = _current_cron_store()
...
def record_ticker_error(message: str) -> None:
    ...
    The ticker thread lives inside the gateway process; ``vigil cron
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.vigil/scripts/; ``.sh`` / ``.bash`` files run via bash,
    ...
    # covered, not just `vigil cron create`.
...
def _machine_id() -> str:
    ...
    Uses ``VIGIL_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("VIGIL_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from VIGIL_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                ...
                # desktop both run in-process 60s tickers on one VIGIL_HOME)
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from VIGIL_CRON_TIMEOUT),
