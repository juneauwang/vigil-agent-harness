...
Thank you for contributing to Vigil! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Vigil to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo juneauwang/vigil-agent-harness "<your terms>"
  gh search prs --repo juneauwang/vigil-agent-harness --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/juneauwang/vigil-agent-harness) · [PRs (all states)](https://github.com/juneauwang/vigil-agent-harness).
...
Related: #38284 covers the agent-side analog — Vigil itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Vigil GitHub Discussions](https://github.com/juneauwang/vigil-agent-harness). Users can install it with `vigil skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.vigil/plugins/` (or via a pip entry point).
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Vigil ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.vigil/plugins/`, project `.hermes/plugins/`, or a pip entry point) — see [Build a Vigil Plugin](https://github.com/juneauwang/vigil-agent-harness)
- Register lifecycle hooks (`pre_tool_call`, `post_tool_call`, `pre_llm_call`, `post_llm_call`, `on_session_start`, `on_session_end`), tools (`ctx.register_tool`), and CLI subcommands (`ctx.register_cli_command`) through the surface we already expose — no core changes needed
- If your plugin needs a capability the framework doesn't expose, that's a feature request to **widen the generic plugin surface** (a new hook or `ctx` method) — never special-case your plugin in core
- Promote it in the [Vigil GitHub Discussions](https://github.com/juneauwang/vigil-agent-harness) `#plugins-skills-and-skins` channel so users can find and install it
...
`$VIGIL_HOME/hermes-agent` (usually `~/.vigil/hermes-agent`). That keeps your
...
uv venv ~/.vigil/venvs/hermes-dev --python 3.11
export VIRTUAL_ENV="$HOME/.vigil/venvs/hermes-dev"
...
│
├── skills/                   # Bundled skills (copied to ~/.vigil/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (github.com/juneauwang/vigil-agent-harness)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.vigil/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.vigil/`)
...
| `~/.vigil/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.vigil/cron/` | Scheduled job data |
| `~/.vigil/whatsapp/session/` | WhatsApp bridge credentials |
...
(for example `_VIGIL_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.vigil`:
...
The user may skip setup and keep loading the skill. Vigil only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
2. **Tools referenced in SKILL.md prose must be native Vigil tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Vigil" is the secondary collaborator. If the contributor's commit shows "Vigil" as author because they used Vigil to draft the skill, replace it with their actual name — credit the human, not the tool.
...
Vigil uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.vigil/skins/<name>.yaml`:
...
Vigil runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
Vigil has terminal access. Security matters.
...
- Use [GitHub Issues](https://github.com/juneauwang/vigil-agent-harness)
...
- **Discord**: [https://github.com/juneauwang/vigil-agent-harness](https://github.com/juneauwang/vigil-agent-harness) — for questions, showcasing projects, and sharing skills
