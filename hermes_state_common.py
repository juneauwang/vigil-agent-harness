...
# batch81（OPS-DELTA #96）：压缩后自检异常 → 会话标记需恢复（引擎层防挂死：
# 压缩返回的消息列表不完整时不再继续调用，UI 侧按非 running 状态提示重开）。
SESSION_STATUS_NEEDS_RECOVERY = "needs_recovery"
...
# optimized via ``vigil sessions optimize-storage``. A legacy DB sits at
...
# `vigil sessions optimize-storage` rebuilds it on a capable host.
...
# triggers repairable UNTIL the user opts into `vigil db optimize`. This is
