...
# `vigil` user (UID 10000). When an operator runs `docker exec <c> vigil ...`
# the default UID is root (0), and any file the command writes under
# $VIGIL_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> vigil login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Vigil is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> vigil
...
# When invoked as root, it drops to the vigil user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $VIGIL_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user vigil`, kanban subagents, etc. — it short-circuits
...
#
# Opt-out: set VIGIL_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the vigil CLI. Default is to drop.
...
# supervised processes (uid 10000) and for `docker exec --user vigil`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${VIGIL_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the vigil user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "hermes-shim: re-run with --user hermes or set VIGIL_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the vigil user's home before dropping privileges. Without
