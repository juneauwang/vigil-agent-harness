...
# Vigil WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: VIGIL_TINI_SHIM_TARGET overrides the `/init` path (and
# VIGIL_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${VIGIL_TINI_SHIM_TARGET:-/init}"
WRAPPER="${VIGIL_TINI_SHIM_WRAPPER:-/opt/hermes/docker/main-wrapper.sh}"
