...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{VIGIL_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``VIGIL_HOME`` is a **durable** store — on Vigil
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``vigil gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting VIGIL_HOME."""
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable VIGIL_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (VIGIL_HOME is a
    durable Fly volume on Vigil Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    marker that survived a machine restart on the durable VIGIL_HOME volume is
