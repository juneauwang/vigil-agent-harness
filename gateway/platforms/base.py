...
# Audio file extensions Vigil recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to Vigil but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Vigil' DM-topic helper
...
def build_auto_tts_output_path(platform) -> str:
    ...
    TTS tool's ``VIGIL_SESSION_PLATFORM`` contextvar — that contextvar is
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.vigil/.env manually."
)
...
MEDIA_DELIVERY_STRICT_ENV = "VIGIL_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _VIGIL_HOME / "image_cache",
    _VIGIL_HOME / "audio_cache",
    _VIGIL_HOME / "video_cache",
    _VIGIL_HOME / "document_cache",
    _VIGIL_HOME / "browser_screenshots",
    ...
    _VIGIL_HOME / "cache" / "images",
    _VIGIL_HOME / "cache" / "audio",
    _VIGIL_HOME / "cache" / "videos",
    _VIGIL_HOME / "cache" / "documents",
    _VIGIL_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Vigil root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* VIGIL_HOME's cache, so a gateway running at
    the root (e.g. ``VIGIL_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when VIGIL_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _VIGIL_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("VIGIL_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _VIGIL_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Vigil profile and shared Vigil root both contain control
    ...
    # VIGIL_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.vigil
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Vigil
    secrets (``~/.vigil/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``VIGIL_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Hermes-managed cache, under an operator-allowlisted root
    (``VIGIL_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Vigil root (~/.vigil/.env,
    ...
    # ``MEDIA:~/.vigil/google_token.json``) remain rejected.
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``VIGIL_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Vigil commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # Cross-VIGIL_HOME token takeover is armed by GatewayRunner only for
        ...
        self._busy_text_mode: str = (
            os.environ.get("VIGIL_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "VIGIL_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "VIGIL_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Vigil sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        A live cross-VIGIL_HOME holder may be replaced only when the runner
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Vigil home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Vigil home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.vigil/media/generated/stale.png"}
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          VIGIL_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          VIGIL_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          VIGIL_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("VIGIL_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("VIGIL_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("VIGIL_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
                ...
                if (self._should_auto_tts_for_chat(event.source.chat_id)
                    ...
                    try:
                        ...
                        if check_tts_requirements():
                            ...
                            # VIGIL_SESSION_PLATFORM contextvar the tool would
    ...
    def build_source(
        ...
        per-profile VIGIL_HOME isolation.
        """
