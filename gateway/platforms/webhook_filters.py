...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.vigil to the active profile home."""
...
def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under VIGIL_HOME/scripts."""
    ...
    if raw_text == "~/.vigil" or raw_text.startswith("~/.vigil/"):
