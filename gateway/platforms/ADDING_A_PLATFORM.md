...
There are two ways to add a platform to the Vigil gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.vigil/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Vigil code**.
...
Checklist for integrating a platform directly into the Vigil core.
