...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``vigil setup`` display).
    ...
    # built-ins).  Used by ``vigil gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `vigil`
        # invocation -- including plain `vigil chat`, which never touches any
        ...
        # `vigil setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``vigil setup``/``gateway status``, channel
