...
class GatewayKanbanWatchersMixin:
        ...
        while self._running:
            try:
                def _collect():
                    ...
                    # VIGIL_KANBAN_DB pins the board path; without this guard
    ...
    async def _kanban_dispatcher_watcher(self) -> None:
        ...
        try:
            ...
            return
        env_override = os.environ.get("VIGIL_KANBAN_DISPATCH_IN_GATEWAY", "").strip().lower()
        if env_override in {"0", "false", "no", "off"}:
            logger.info("kanban dispatcher: disabled via VIGIL_KANBAN_DISPATCH_IN_GATEWAY env")
            return
        ...
        def _ready_nonempty() -> bool:
            ...
            task on ANY board whose assignee maps to a real Vigil profile
            ...
            PATH, missing venv, credential loss for a real Vigil profile).
            """
            try:
                boards = _kb.list_boards(include_archived=False)
            except Exception:
        ...
        def _auto_decompose_tick(auto_decompose_per_tick: int) -> int:
            ...
            for b in boards:
                ...
                prev_env = os.environ.get("VIGIL_KANBAN_BOARD")
                try:
                    os.environ["VIGIL_KANBAN_BOARD"] = slug
                    try:
                    ...
                    if prev_env is None:
                        ...
                        os.environ["VIGIL_KANBAN_BOARD"] = prev_env
            return successes
