...
* Stateless request/response adapters (the API server,
  ...
  ``X-Vigil-Session-Id`` key real gateway/HQ turns run under
  ...
  server with the raw session id in the ``X-Vigil-Session-Id`` header — the
...
async def deliver_wake(
    ...
    *,
    ...
    ``session_id`` is the RAW session id (the ``X-Vigil-Session-Id`` value /
...
async def _self_post_chat_completion(
    ...
    Session continuation via ``X-Vigil-Session-Id`` is 403-gated on
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "X-Vigil-Session-Id is rejected (403) on an unauthenticated API "
